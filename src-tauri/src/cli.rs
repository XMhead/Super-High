...
fn dispatch(args: &[String], stdout: &mut dyn Write) -> Result<()> {
    ...
    match command.as_str() {
        ...
        "items" => crate::cli_items::handle_items(&mut args, stdout),
        "memory" => handle_memory(&mut args, stdout),
        "prompt" => handle_prompt(&mut args, stdout),
        "search" => handle_search(&mut args, stdout),
        "service" => crate::cli_service::handle_service(&mut args, stdout),
        "terminal" => crate::cli_terminal::handle_terminal(&mut args, stdout),
        "tools" => crate::cli_tools::handle_tools(&mut args, stdout),
        _ => bail!("unknown command `{command}`. Run `superhigh-cli --help`."),
    }
}

fn handle_prompt(args: &mut Vec<String>, stdout: &mut dyn Write) -> Result<()> {
    if args.is_empty() || is_help(&args[0]) {
        writeln!(stdout, "Usage:\n  superhigh-cli prompt spacing [text...] [--timeout-ms <n>] [--json]\n  superhigh-cli prompt spacing status [--json]\n  superhigh-cli prompt spacing setup [--json]\n\nReads UTF-8 text from stdin when text is omitted.")?;
        return Ok(());
    }
    let subcommand = args.remove(0);
    if subcommand != "spacing" {
        bail!("unknown prompt command `{subcommand}`. Run `superhigh-cli prompt --help`.");
    }
    ...
    if args.first().is_some_and(|value| value == "status") {
        ...
        let status = crate::prompt_spacing::status(&crate::prompt_spacing::cli_resource_dir());
        if json {
    ...
    if args.first().is_some_and(|value| value == "setup") {
        args.remove(0);
        ensure_no_unexpected_options(args)?;
        ensure_no_positionals(args)?;
        tokio::runtime::Runtime::new()?.block_on(crate::prompt_spacing::install(
            &crate::prompt_spacing::cli_resource_dir(),
        ))?;
        if json {
            return write_json(
                stdout,
                &crate::prompt_spacing::status(&crate::prompt_spacing::cli_resource_dir()),
            );
        }
        writeln!(stdout, "prompt spacing resources are ready")?;
        return Ok(());
    }
    let timeout_ms = take_option(args, "--timeout-ms")?
        .map(|value| {
            value
                .parse::<u64>()
    ...
    let text = crate::prompt_spacing::normalize_with_resource_dir_and_timeout(
        &prompt,
        &crate::prompt_spacing::cli_resource_dir(),
    ...
    if json {
        return write_json(
            stdout,
            &crate::prompt_spacing::PromptSpacingCliResult {
                text,
...
Usage:
  ...
  superhigh-cli files <ls|read|write|touch|mkdir|rename|delete> ...
  superhigh-cli items <search|keys> ...
  ...
  superhigh-cli terminal <list|logs|create|write|key|resize|close> ...
  superhigh-cli tools <list|run> ...

Options:
  --json            All commands support machine-readable JSON.
  --project <path>  Supported by items/memory/search/service/terminal/tools.
                    service requires an absolute path; terminal can query all projects.
                    Other project commands default to the current directory.
  Run superhigh-cli <command> --help for complete arguments and options.
"
    )?;
    Ok(())
}
