...
mod tests {
    ...
    fn mobile_preferences_patch_preserves_other_and_unknown_fields() {
        let directory = tempfile::tempdir().unwrap();
        let db = AppDb::new(directory.path()).unwrap();
        db.connection.lock().unwrap().execute("INSERT INTO app_settings (id, json) VALUES (1, ?1)", params![r#"{"themeId":"old","hiddenCliProviderIds":["dsh"],"futureSetting":{"keep":true}}"#]).unwrap();
        let settings = db.patch_mobile_preferences(Some("new".into()), None).unwrap();
        assert_eq!(settings.theme_id, "new");
        assert_eq!(settings.hidden_cli_provider_ids, vec!["dsh"]);
