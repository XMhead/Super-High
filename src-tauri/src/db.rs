...
use crate::models::{AppSettings, MemoRecord, RecentProject, SshConnectionConfig};
...
impl AppDb {
    ...
    pub fn list_ssh_connections(&self) -> anyhow::Result<Vec<SshConnectionConfig>> {
        let connection = self.connection.lock().unwrap();
        let mut statement = connection.prepare(
            "SELECT id, name, host, port, username, extra_args, created_at, updated_at FROM ssh_connections ORDER BY name",
        )?;
        let rows = statement.query_map([], |row| {
    ...
    pub fn save_ssh_connection(&self, config: &SshConnectionConfig) -> anyhow::Result<()> {
        let connection = self.connection.lock().unwrap();
        connection.execute(
            "INSERT INTO ssh_connections (id, name, host, port, username, extra_args, created_at, updated_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)
             ON CONFLICT(id) DO UPDATE SET
               name=excluded.name, host=excluded.host, port=excluded.port,
               username=excluded.username, extra_args=excluded.extra_args,
    ...
    pub fn delete_ssh_connection(&self, id: &str) -> anyhow::Result<()> {
        let connection = self.connection.lock().unwrap();
        connection.execute("DELETE FROM ssh_connections WHERE id = ?1", params![id])?;
        Ok(())
    }
