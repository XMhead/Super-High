...
impl AppDb {
    ...
    fn migrate(&self) -> anyhow::Result<()> {
      ...
      INSERT OR IGNORE INTO project_data_registry (path) SELECT path FROM recent_projects;
      ...
      CREATE TABLE IF NOT EXISTS mobile_host_instances (
    ...
    pub fn upsert_recent_project(&self, project: &RecentProject) -> anyhow::Result<()> {
        let connection = self.connection.lock().unwrap();
        connection.execute(
            "INSERT OR IGNORE INTO project_data_registry (path) VALUES (?1)",
            params![project.path],
        )?;
        connection.execute(
            r#"
    ...
    pub fn upsert_mobile_host_instance(
        &self,
        instance_id: &str,
        process_id: u32,
        port: u16,
        ...
        self.with_connection(|connection| {
            connection.execute(
                "DELETE FROM mobile_host_instances WHERE process_id = ?1 AND instance_id <> ?2",
                params![i64::from(process_id), instance_id],
            )?;
            connection.execute(
                r#"
                INSERT INTO mobile_host_instances (instance_id, process_id, port, active_project_path, updated_at)
                ...
                "#,
                params![instance_id, i64::from(process_id), i64::from(port), active_project_path],
            )?;
            Ok(())
        })
    }

    pub fn remove_mobile_host_instance(&self, instance_id: &str) -> anyhow::Result<()> {
        self.with_connection(|connection| {
            connection.execute(
                "DELETE FROM mobile_host_instances WHERE instance_id = ?1",
                params![instance_id],
            )?;
            Ok(())
        })
    ...
    pub fn list_mobile_host_instances(&self) -> anyhow::Result<Vec<MobileHostInstance>> {
        self.with_connection(|connection| {
            let mut statement = connection.prepare(
                "SELECT instance_id, process_id, port, active_project_path FROM mobile_host_instances ORDER BY updated_at DESC",
            )?;
            let rows = statement.query_map([], |row| {
                Ok(MobileHostInstance {
                    instance_id: row.get(0)?,
                    process_id: u32::try_from(row.get::<_, i64>(1)?).map_err(|error| {
                        rusqlite::Error::FromSqlConversionFailure(1, rusqlite::types::Type::Integer, Box::new(error))
                    })?,
                    port: u16::try_from(row.get::<_, i64>(2)?).map_err(|error| {
                        rusqlite::Error::FromSqlConversionFailure(2, rusqlite::types::Type::Integer, Box::new(error))
                    })?,
                    active_project_path: row.get(3)?,
                })
            })?;
...
mod tests {
    ...
    #[test]
    fn mobile_host_instances_track_each_workspace_and_replace_reused_process_ids() {
        let directory = tempfile::tempdir().unwrap();
        let db = AppDb::new(directory.path()).unwrap();
        db.upsert_mobile_host_instance("first", 41, 10320, Some("D:/工作区甲")).unwrap();
        db.upsert_mobile_host_instance("second", 42, 43125, Some("D:/工作区乙")).unwrap();
        let instances = db.list_mobile_host_instances().unwrap();
        assert_eq!(instances.len(), 2);
        assert!(instances.iter().any(|instance| instance.port == 43125
            && instance.active_project_path.as_deref() == Some("D:/工作区乙")));

        db.upsert_mobile_host_instance("replacement", 42, 44000, Some("D:/new")).unwrap();
        let instances = db.list_mobile_host_instances().unwrap();
        assert_eq!(instances.len(), 2);
        assert!(!instances.iter().any(|instance| instance.instance_id == "second"));
        db.remove_mobile_host_instance("replacement").unwrap();
        assert_eq!(db.list_mobile_host_instances().unwrap().len(), 1);
    }
