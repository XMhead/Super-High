...
/// Lightweight cross-workflow references shared by project cards.
...
impl WorkflowRefs {
    pub fn is_empty(&self) -> bool {
        self.cards.is_empty() && self.docs.is_empty() && self.files.is_empty()
    }
}
...
/// An issue card. Persisted as a markdown file with frontmatter under
/// `<project>/.superhigh/cards/<status-dir>/<id>.md`, so a proposing CLI can
// ── Remote workspace (Tailscale + SMB) ──

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RemoteConnectionConfig {
    /// Tailscale IP or hostname of the remote server.
    pub remote_host: String,
    /// SMB share name on the remote server.
    pub share_name: String,
    /// Local drive letter to map (e.g. "S").
    pub local_drive_letter: String,
    /// Optional subdirectory under the share to use as workspace root.
    pub remote_root: String,
    /// Windows username on the remote server.
    pub username: String,
}
...
pub struct RemoteDriveStatus {
    ...
    /// Whether Tailscale CLI is available locally.
    pub tailscale_available: bool,
    /// Whether the drive letter is currently mapped.
    pub drive_mapped: bool,
    /// The UNC path (e.g. "\\100.x.x.x\Projects").
    pub unc_path: String,
    /// The local path (e.g. "S:\").
    pub local_path: String,
    /// The workspace root path (local_path + remote_root).
    pub workspace_root: String,
    /// Individual status checks.
    pub checks: Vec<RemoteServerCheck>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RemoteServerCheck {
    pub id: String,
    pub label: String,
    pub ok: bool,
    pub detail: String,
