...
use dsh_profiles::{DshProfileInfo, DshProviderInfo};
use fs_ops::{
...
use models::{
    ...
    ProjectSearchResult, RecentProject, RemoteConnectionConfig, RemoteDriveStatus, RemoteMapResult,
    SshConnectionConfig, TerminalSessionDto, VitePressDocsInfo, Workspace, WorkspaceWatchTarget,
};
...
#[tauri::command]
fn list_dsh_providers_command() -> Vec<DshProviderInfo> {
    let _ = dsh_editor::ensure_runtime();
    dsh_profiles::list_dsh_providers()
}

#[tauri::command]
fn list_dsh_profiles_command() -> Vec<DshProfileInfo> {
    let _ = dsh_editor::ensure_runtime();
    dsh_profiles::list_dsh_profiles()
}

/// dsh 全局默认路由（config.toml 的 active_provider / active_model）。
#[tauri::command]
fn get_dsh_default_model_command() -> Option<DshDefaultModel> {
    let _ = dsh_editor::ensure_runtime();
    dsh_profiles::list_dsh_default().map(|(provider, model)| DshDefaultModel { provider, model })
}
...
/// dsh 供应商测试用 API Key（config.toml）；无则 None。
#[tauri::command]
fn get_dsh_provider_api_key_command(provider_id: String) -> Option<String> {
    let _ = dsh_editor::ensure_runtime();
    dsh_profiles::api_key_for_provider(&provider_id)
}

#[tauri::command]
fn add_dsh_provider_command(
    provider: dsh_editor::DshProviderInput,
) -> Result<Vec<DshProviderInfo>, String> {
    dsh_editor::add_dsh_provider(provider)
}

#[tauri::command]
fn update_dsh_provider_command(
    #[allow(non_snake_case)] originalId: String,
    provider: dsh_editor::DshProviderInput,
) -> Result<Vec<DshProviderInfo>, String> {
    dsh_editor::update_dsh_provider(&originalId, provider)
}

#[tauri::command]
fn delete_dsh_provider_command(
    #[allow(non_snake_case)] providerId: String,
) -> Result<Vec<DshProviderInfo>, String> {
    dsh_editor::delete_dsh_provider(&providerId)
}

/// 设置 dsh 全局默认模型（写 config.toml，并同步 rc.6 兼容路由）。
#[tauri::command]
fn set_dsh_default_model_command(
    #[allow(non_snake_case)] providerId: String,
    #[allow(non_snake_case)] modelId: String,
) -> Result<(), String> {
    dsh_editor::set_dsh_default_model(&providerId, &modelId)
}

/// 删除不需要的 dsh 分组目录（~/.dsh/profiles/<id>）。dsh-tui 启动分组拒绝删除。
#[tauri::command]
fn delete_dsh_profile_command(
    #[allow(non_snake_case)] profileId: String,
) -> Result<Vec<DshProfileInfo>, String> {
    dsh_editor::delete_dsh_profile(&profileId)
}
...
#[tauri::command]
fn default_remote_config_command() -> RemoteConnectionConfig {
    default_remote_connection_config()
}

#[tauri::command]
fn remote_drive_status_command(config: RemoteConnectionConfig) -> RemoteDriveStatus {
    remote_drive_status(&config)
}

#[tauri::command]
fn map_remote_drive_command(config: RemoteConnectionConfig) -> RemoteMapResult {
    map_remote_drive(&config)
}

#[tauri::command]
fn unmap_remote_drive_command(letter: String) -> RemoteMapResult {
    unmap_remote_drive(&letter)
}

#[tauri::command]
fn remote_map_script_command(config: RemoteConnectionConfig) -> String {
    remote_server::map_script(&config)
}

#[tauri::command]
fn remote_verify_script_command(config: RemoteConnectionConfig) -> String {
    remote_server::verify_script(&config)
}

#[tauri::command]
fn remote_tailscale_status_command() -> serde_json::Value {
    serde_json::json!({
        "available": remote_server::tailscale_available(),
        "ip": remote_server::tailscale_ip(),
    })
}
...
fn start_mobile_host_command(
    ...
    state
        .mobile_host
        .start(Arc::clone(&state.db), config)
        .map_err(|error| error.to_string())
}
...
/// 创建受用户显式点击触发的学习任务文件，并由当前 DSH 会话读取执行。
/// 对话原文仅保存在当前工作区的运行时文件中；全局知识库由 Agent-Learning skill
/// 在确认内容可复用、已脱敏后自行维护。
#[tauri::command]
fn create_agent_learning_task_command(
    workspace_path: String,
    ...
    source_cwd: String,
    transcript: String,
    native_transcript: Option<NativeCliTranscript>,
) -> Result<String, String> {
    let workspace = PathBuf::from(&workspace_path);
    let transcript = transcript.trim();
    if transcript.is_empty() {
        return Err("当前会话没有可供学习的对话记录".to_string());
    }
    if transcript.len() > 512 * 1024 {
        return Err("当前会话记录过大，请先缩小到需要沉淀的内容".to_string());
    }
    ...
    let task_path = task_dir.join(format!("learn-{}.md", Uuid::new_v4()));
    let native_metadata = native_transcript
        .as_ref()
        .map(|value| {
            format!(
                "- nativeThreadId: {}\\n- nativeTranscriptPath: {}\\n",
                value.native_thread_id, value.source_path
            )
        })
        .unwrap_or_else(|| "- nativeThreadId: unavailable\\n".to_string());
    let preferred_transcript = native_transcript
        .as_ref()
        .map(|value| value.transcript.as_str())
        .unwrap_or(transcript);
    let transcript_kind = if native_transcript.is_some() {
        "Codex JSONL（结构化原生记录）"
    } else {
        "SuperHigh 可见终端和输入记录"
    };
    let content = format!(
        "# Agent-Learning 任务\\n\\n这是用户在 SuperHigh 中显式点击“学习此对话”后创建的任务。\\n\\n## 来源元数据\\n\\n- source: SuperHigh embedded CLI\\n- provider: {}\\n- superhighSessionId: {}\\n- sessionTitle: {}\\n- cwd: {}\\n- createdAt: {}\\n{}\\n## 执行要求\\n\\n1. 先读取全局 `Agent-Learning` skill。\\n2. 仅分析下方这份明确指定的会话记录，不扫描其他历史会话。\\n3. 提炼可观察的需求澄清、决策依据、工具调用链、验证和修正；不要编造或复述不可观察的内部思维链。\\n4. 先向用户展示候选沉淀和脱敏结果；只有用户确认后才写入全局知识库。\\n5. 全局知识库不得保存项目私有凭据、原始敏感内容或不可验证结论。\\n\\n## 会话记录（{}）\\n\\n```text\\n{}\\n```\\n",
        source_provider.trim(),
        source_session_id.trim(),
        source_session_title.trim(),
        source_cwd.trim(),
        "由 SuperHigh 本地时间创建",
        native_metadata,
        transcript_kind,
    ...
    Ok(task_path.to_string_lossy().replace('\\', "/"))
}
...
#[tauri::command]
fn list_ssh_connections_command(
    state: State<'_, AppState>,
...
#[tauri::command]
fn save_ssh_connection_command(
    state: State<'_, AppState>,
    config: SshConnectionConfig,
) -> Result<(), String> {
    state
        .db
        .save_ssh_connection(&config)
        .map_err(|error| error.to_string())
}

#[tauri::command]
fn delete_ssh_connection_command(state: State<'_, AppState>, id: String) -> Result<(), String> {
    state
        .db
...
#[tauri::command]
fn create_ssh_terminal_session_command(
    app: tauri::AppHandle,
    state: State<'_, AppState>,
    workspace_id: Option<String>,
    ...
    let config = state
        ...
        .ok_or_else(|| "SSH 连接配置未找到".to_string())?;
    state
        .terminal
        .create_ssh_session(&app, workspace_id, &config, &PathBuf::from(cwd))
        .map_err(|error| error.to_string())
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            list_dsh_providers_command,
            list_dsh_profiles_command,
            get_dsh_default_model_command,
            get_dsh_provider_api_key_command,
            add_dsh_provider_command,
            update_dsh_provider_command,
            delete_dsh_provider_command,
            set_dsh_default_model_command,
            delete_dsh_profile_command,
            ...
            default_remote_config_command,
            remote_drive_status_command,
            map_remote_drive_command,
            unmap_remote_drive_command,
            remote_map_script_command,
            remote_verify_script_command,
            remote_tailscale_status_command,
            ...
            create_agent_learning_task_command,
            ...
            list_ssh_connections_command,
            save_ssh_connection_command,
            delete_ssh_connection_command,
            create_ssh_terminal_session_command,
            open_path_command,
...
mod tests {
    ...
    fn explorer_select_arg_keeps_flag_and_file_path_separate() {
        let path = PathBuf::from(r"D:\Work\app\src\App.vue");

        assert_eq!(
            explorer_args_for_path(&path, true),
            vec![
                "/select,".to_string(),
                r"D:\Work\app\src\App.vue".to_string()
            ]
        );
    }
    ...
    fn content_security_policy_allows_data_image_previews() {
        ...
        assert!(
            ...
            "CSP must allow img-src data: for image previews"
        );
    }
    ...
    fn media_file_associations_are_registered_as_viewers() {
        ...
        for expected in [
            "png", "jpg", "jpeg", "gif", "webp", "bmp", "svg", "ico", "avif", "mp4", "webm", "ogg",
            "ogv", "mov",
        ] {
    ...
    fn explorer_folder_arg_is_the_folder_path_only() {
        let path = PathBuf::from(r"D:\Work\app\src");

        assert_eq!(
            explorer_args_for_path(&path, false),
            vec![r"D:\Work\app\src".to_string()]
        );
    }
    ...
    fn strips_extended_length_prefix_before_passing_to_explorer() {
        ...
        assert_eq!(explorer_path_string(&path), r"D:\Work\app\src\App.vue");
    }
