...
use fs_ops::{
    ...
    read_image_as_data_url, read_media_as_data_url, read_text_file, rename_path,
    replace_image_from_clipboard, resolve_super_high_root, search_project_files,
    write_text_file,
};
use mobile_host::{MobileHostManager, SharedMobileHostManager};
use models::{
    AppSettings, AppStorageInfo, ChannelConfig, ChannelProbeResult, CliProviderEnvironment, EndpointLatencyResult, FetchedModel, MemoRecord,
    MobileHostConfig, MobileHostStatus, PluginListResponse, PluginShellExecRequest,
...
#[tauri::command]
fn create_project_startup_terminal_session_command(
    app: tauri::AppHandle,
...
#[tauri::command]
fn start_project_services_command(
    app: tauri::AppHandle,
    state: State<'_, AppState>,
    workspace_id: Option<String>,
    project_path: String,
...
#[tauri::command]
fn list_directory_command(path: String) -> Result<models::DirectoryListing, String> {
...
#[tauri::command]
fn sync_workspace_watch_roots_command(
    app: tauri::AppHandle,
    state: State<'_, AppState>,
...
#[tauri::command]
fn sync_workspace_watch_targets_command(
    app: tauri::AppHandle,
    state: State<'_, AppState>,
...
#[tauri::command]
fn create_terminal_session_command(
    app: tauri::AppHandle,
    state: State<'_, AppState>,
    workspace_id: Option<String>,
...
#[tauri::command]
fn detect_cli_environments_command() -> Result<Vec<CliProviderEnvironment>, String> {
...
#[tauri::command]
fn close_terminal_session_command(
    state: State<'_, AppState>,
    session_id: String,
) -> Result<(), String> {
    state
...
#[tauri::command]
fn close_terminal_sessions_for_workspace_command(
    state: State<'_, AppState>,
    workspace_id: String,
) -> Result<(), String> {
    state
        .terminal
        .close_sessions_for_workspace(&workspace_id)
        .map_err(|error| error.to_string())
}
...
#[tauri::command]
fn detect_vitepress_docs_command(
    state: State<'_, AppState>,
    project_path: String,
...
#[tauri::command]
fn ensure_vitepress_docs_command(
    state: State<'_, AppState>,
    project_path: String,
) -> Result<VitePressDocsInfo, String> {
    state
...
#[tauri::command]
fn stop_vitepress_docs_command(
    state: State<'_, AppState>,
    project_path: String,
) -> Result<VitePressDocsInfo, String> {
    state
...
#[tauri::command]
fn shutdown_app_command(app: tauri::AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    let terminal_result = state.terminal.close_all_sessions();
    let vitepress_result = state.vitepress.stop_all();
    state.mobile_host.stop();

    if let Err(error) = terminal_result {
        return Err(error.to_string());
    }
    if let Err(error) = vitepress_result {
        return Err(error.to_string());
    }
...
fn normalize_open_path_input(path: &str) -> PathBuf {
    normalize_drive_relative_path(PathBuf::from(path))
}
...
mod tests {
    use super::{
        ...
        normalize_path, startup_target_from_cli_arg, workspace_folder_from_cli_arg,
    };
    ...
    #[test]
    fn content_security_policy_allows_data_media_previews() {
        let config_path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tauri.conf.json");
        let config = fs::read_to_string(config_path).unwrap();
        let config: Value = serde_json::from_str(&config).unwrap();
        let csp = config["app"]["security"]["csp"].as_str().unwrap();
        ...
        assert!(
            ...
            "CSP must allow media-src data: for file-manager video previews"
        );
    }
