...
mod tests {
    ...
    fn workspace(root: &Path) -> PluginWorkspaceContext {
        PluginWorkspaceContext {
            name: "示例项目目录名服务端".to_string(),
            display_name: "示例项目目录名服务端".to_string(),
    ...
    fn discovers_matching_plugin_by_workspace_name_and_exists() {
        ...
        let project = temp.path().join("示例项目目录名服务端");
        fs::create_dir_all(project.join("SuperCore源码")).unwrap();
        ...
        write_plugin(
            ...
            r#"{
              ...
              "main":"main.js",
              "style":"style.css",
              "activation":{"all":[{"workspaceName":"示例项目目录名服务端"},{"exists":"SuperCore源码"}]},
              "permissions":["all"],
              "unsafe":true
            }"#,
        );
    ...
    fn marks_unmatched_when_workspace_missing() {
        ...
        write_plugin(
            &temp.path().join("plugins"),
            "horizon-tools",
            r#"{"id":"horizon-tools","name":"示例项目目录名工具","version":"0.1.0","main":"main.js","activation":{"exists":"AGENTS.md"}}"#,
        );
    ...
    fn disables_all_plugins_when_cli_flag_set() {
        ...
        write_plugin(
            &temp.path().join("plugins"),
            "horizon-tools",
            r#"{"id":"horizon-tools","name":"示例项目目录名工具","version":"0.1.0","main":"main.js"}"#,
        );
    ...
    fn disables_selected_plugin_ids() {
        ...
        let response =
            discover_plugins(temp.path(), None, &["horizon-tools".to_string()], false).unwrap();
        assert_eq!(response.plugins[0].status, PluginRuntimeStatus::Disabled);
    }
    ...
    fn loads_project_local_plugins_and_prefers_them_over_global() {
        let temp = tempfile::tempdir().unwrap();
        let project = temp.path().join("示例项目服务端");
        ...
        write_plugin(
            &project_plugins,
            "tide-tools",
            r#"{"id":"tide-tools","name":"示例项目工具","version":"0.1.0","main":"main.js"}"#,
        );
