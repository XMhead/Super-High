...
use crate::{
    ...
    models::{MobileHostApiStatus, MobileHostConfig, MobileHostStatus, MobileWorkspaceHost, ProjectSearchOptions},
...
impl Default for MobileHostManager {
    fn default() -> Self {
        Self {
            lifecycle: Mutex::new(()),
...
impl MobileHostManager {
    pub fn set_active_project(&self, path: Option<String>) -> anyhow::Result<()> {
        *self.active_project.lock().map_err(|_| anyhow::anyhow!("当前工作区状态不可用"))? = path.clone();
        if let Some(runtime) = self.runtime.lock().unwrap().as_ref() {
            runtime.db.upsert_mobile_host_instance(
                &self.instance_id,
                std::process::id(),
                runtime.port,
                path.as_deref(),
    ...
    pub fn start(
        ...
        self.start_runtime(db, app, terminal, config)
    }
    ...
    fn start_runtime(
        ...
        let listener = bind_mobile_host_listener(config.port)?;
        listener
        ...
        let active_project_path = self.active_project.lock().ok().and_then(|path| path.clone());
        db.upsert_mobile_host_instance(
            &self.instance_id,
            std::process::id(),
            port,
    ...
    fn stop_runtime(&self) {
        if let Some(mut runtime) = self.runtime.lock().unwrap().take() {
            stop_listener_thread(&runtime.stop, runtime.listener_thread.take());
            let _ = runtime.db.remove_mobile_host_instance(&self.instance_id);
        }
    }
    ...
    pub fn synchronize_enabled(
        ...
        let _lifecycle = self.lifecycle.lock().unwrap();
        if self.monitor_stop.load(Ordering::Relaxed) || !db.mobile_host_enabled()? {
            self.stop_runtime();
            return Ok(());
        }
    ...
    pub fn matches_config(&self, config: &MobileHostConfig) -> bool {
        let config = normalized_config(config.clone());
        self.runtime.lock().unwrap().as_ref().is_some_and(|runtime| {
            runtime.configured_port == config.port && runtime.token == config.token
        })
    }
    ...
    pub fn monitoring_stopped(&self) -> bool {
        self.monitor_stop.load(Ordering::Relaxed)
    }
...
fn stop_listener_thread(stop: &AtomicBool, listener_thread: Option<thread::JoinHandle<()>>) {
...
fn bind_mobile_host_listener(port: u16) -> anyhow::Result<TcpListener> {
    let listener = match TcpListener::bind((BIND_ADDRESS, port)) {
        Ok(listener) => listener,
        Err(error) if error.kind() == std::io::ErrorKind::AddrInUse => {
            TcpListener::bind((BIND_ADDRESS, 0)).with_context(|| {
                format!("failed to bind mobile host after port {port} was occupied")
            })?
        }
        Err(error) => return Err(error)
            .with_context(|| format!("failed to bind mobile host on port {port}")),
    };
    Ok(listener)
}
...
fn handle_status(stream: &mut TcpStream, context: &MobileHostContext) {
    ...
    let status = MobileHostApiStatus {
        ...
        workspace_hosts: workspace_hosts(&context.db),
...
fn workspace_hosts(db: &AppDb) -> Vec<MobileWorkspaceHost> {
    let Ok(instances) = db.list_mobile_host_instances() else {
        return Vec::new();
    };
    let mut hosts = Vec::new();
    for instance in instances {
        if !process_is_running(instance.process_id) {
            let _ = db.remove_mobile_host_instance(&instance.instance_id);
            continue;
        }
        let Some(path) = instance.active_project_path else { continue; };
        if hosts.iter().any(|host: &MobileWorkspaceHost| paths_equal(&host.path, &path)) {
            continue;
        }
        hosts.push(MobileWorkspaceHost { path, port: instance.port });
    }
    hosts
}

fn paths_equal(left: &str, right: &str) -> bool {
    left.trim_end_matches(['/', '\\']).replace('\\', "/").eq_ignore_ascii_case(
        &right.trim_end_matches(['/', '\\']).replace('\\', "/"),
    )
}
...
mod tests {
    ...
    #[test]
    fn occupied_configured_port_gets_a_distinct_instance_listener() {
        let first = std::net::TcpListener::bind((super::BIND_ADDRESS, 0)).unwrap();
        let occupied = first.local_addr().unwrap().port();
        let second = super::bind_mobile_host_listener(occupied).unwrap();
        assert_ne!(second.local_addr().unwrap().port(), occupied);
    }

    #[test]
    fn joined_listener_thread_releases_the_configured_port_before_restart() {
        let listener = std::net::TcpListener::bind((super::BIND_ADDRESS, 0)).unwrap();
        ...
        super::stop_listener_thread(&stop, Some(listener_thread));
        let rebound = super::bind_mobile_host_listener(port).unwrap();
        assert_eq!(rebound.local_addr().unwrap().port(), port);
    }

    #[cfg(windows)]
    #[test]
    fn workspace_hosts_keep_live_instances_and_prune_dead_ones() {
        ...
        db.upsert_mobile_host_instance("dead", u32::MAX, 44000, Some("E:/stale")).unwrap();
        let hosts = super::workspace_hosts(&db);
        assert_eq!(hosts.len(), 1);
        assert_eq!(hosts[0].path, r"D:\工作区甲\");
        assert_eq!(hosts[0].port, 43125);
        assert!(db.list_mobile_host_instances().unwrap().iter().all(|instance| instance.instance_id != "dead"));
        assert!(super::paths_equal(r"D:\工作区甲\", "d:/工作区甲"));
    }
