...
use crate::models::{
    CliProviderEnvironment, NativeCliTranscript, SshConnectionConfig, TerminalBufferSnapshot,
...
struct CliLaunch {
    ...
    /// Environment values owned by SuperHigh for this child only.  DSH API
    /// keys are loaded from config.toml here and never persisted to the OS
...
// Full-screen TUIs such as dsh-tui repaint large synchronized frames; keep a
...
// dsh-tui treats ordinary newlines received from stdin as Enter. Messages
// sent by Super High must use the terminal bracketed-paste protocol so a
// multi-line draft remains one prompt until the final Enter is written.
const DSH_BRACKETED_PASTE_START: &str = "\x1b[200~";
const DSH_BRACKETED_PASTE_END: &str = "\x1b[201~";
const DSH_BRACKETED_PASTE_SUBMIT_DELAY_MS: u64 = 50;

const CLI_PROVIDERS: [CliProviderDefinition; 7] = [
...
impl TerminalManager {
    pub fn update_blocking_sessions(&self) -> anyhow::Result<Vec<UpdateBlockingSession>> {
        ...
        for (id, session) in sessions.iter() {
            if let Some(child) = &session.child {
                let mut child = child.lock().map_err(|_| anyhow::anyhow!("终端进程状态不可用"))?;
                if child.try_wait().context("无法确认终端是否已退出")?.is_none() {
                    blocking.push(UpdateBlockingSession { id: id.clone(), title: session.title.clone() });
                }
            }
        }
    ...
    pub fn install_update_when_idle(&self, install: impl FnOnce() -> anyhow::Result<()>) -> anyhow::Result<()> {
    ...
    fn create_session_from_launch(
        ...
        // DSH's config.toml is authoritative for its child credentials.
        // Apply these after caller-provided session variables so a stale
        // inherited value cannot silently replace the selected provider key.
        for (key, value) in &launch.child_env {
            builder.env(key, value);
        }
    ...
    pub fn write_input(&self, session_id: &str, input: &str) -> anyhow::Result<()> {
        ...
        if session.provider_kind == "dsh" {
            write_dsh_terminal_input_to_writer(writer, input)
        } else {
...
/// Encode application-provided multi-line dsh input as one bracketed paste.
///
/// The final carriage return is the explicit submit action used by the
/// conversation box.  It must stay outside the paste markers; otherwise
/// dsh-tui correctly inserts it as content instead of submitting the draft.
fn prepare_dsh_terminal_input(input: &str) -> String {
    if input.is_empty()
        || input.contains(DSH_BRACKETED_PASTE_START)
        || input.contains(DSH_BRACKETED_PASTE_END)
    {
        return input.to_string();
    }

    let (body, submit) = if input.ends_with('\r') {
        (&input[..input.len() - 1], "\r")
    } else {
        (input, "")
    };
    ...
    format!("{DSH_BRACKETED_PASTE_START}{normalized}{DSH_BRACKETED_PASTE_END}{submit}")
}

fn write_dsh_terminal_input_to_writer(
    writer: &Arc<Mutex<Box<dyn Write + Send>>>,
    input: &str,
) -> anyhow::Result<()> {
    let prepared = prepare_dsh_terminal_input(input);
    let is_encoded_submit = prepared.ends_with('\r')
        && prepared.starts_with(DSH_BRACKETED_PASTE_START)
        && prepared[..prepared.len() - 1].ends_with(DSH_BRACKETED_PASTE_END);
    if !is_encoded_submit {
        return write_terminal_input_to_writer(writer, &prepared);
    }

    // Keep the submit key in a separate PTY write. This gives dsh-tui one event
    // loop turn to commit the pasted draft before Enter reads the new value,
    // and also prevents a large ConPTY paste from being mistaken for several
    // independent line submissions.
    let paste = &prepared[..prepared.len() - 1];
    let mut writer = writer.lock().unwrap();
    writer.write_all(paste.as_bytes())?;
    writer.flush()?;
    thread::sleep(Duration::from_millis(DSH_BRACKETED_PASTE_SUBMIT_DELAY_MS));
...
/// DEC 2026 同步输出标记（BSU/ESU）。dsh-tui 用它包裹每一帧差分 patch；
...
pub fn open_external_cli(provider_kind: &str, cwd: &Path) -> anyhow::Result<()> {
    ...
    spawn_external_cmd(&command_args, cwd, &launch.child_env)
...
/// console, then mark the process so dsh-tui can use the conhost-safe renderer.
fn external_console_setup_prefix() -> &'static str {
    concat!(
        ...
        r#"set "SUPERHIGH_EXTERNAL_CONSOLE=1" & "#,
        r#"chcp 65001>nul & "#,
    )
}
...
fn spawn_external_cmd(
    ...
    let cwd_wide = cwd
        ...
        .chain(std::iter::once(0))
        .collect::<Vec<_>>();
    // Build an inherited environment snapshot and override only the values
    // owned by this launch.  Passing an environment block keeps DSH keys out
    // of the command line and out of the parent/user environment.
    let mut environment = BTreeMap::<String, (OsString, OsString)>::new();
    for (key, value) in std::env::vars_os() {
        environment.insert(key.to_string_lossy().to_ascii_lowercase(), (key, value));
    }
    for (key, value) in child_env {
        let key_os = OsString::from(key);
        environment.insert(key.to_ascii_lowercase(), (key_os, OsString::from(value)));
    }
...
fn resolve_launch(provider_kind: &str) -> anyhow::Result<CliLaunch> {
    ...
    let child_env = if provider_kind == "dsh" {
        crate::dsh_editor::prepare_runtime().map_err(|error| anyhow::anyhow!(error))?
    } else {
        Vec::new()
    };
...
fn configure_terminal_color_environment(builder: &mut CommandBuilder, provider_kind: &str) {
    builder.env("TERM", "xterm-256color");
    builder.env("COLORTERM", "truecolor");
    if provider_kind != "codex" && provider_kind != "dsh" {
        builder.env("FORCE_COLOR", "1");
        return;
    }

    // Codex and dsh-tui enable their rich TUI rendering from terminal capability variables.
...
fn find_provider_executable(provider: &CliProviderDefinition, dirs: &[PathBuf]) -> Option<PathBuf> {
    ...
    add_superhigh_cli_dirs(&mut bundled_dirs);
    let preferred_dirs = preferred_provider_dirs(provider.kind);
    find_provider_executable_in(provider, &bundled_dirs, &preferred_dirs, dirs)
}
...
fn find_provider_executable_in(
    ...
    // DSH is bundled with Super High. Prefer that copy so its CLI version stays
    // aligned with the profile dependencies installed by this repository.
    if provider.kind == "dsh" {
        if let Some(path) = find_executable(provider.command, bundled_dirs) {
            return Some(path);
        }
    }
...
fn args_for_provider(provider_kind: &str) -> Vec<&'static str> {
    match provider_kind {
        ...
        // dsh boots the fixed local dsh-tui profile as a terminal session.
        // 配置文件位于 $DSH_HOME/profiles/dsh-tui。
        "dsh" => vec!["--profile", "dsh-tui"],
        "local" => vec!["-NoLogo"],
        _ => Vec::new(),
    }
}
...
fn resume_args_for_provider(provider_kind: &str, native_session_id: &str) -> Option<Vec<String>> {
    ...
    let args = match provider_kind {
        ...
        "dsh" => vec!["--resume", native_session_id],
        _ => return None,
    };
...
/// dsh 现在是 Super High 的 npm 依赖，即使没有全局安装也能被检测到。
...
mod tests {
    use super::{
        ...
        external_console_setup_prefix, find_executable, find_provider_executable_in,
        ...
        prepare_dsh_terminal_input, project_service_session_matches, resume_args_for_provider,
        split_pending_at_frame_boundary, split_pending_for_size_emit, CliLaunch,
        CliProviderDefinition, TerminalManager, TerminalOutputBuffer, TerminalSession,
        DSH_BRACKETED_PASTE_END, DSH_BRACKETED_PASTE_START, SYNC_OUTPUT_SET,
    ...
    #[test]
    fn dsh_terminal_gets_xterm_frame_capabilities() {
        let mut builder = CommandBuilder::new("dsh");
        builder.env("NO_COLOR", "1");

        configure_terminal_color_environment(&mut builder, "dsh");

        assert_eq!(builder.get_env("TERM").unwrap(), "xterm-256color");
        assert_eq!(builder.get_env("COLORTERM").unwrap(), "truecolor");
        assert_eq!(builder.get_env("FORCE_COLOR").unwrap(), "3");
        assert_eq!(builder.get_env("CLICOLOR_FORCE").unwrap(), "1");
        assert_eq!(builder.get_env("TERM_PROGRAM").unwrap(), "SuperHigh");
        assert!(builder.get_env("NO_COLOR").is_none());
        #[cfg(windows)]
        assert_eq!(builder.get_env("WT_SESSION").unwrap(), "SuperHigh");
    }
    ...
    fn codex_external_handoff_quits_and_removes_the_internal_session() {
        ...
        let writer: Arc<Mutex<Box<dyn io::Write + Send>>> = Arc::new(Mutex::new(Box::new(
        ...
        manager
            .stop_codex_session_for_external_handoff(
                "codex-session",
    ...
    #[test]
    fn dsh_launches_the_dsh_tui_profile() {
        assert_eq!(args_for_provider("dsh"), vec!["--profile", "dsh-tui"]);
    }
    ...
    fn native_session_resume_uses_each_cli_real_argv_shape() {
        ...
        assert_eq!(
            resume_args_for_provider("dsh", "dsh-id"),
            Some(vec!["--resume".to_string(), "dsh-id".to_string()])
        );
        assert_eq!(
    ...
    #[test]
    fn dsh_multiline_input_is_one_bracketed_paste_before_submit() {
        let input = "第一段\n第二段\r\n第三段\r";

        assert_eq!(
            prepare_dsh_terminal_input(input),
            format!("{DSH_BRACKETED_PASTE_START}第一段\n第二段\n第三段{DSH_BRACKETED_PASTE_END}\r")
        );
    }

    #[test]
    fn dsh_single_line_and_existing_paste_input_are_not_rewritten() {
        assert_eq!(prepare_dsh_terminal_input("single line\r"), "single line\r");

        let already_encoded =
            format!("{DSH_BRACKETED_PASTE_START}第一段\n第二段{DSH_BRACKETED_PASTE_END}\r");
        assert_eq!(
            prepare_dsh_terminal_input(&already_encoded),
            already_encoded
        );
    }

    #[test]
    fn dsh_paste_without_submit_keeps_all_newlines_inside_markers() {
        let input = "第一段\r\n第二段\n";

        assert_eq!(
            prepare_dsh_terminal_input(input),
            format!("{DSH_BRACKETED_PASTE_START}第一段\n第二段\n{DSH_BRACKETED_PASTE_END}")
        );
    }
    ...
    fn external_console_clears_inherited_terminal_capability_vars() {
        ...
        assert!(prefix.contains("SUPERHIGH_EXTERNAL_CONSOLE=1"));
        assert!(prefix.contains("chcp 65001"));

        let command_line = external_cmd_command_line(
            "cmd.exe",
            r#"call "C:\Users\Test User\dsh.cmd" "--profile" "dsh-tui""#,
        );
        assert!(command_line.starts_with("\"cmd.exe\" /d /k "));
        assert!(command_line.contains(prefix));
        assert!(command_line.contains(r#"call "C:\Users\Test User\dsh.cmd" "--profile" "dsh-tui""#));
    }
    ...
    fn node_modules_bin_is_added_when_present() {
        ...
        fs::write(bin.join("dsh.cmd"), "").unwrap();
    ...
    #[test]
    fn dsh_prefers_bundled_cli_over_global_cli() {
        let bundled = tempfile::tempdir().unwrap();
        let global = tempfile::tempdir().unwrap();
        let file_name = if cfg!(windows) { "dsh.cmd" } else { "dsh" };
        let bundled_path = bundled.path().join(file_name);
        fs::write(&bundled_path, "").unwrap();
        fs::write(global.path().join(file_name), "").unwrap();
        let provider = CliProviderDefinition {
            kind: "dsh",
            name: "dsh-tui",
            command: "dsh",
        };

        let found = find_provider_executable_in(
            &provider,
