...
fn resolve_launch(provider_kind: &str) -> anyhow::Result<CliLaunch> {
    ...
    let child_env = if provider_kind == "dsh" {
        let config = crate::dsh_config::load_existing()
            .map_err(|error| anyhow::anyhow!(error))?
            .context("未找到现有 dsh config.toml；请检查本机固定 dsh-tui 配置")?;
        crate::dsh_config::runtime_environment(&config)
    } else {
        Vec::new()
    };
...
mod tests {
    ...
    #[test]
    fn dsh_launch_keeps_fixed_profile_arguments() {
