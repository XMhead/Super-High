    ...
    for service in config.services {
        ...
        if service.health_check.as_ref().is_some_and(tcp_health_check) {
            let result = report(
                ...
                None,
                Some("检测到外部实例正在监听端口，Super High 无法接管其控制台；请先停止该实例，再从本地终端启动".to_string()),
            )?;
            status_by_id.insert(service.id.clone(), result.status.clone());
            results.push(result);
            continue;
...
pub fn restart_project_service(
    app: &AppHandle,
    terminal: &TerminalManager,
    workspace_id: Option<String>,
    workspace: &Path,
    ...
    let config = crate::project_startup::load_project_startup_config(workspace)?
        .context("project startup config not found")?;
    if config.mode != ProjectStartupMode::Services {
        bail!("project startup config is not a service chain");
    }
    let service = match service_id.map(str::trim).filter(|value| !value.is_empty()) {
        Some(service_id) => config
            .services
            .iter()
            .find(|service| service.id == service_id)
            .cloned()
            .with_context(|| format!("unknown service: {service_id}"))?,
        None if config.services.len() == 1 => config.services[0].clone(),
        None => bail!("restart requires --service because this project has multiple services"),
    };

    if let Some(session_id) = terminal
        .find_live_project_service_session(&service.name, Path::new(&service.working_directory))
    {
    ...
    start_project_services(app, terminal, workspace_id, workspace, config)
}
