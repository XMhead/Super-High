...
use crate::models::{
    DirectoryListing, DragonCoreInfo, FileEntry, FileEntryType, FileOperationResult,
...
const DRAGON_CONFIG_FILE_NAME: &str = "main.md";
const DRAGON_OUTPUT_FILE_NAME: &str = "output.png";
const DRAGON_TEMPLATE: &str = "```dragon-pixel\nwidth: 16\nheight: 16\n\ncolors:\n[0]=rgba(0,0,0,0)\n[1]=&a\n[2]=#ffcc00ff\n\npixels:\nx:1,y:1[1]\nx:2,y:1[2]\nx:1~10,y:4[1]\nx:4,y:1~10[2]\n```\n";
...
pub fn ensure_dragon_core() -> anyhow::Result<DragonCoreInfo> {
    let app_root = resolve_super_high_root()?;
    ensure_dragon_core_at(&app_root)
}

pub fn ensure_dragon_core_at(app_root: &Path) -> anyhow::Result<DragonCoreInfo> {
    let metadata = fs::metadata(app_root)
        .with_context(|| format!("failed to inspect Super High root {}", app_root.display()))?;
    if !metadata.is_dir() {
        anyhow::bail!("Super High root is not a directory: {}", app_root.display());
    }

    let dragon_core_path = app_root.join("DragonCore");
    fs::create_dir_all(&dragon_core_path)
        .with_context(|| format!("failed to create {}", dragon_core_path.display()))?;

    let config_path = dragon_core_path.join(DRAGON_CONFIG_FILE_NAME);
    if !config_path.exists() {
        fs::write(&config_path, DRAGON_TEMPLATE)
            .with_context(|| format!("failed to create {}", config_path.display()))?;
    }

    let config_content = read_text_file(&config_path)?;
    let output_path = dragon_core_path.join(DRAGON_OUTPUT_FILE_NAME);

    Ok(DragonCoreInfo {
        app_root_path: normalize_path(app_root),
        dragon_core_path: normalize_path(&dragon_core_path),
...
pub fn write_dragon_output_data_url(
    data_url: &str,
    output_file_name: Option<&str>,
) -> anyhow::Result<String> {
    let app_root = resolve_super_high_root()?;
    write_dragon_output_data_url_at(&app_root, data_url, output_file_name)
}

pub fn write_dragon_output_data_url_at(
    app_root: &Path,
    data_url: &str,
    output_file_name: Option<&str>,
) -> anyhow::Result<String> {
    ...
    let encoded = data_url
        .strip_prefix(prefix)
        .ok_or_else(|| anyhow::anyhow!("export data must be a PNG data URL"))?;
    let bytes = STANDARD
        .decode(encoded)
        .context("failed to decode PNG data URL")?;
    if !bytes.starts_with(b"\x89PNG\r\n\x1a\n") {
        anyhow::bail!("decoded export data is not a PNG file");
    }

    let dragon_core_path = app_root.join("DragonCore");
    fs::create_dir_all(&dragon_core_path)
        .with_context(|| format!("failed to create {}", dragon_core_path.display()))?;
    let output_path = dragon_core_path.join(dragon_output_file_name(output_file_name));
...
fn dragon_output_file_name(raw: Option<&str>) -> String {
    let Some(raw_name) = raw.map(str::trim).filter(|value| !value.is_empty()) else {
        return DRAGON_OUTPUT_FILE_NAME.to_string();
    };
    let file_name = Path::new(raw_name)
        .file_name()
        .and_then(|value| value.to_str())
        .unwrap_or(DRAGON_OUTPUT_FILE_NAME);
    let stem = Path::new(file_name)
        .file_stem()
        .and_then(|value| value.to_str())
        .unwrap_or("output");
    let safe_stem = sanitize_windows_file_stem(stem);
    format!("{}.png", safe_stem)
}

fn sanitize_windows_file_stem(raw: &str) -> String {
    let value = raw
        .chars()
        .map(|character| {
...
mod tests {
    use super::{
        copy_paths_to_directory, ensure_dragon_core_at, find_super_high_root_from,
        move_paths_to_directory, normalize_path, read_office_file_base64, save_image_data_url, search_project_files,
        write_dragon_output_data_url_at, write_text_file, MAX_SEARCH_TEXT_FILE_BYTES,
    };
    ...
    #[test]
    fn ensure_dragon_core_creates_default_config_in_super_high_root() {
        let directory = tempfile::tempdir().unwrap();

        let info = ensure_dragon_core_at(directory.path()).unwrap();

        assert!(directory.path().join("DragonCore").is_dir());
        assert!(directory
            .path()
            .join("DragonCore")
            .join("main.md")
            .is_file());
        assert!(info.config_content.contains("width: 16"));
        assert_eq!(
            info.app_root_path,
            directory.path().to_string_lossy().replace('\\', "/")
        );
        assert!(info.config_path.ends_with("DragonCore/main.md"));
        assert!(info.output_path.ends_with("DragonCore/output.png"));
    }
    ...
    #[test]
    fn write_dragon_output_data_url_writes_fixed_png_path() {
        let directory = tempfile::tempdir().unwrap();
        let data_url = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/p9sAAAAASUVORK5CYII=";

        let output_path =
            write_dragon_output_data_url_at(directory.path(), data_url, None).unwrap();

        assert!(output_path.ends_with("DragonCore/output.png"));
        let bytes = std::fs::read(directory.path().join("DragonCore").join("output.png")).unwrap();
        assert!(bytes.starts_with(b"\x89PNG\r\n\x1a\n"));
    }

    #[test]
    fn write_dragon_output_data_url_uses_custom_png_name() {
        let directory = tempfile::tempdir().unwrap();
        let data_url = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/p9sAAAAASUVORK5CYII=";

        let output_path = write_dragon_output_data_url_at(
            directory.path(),
            data_url,
            Some(r"..\gold:sword.dragon"),
        )
        .unwrap();

        assert!(output_path.ends_with("DragonCore/gold-sword.png"));
        assert!(directory
            .path()
            .join("DragonCore")
            .join("gold-sword.png")
