...
fn default_model(config: &ChannelConfig) -> String {
    ...
    match config.provider {
        ...
        // dsh 默认走本机 dsh-tui 配置的 grok provider（OpenAI Responses 端点）。
        ChannelProviderKind::Dsh => "grok-4.6".to_string(),
...
/// （Claude → Anthropic Messages；Codex / dsh / grok → OpenAI Responses；Gemini → generateContent）。
fn effective_api_format(config: &ChannelConfig) -> ChannelApiFormat {
    config.api_format.unwrap_or_else(|| match config.provider {
        ...
        ChannelProviderKind::Codex | ChannelProviderKind::Dsh | ChannelProviderKind::Grok => {
...
mod tests {
    ...
    #[test]
    fn dsh_default_endpoint_appends_v1_responses() {
        let channel = config(ChannelProviderKind::Dsh, "https://api.aizzz.xyz/v1");
        assert_eq!(
            build_endpoint(&channel),
            "https://api.aizzz.xyz/v1/responses"
        );
    }
    ...
    fn default_format_follows_provider() {
        ...
        let dsh = config(ChannelProviderKind::Dsh, "https://api.aizzz.xyz/v1");
    ...
    fn default_model_falls_back_per_provider() {
        ...
        assert_eq!(default_model(&dsh), "grok-4.6");
    ...
    #[test]
    fn channel_config_deserializes_dsh_provider() {
        let json = r#"{
            "id": "c1",
            "name": "test",
            "provider": "dsh",
            "websiteUrl": "https://aizzz.xyz",
            "baseUrl": "https://api.aizzz.xyz/v1",
            "apiKey": "sk-test",
            "model": ""
        }"#;
        let channel: ChannelConfig = serde_json::from_str(json).unwrap();
        assert_eq!(channel.provider, ChannelProviderKind::Dsh);
