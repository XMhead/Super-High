...
发布时统一更新 `package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 和 `src-tauri/tauri.conf.json` 的应用版本，将 `v版本号` 标签指向公开 `main` 的当前提交，然后执行 `gh workflow run release.yml --ref main -f tag=v版本号`。工作流核对版本，构建签名安装包并生成 `latest.json`，所有必需产物验证通过后才正式发布。签名私钥须长期备份，不得提交到仓库。
...
| 内嵌终端 | 本地终端、AI CLI 会话（Codex / Claude） |
...
> 安装包通过 `main` 分支的 `release.yml` 手动调度，传入已推送的版本标签；同一分支下的编译缓存供后续版本复用。仅推标签不会启动构建。
...
实时开发预览：运行 `npm run mobile:dev`，电脑打开 `http://localhost:1421/`，在手机尺寸框中查看真实界面；保存 `src/mobile/` 修改后自动热更新。手机浏览器打开 `http://电脑局域网IP:1421/mobile/`，或在新版 App 填写 Host 地址 `http://电脑局域网IP:1421`，输入电脑 Host 的同一令牌，即可同步查看修改。保持开发服务运行即可，无需重新打包。

开发代理默认连接本机 `10320` 端口，其他 Host 地址通过启动前设置 `SUPERHIGH_MOBILE_HOST` 指定。开发预览入口不进入安装包。
