...
默认在维护者本机从公开源码构建 Windows 安装包与同版 APK，使用 `scripts/publish-local-release.mjs --source <干净公开工作树> --tag v<版本>` 上传并验证 Release；上传失败加 `--resume` 复用已验收产物。云端工作流仅在用户明确要求时使用。构建目录与在用程序隔离，不打断终端会话。
