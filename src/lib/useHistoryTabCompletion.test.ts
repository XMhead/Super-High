...
describe('useHistoryTabCompletion', () => {
  const history = [
    ...
    userMessage('n5', '图片路径：D:/示例项目/服务端/plugins/a.png', '2026-08-16T10:00:00.000Z'),
    userMessage('n6', '把 D:\\示例项目\\服务端\\plugins\\a.png 收录', '2026-08-15T10:00:00.000Z'),
  ...
  it('path fragment completion replaces only the path run and keeps surrounding text', () => {
    const { completion, setDraft, getDraft } = setup(history)
    setDraft('把 D:/示例项目/服 收录', 11)
    completion.refresh()
    expect(completion.popupOpen.value).toBe(true)
    expect(completion.candidates.value[0]).toBe('D:/示例项目/服务端/plugins/a.png')
    // 路径标签显示尾部
    expect(completion.candidateLabels.value[0]).toBe('服务端/plugins/a.png')
    expect(completion.confirm()).toBe(true)
    expect(getDraft()).toBe('把 D:/示例项目/服务端/plugins/a.png 收录')
  })
