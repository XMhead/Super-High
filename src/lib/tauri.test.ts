...
describe('tauri backend bridge', () => {
  ...
  it('uses the registered Tauri command for DragonCore GUI opening', async () => {
    mocks.invoke.mockResolvedValueOnce({
      playerName: 'NO_mc',
      guiName: 'd队伍/副本列表',
      filePath: 'D:/One/DragonCore/Gui/d队伍/副本列表.yml',
      command: 'core opengui NO_mc d队伍/副本列表',
      output: '',
    })

    const result = await backend.openDragonCoreGui(
      'D:/One',
      'D:/One/DragonCore/Gui/d队伍/副本列表.yml',
    )

    expect(mocks.invoke).toHaveBeenCalledWith('open_dragoncore_gui_command', {
      projectPath: 'D:/One',
      filePath: 'D:/One/DragonCore/Gui/d队伍/副本列表.yml',
    })
    expect(result.command).toBe('core opengui NO_mc d队伍/副本列表')
  })
  ...
  it('maps native conversation listing, reading, and resume to their Tauri commands', async () => {
    ...
    await backend.readWorkspaceCliConversation('D:/One', 'C:/Users/ExampleUser01/.codex/sessions/rollout.jsonl')
  ...
  it('checks whether a terminal path is an existing file through the registered command', async () => {
    ...
    await expect(backend.isFile('C:/Users/ExampleUser01/AppData/Local/Temp/analysis.json')).resolves.toBe(true)

    expect(mocks.invoke).toHaveBeenLastCalledWith('is_file_command', {
      path: 'C:/Users/ExampleUser01/AppData/Local/Temp/analysis.json',
    })
  })

  it('resolves terminal files and folders through the registered command', async () => {
    const target = { path: 'C:/Users/ExampleUser01/AppData/Local/Temp/frame-005.png', isFile: true }
