...
function response(): PluginListResponse {
  return {
    ...
    plugins: [{
      ...
      name: '示例项目目录名工具',
      version: '0.1.0',
      rootPath: 'C:/plugins/horizon-tools',
      main: 'main.js',
      mainPath: 'C:/plugins/horizon-tools/main.js',
      style: 'style.css',
      stylePath: 'C:/plugins/horizon-tools/style.css',
      source: 'global',
      permissions: ['all'],
      unsafe: true,
      status: 'matched',
...
describe('pluginRuntime', () => {
  ...
  it('activates matched plugins and disposes them on workspace mismatch', async () => {
    ...
    const runtime = createPluginRuntime({
      listPlugins,
      readPluginAsset: vi.fn(async (_id, path) => path === 'style.css' ? '.horizon { color: red }' : ''),
    ...
    expect(document.head.querySelector('style[data-superhigh-plugin="horizon-tools"]')?.textContent).toContain('color: red')

    await runtime.refresh(null, [])
    expect(dispose).toHaveBeenCalledOnce()
    expect(document.head.querySelector('style[data-superhigh-plugin="horizon-tools"]')).toBeNull()
  })
  ...
  it('registers commands and runs them through the runtime', async () => {
    ...
    const runtime = createPluginRuntime({
      ...
      loadModule: vi.fn(async () => ({
        default: (ctx: any) => ctx.ui.registerCommand({ id: 'horizon.hello', title: 'Hello', run }),
      })),
      getUnsafe: () => ({ window, document }),
      getWorkspaceApi: () => ({ rootPath: 'D:/Project' }),
      getEditorApi: () => ({}),
    ...
    await runtime.refresh(workspace(), [])
    expect(runtime.getCommands()).toEqual([{ id: 'horizon.hello', pluginId: 'horizon-tools', title: 'Hello', tooltip: undefined }])
    await runtime.runCommand('horizon.hello')
  ...
  it('marks plugin failed when activate throws and keeps other plugins loadable', async () => {
    ...
    const runtime = createPluginRuntime({
      ...
      readPluginAsset: vi.fn(async () => ''),
      execPluginShell: vi.fn(),
      loadModule: vi.fn(async (plugin) => plugin.id === 'horizon-tools'
    ...
    await runtime.refresh(workspace(), [])
    expect(runtime.getStatuses().find((item) => item.id === 'horizon-tools')?.status).toBe('failed')
  ...
  it('retries a previously failed matched plugin on a later refresh', async () => {
    ...
    expect(runtime.getStatuses().find((item) => item.id === 'horizon-tools')?.status).toBe('loaded')
  })
  ...
  it('does not activate disabled matched plugins and disposes active plugins when they become disabled', async () => {
    ...
    await runtime.refresh(workspace(), ['horizon-tools'])
  ...
  it('invalidates in-flight refreshes when disposeAll runs before listPlugins resolves', async () => {
    ...
    expect(activate).not.toHaveBeenCalled()
    expect(runtime.getCommands()).toEqual([])
    expect(runtime.getStatuses().find((item) => item.id === 'horizon-tools')?.status).not.toBe('loaded')
  ...
  it('keeps newer commands when stale activation cleanup finishes for the same plugin id', async () => {
    ...
    expect(runtime.getCommands()).toEqual([{
      id: 'horizon-tools.new-command',
      pluginId: 'horizon-tools',
      title: 'New command',
      tooltip: undefined,
    }])
  ...
  it('continues runtime-owned cleanup when plugin disposer throws', async () => {
    ...
    const runtime = createPluginRuntime({
      ...
      loadModule: vi.fn(async () => ({
        default: (ctx: any) => {
          ctx.ui.registerCommand({ id: 'horizon.hello', title: 'Hello', run: vi.fn() })
          return () => {
    ...
    expect(runtime.getCommands()).toEqual([])
    expect(document.head.querySelector('style[data-superhigh-plugin="horizon-tools"]')).toBeNull()
    expect(runtime.getStatuses().find((item) => item.id === 'horizon-tools')?.status).toBe('matched')
  ...
  it('uses the host app version and targets requested plugin ids for reload and disable', async () => {
    ...
    const firstApp = appContexts.get('horizon-tools')
    ...
    expect(disposers.get('horizon-tools')).not.toHaveBeenCalled()
    expect(runtime.getStatuses().find((item) => item.id === 'horizon-tools')?.status).toBe('loaded')
  ...
  it('does not reload disabled plugins and notifies immediately after disablePlugin disposes them', async () => {
    ...
    await runtime.refresh(workspace(), [])
    expect(activations.get('horizon-tools')).toBe(1)
    expect(activations.get('second-tool')).toBe(1)
    ...
    await firstApp.disablePlugin('horizon-tools')
    expect(disposers.get('horizon-tools')).toHaveBeenCalledOnce()
    expect(runtime.getStatuses().find((item) => item.id === 'horizon-tools')?.status).toBe('matched')
  ...
  it('keeps same-session disabled plugins gated from reload until refresh clears the disabled list', async () => {
    ...
    const app = appContexts.get('horizon-tools')
    await app.disablePlugin('horizon-tools')
    expect(disposers.get('horizon-tools')).toHaveBeenCalledOnce()
    expect(runtime.getStatuses().find((item) => item.id === 'horizon-tools')?.status).toBe('matched')
    expect(onChanged).toHaveBeenCalledTimes(2)

    await app.reloadPlugin('horizon-tools')
    expect(activations.get('horizon-tools')).toBe(1)
    expect(runtime.getStatuses().find((item) => item.id === 'horizon-tools')?.status).toBe('matched')
    expect(onChanged).toHaveBeenCalledTimes(3)

    await runtime.refresh(workspace(), [])
    expect(activations.get('horizon-tools')).toBe(2)
    expect(runtime.getStatuses().find((item) => item.id === 'horizon-tools')?.status).toBe('loaded')
    expect(onChanged).toHaveBeenCalledTimes(4)
  })
  ...
  it('marks the plugin failed when the default loader hits a circular local import', async () => {
    ...
    const status = runtime.getStatuses().find((item) => item.id === 'horizon-tools')
