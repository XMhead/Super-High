...
export const backend = {
  ...
  listDshProviders() {
    return command<DshProviderInfo[]>('list_dsh_providers_command')
  },
  listDshProfiles() {
    return command<DshProfileInfo[]>('list_dsh_profiles_command')
  },
  getDshDefaultModel() {
    return command<DshDefaultModel | null>('get_dsh_default_model_command')
  },
  getDshProviderApiKey(providerId: string) {
    return command<string | null>('get_dsh_provider_api_key_command', { providerId })
  },
  addDshProvider(provider: DshProviderInput) {
    return command<DshProviderInfo[]>('add_dsh_provider_command', { provider })
  },
  updateDshProvider(originalId: string, provider: DshProviderInput) {
    return command<DshProviderInfo[]>('update_dsh_provider_command', { originalId, provider })
  },
  deleteDshProvider(providerId: string) {
    return command<DshProviderInfo[]>('delete_dsh_provider_command', { providerId })
  },
  setDshDefaultModel(providerId: string, modelId: string) {
    return command<void>('set_dsh_default_model_command', { providerId, modelId })
  },
  deleteDshProfile(profileId: string) {
    return command<DshProfileInfo[]>('delete_dsh_profile_command', { profileId })
  },
  listCcProviders() {
  ...
  ensureDragonCore() {
    return command<DragonCoreInfo>('ensure_dragon_core_command')
  },
  exportDragonImage(dataUrl: string, outputFileName?: string) {
    return command<string>('export_dragon_image_command', { dataUrl, outputFileName })
  },
