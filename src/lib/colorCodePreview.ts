...
const COLOR_TOKEN_PATTERN = /rgba\(\s*[^)]*?\s*\)|[&§]#(?:[0-9a-fA-F]{6}|[0-9a-fA-F]{8})|[&§]x(?:[&§][0-9a-fA-F]){6}|[&§][0-9a-fA-FrR]/gi
...
export function buildColorCodePreviewRanges(text: string): ColorCodePreviewRange[] {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    for (const match of line.matchAll(COLOR_TOKEN_PATTERN)) {
      ...
      if (token.toLowerCase().startsWith('rgba(')) {
      ...
      const nextColor = colorForMinecraftToken(token)
      if (activeColor && tokenStart > segmentStart) {
        ranges.push(toRange(lineIndex, segmentStart, tokenStart, activeColor))
      }
...
function colorForMinecraftToken(token: string): string | null {
  const lower = token.toLowerCase()
  if (lower.endsWith('r') && lower.length === 2) return null

  if (lower.length === 2) {
    return LEGACY_COLORS[lower[1]] ?? null
  }

  if (lower[1] === '#') {
    const hex = token.slice(2).replace(/[^0-9a-f]/gi, '').toUpperCase()
    return hex.length === 6 || hex.length === 8 ? `#${hex}` : null
  }

  if (lower[1] === 'x') {
    const digits = token.slice(2).replace(/[^0-9a-f]/gi, '').toUpperCase()
