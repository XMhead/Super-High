...
describe('ansi status panel', () => {
  it('renders synchronized repaint frames in a TTY', async () => {
    ...
    const panel = createAnsiStatusPanel({ title: 'DragonCore 生图', stream, width: 44 })
    ...
    expect(output).toContain('DragonCore 生图')
  ...
  it('keeps non-interactive output clean', async () => {
    ...
    const panel = createAnsiStatusPanel({ title: 'DragonCore 生图', stream })
