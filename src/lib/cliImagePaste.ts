...
export function buildCliMessageWithImages(
  text: string,
  imagePaths: string[],
  providerKind?: string,
): string {
  ...
  if (providerKind === 'dsh') {
    const imageBlocks = uniquePaths
      .map((path) => `[[superhigh-image path="${path.replace(/"/g, '&quot;')}"]]`)
      .join('\n')
    return trimmed ? `${imageBlocks}\n${trimmed}` : imageBlocks
  }
  const imageBlock = uniquePaths
    ...
    .join('\n')
  const instruction = '请读取并查看上述本地图片文件后再回答；若无法访问该路径，请明确说明。'
