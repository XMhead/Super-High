...
describe('path helpers', () => {
  ...
  it('removes a leading slash before windows drive paths', () => {
    expect(buildBreadcrumbs('/D:/ExampleProject/src/App.vue')[0]).toEqual({ label: 'D:', path: 'D:/' })
  })

  it('builds compact breadcrumbs relative to the workspace root', () => {
    expect(buildWorkspaceBreadcrumbs(
      'D:/Work/server/plugins/SampleTools/buffs.yml',
      'D:/Work/server/plugins',
    )).toEqual([
      {
        label: 'SampleTools',
        path: 'D:/Work/server/plugins/SampleTools',
      },
      {
        label: 'buffs.yml',
        path: 'D:/Work/server/plugins/SampleTools/buffs.yml',
      },
    ])
  })
