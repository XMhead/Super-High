...
export class MobileHostApi {
  ...
  async terminalBuffer(sessionId: string, after: number, options: { waitMs?: number; cols?: number; rows?: number; signal?: AbortSignal } = {}) {
    const params = new URLSearchParams({ sessionId, after: String(after) })
    for (const key of ['waitMs', 'cols', 'rows'] as const) {
      if (options[key] !== undefined) params.set(key, String(options[key]))
    }
    const response = await this.request(`/api/mobile/terminals/buffer?${params}`, undefined, true, options.signal)
    return response.json() as Promise<MobileTerminalBuffer>
  }
  ...
  private async request(path: string, body?: unknown, authenticated = true, signal?: AbortSignal): Promise<Response> {
    const controller = new AbortController()
    const abort = () => controller.abort()
    signal?.addEventListener('abort', abort, { once: true })
    if (signal?.aborted) controller.abort()
    const timeoutMs = path === '/api/mobile/update/download' ? 31 * 60 * 1000
      : path === '/api/mobile/attachments' ? 120000
        : path === '/api/mobile/status' || path.startsWith('/api/mobile/terminals/buffer?') ? 3000 : 35000
