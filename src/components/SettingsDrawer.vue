...
type SettingsSection = 'appearance' | 'mobile' | 'cli' | 'plugins' | 'storage' | 'updates' | 'tutorial'
...
const settingSections: Array<{ id: SettingsSection; label: string; icon: Component }> = [
  ...
  { id: 'tutorial', label: '新手教程', icon: BookOpen },
]
  ...
  <section v-if="store.settingsOpen" class="settings-workspace-panel" aria-label="设置">
    ...
    <div class="settings-page-body">
      ...
      <main class="settings-content">
        <section v-if="activeSection === 'tutorial'" class="settings-content-section">
          <div class="settings-section-heading">
            <div>
              <div class="section-title">桌面版新手教程</div>
              <p>了解项目、对话、文件和终端的日常用法，以及渠道排错和手机端接续。</p>
            </div>
          </div>
          <div class="settings-block">
            <p class="settings-note">完成或关闭后不再自动播放。重放从第一步开始，左右方向键切换，ESC 随时关闭。</p>
            <button class="primary-button small" @click="replayTutorial">
              <BookOpen :size="14" />
              <span>重放新手教程</span>
            </button>
          </div>
