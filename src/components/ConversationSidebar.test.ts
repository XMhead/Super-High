...
describe('ConversationSidebar keyboard navigation', () => {
  ...
  it('shows a compact resume action only for exited conversations', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const store = useWorkspaceStore()
    store.terminalSessions = [
      { id: 'session-live', title: 'Codex', providerKind: 'codex', cwd: 'D:/Project' },
      { id: 'session-ended', title: 'Codex', providerKind: 'codex', cwd: 'D:/Project', restoredFromDisk: true },
    ]
    store.terminalExitCodes['session-ended'] = 0
    const resume = vi.spyOn(store, 'resumeExitedCliSession').mockResolvedValue(null)
    const { app, root } = mountSidebar(pinia)
    await nextTick()

    const buttons = root.querySelectorAll<HTMLButtonElement>('.conversation-icon-button.resume')
    expect(buttons).toHaveLength(1)
    expect(buttons[0].title).toBe('恢复对话')
    buttons[0].click()
    await nextTick()
    expect(resume).toHaveBeenCalledWith('session-ended')
