...
describe('ActivityBar Minecraft entry', () => {
  let pinia: Pinia
  ...
  it('hides the Minecraft entry without a config', async () => {
    const { app, root } = mountActivityBar(pinia)
    await nextTick()

    expect(root.querySelector('[data-testid="minecraft-activity"]')).toBeNull()

    app.unmount()
    root.remove()
  })

  it('shows the Minecraft entry when the store enables it', async () => {
    const store = useWorkspaceStore()
    store.minecraftClientConfigError = 'Launcher path is invalid'
    const { app, root } = mountActivityBar(pinia)
    await nextTick()

    expect(root.querySelector('[data-testid="minecraft-activity"]')).not.toBeNull()
