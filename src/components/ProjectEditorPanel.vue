...
import type { FileEntry, ItemLibrarySource } from '@/types'
...
type EditorItemLibraryRequest = {
  type?: unknown
  requestId?: unknown
  source?: unknown
  query?: unknown
  expandedQuery?: unknown
  page?: unknown
  pageSize?: unknown
}
type EditorMonsterLibraryRequest = {
  type?: unknown
  requestId?: unknown
  query?: unknown
  page?: unknown
...
const mainItemLibrarySource = ref<ItemLibrarySource>('ni')
...
function mountProjectDocument() {
  ...
  const editorWindow = {
    superhigh: {
      itemLibrary: {
        mainSource: mainItemLibrarySource.value,
        getKeys: async (source?: unknown) => {
          if (!workspaceRoot.value) throw new Error('未打开工作区。')
          const selectedSource = source === 'mm' || source === 'ni' ? source : mainItemLibrarySource.value
          return (await backend.itemLibraryKeys(workspaceRoot.value, selectedSource)).itemKeys
        },
      },
    },
    parent: {
  ...
  new Function('window', buildProjectEditorBridgeScript(mainItemLibrarySource.value))(editorWindow)
...
async function loadProjectEditor() {
  ...
  loading.value = true
  try {
    mainItemLibrarySource.value = (await loadProjectItemLibraryMainSource(rootPath)) ?? 'ni'
...
async function handleEditorItemLibraryRequest(data: EditorItemLibraryRequest) {
  if (data.type !== 'superhigh-editor:item-library') return false
  if (typeof data.requestId !== 'string' || !workspaceRoot.value) return true

  try {
    const source = data.source === undefined ? mainItemLibrarySource.value : data.source
    ...
    const expandedQuery = typeof data.expandedQuery === 'string' ? data.expandedQuery : undefined
    ...
    postToProjectEditor({ type: 'superhigh-editor:item-library-result', requestId: data.requestId, result })
  } catch (requestError) {
    postToProjectEditor({ type: 'superhigh-editor:item-library-error', requestId: data.requestId, error: formatError(requestError) })
  }
  return true
}

async function handleEditorMonsterLibraryRequest(data: EditorMonsterLibraryRequest) {
  ...
  try {
    ...
    postToProjectEditor({ type: 'superhigh-editor:monster-library-result', requestId: data.requestId, result })
  } catch (requestError) {
    postToProjectEditor({ type: 'superhigh-editor:monster-library-error', requestId: data.requestId, error: formatError(requestError) })
  }
  return true
}

async function handleEditorMinecraftRequest(data: EditorMinecraftRequest) {
  if (data.type !== 'superhigh-editor:minecraft') return false
  if (typeof data.requestId !== 'string' || typeof data.operation !== 'string' || !workspaceRoot.value) return true

  try {
    let result: unknown
    if (data.operation === 'players') {
      result = await backend.onlinePlayers(workspaceRoot.value)
    ...
    } else if (data.operation === 'open-dragoncore-gui') {
      result = await backend.openDragonCoreGui(workspaceRoot.value, workspaceFilePath(data.path))
    } else {
      throw new Error('不支持的项目编辑器 Minecraft 操作。')
    }
    postToProjectEditor({ type: 'superhigh-editor:minecraft-result', requestId: data.requestId, result })
  } catch (requestError) {
    postToProjectEditor({ type: 'superhigh-editor:minecraft-error', requestId: data.requestId, error: formatError(requestError) })
  }
  return true
}

async function handleEditorDragonCoreRequest(data: EditorDragonCoreRequest) {
  ...
  try {
    ...
    postToProjectEditor({ type: 'superhigh-editor:dragoncore-result', requestId: data.requestId, result })
  } catch (requestError) {
    postToProjectEditor({ type: 'superhigh-editor:dragoncore-error', requestId: data.requestId, error: formatError(requestError) })
  }
  return true
}
...
async function onMessage(event: MessageEvent) {
  const data = event.data as EditorFilesystemRequest & EditorItemLibraryRequest & EditorMonsterLibraryRequest & EditorCodePreviewRequest & EditorUiRequest & EditorMinecraftRequest & EditorDragonCoreRequest & EditorScriptToolRequest & { command?: unknown }
  ...
  if (await handleEditorItemLibraryRequest(data)) return
  if (await handleEditorMonsterLibraryRequest(data)) return
  if (await handleEditorMinecraftRequest(data)) return
  if (await handleEditorDragonCoreRequest(data)) return
  ...
  try {
    ...
    postToProjectEditor({ type: 'superhigh-editor:rcon-result', requestId: data.requestId, result })
  } catch (commandError) {
    postToProjectEditor({ type: 'superhigh-editor:rcon-error', requestId: data.requestId, error: formatError(commandError) })
  }
}
