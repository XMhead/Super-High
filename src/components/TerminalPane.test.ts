...
describe('TerminalPane', () => {
  ...
  beforeEach(() => {
    ...
    mocks.savePastedImage.mockResolvedValue('D:/ExampleProject/.superhigh/pasted-images/paste-test.png')
  ...
  it('shows configured project startup control in local terminal mode', async () => {
    ...
    expect(root.textContent).toContain('启动服务')
  ...
  it('starts the configured project server from the startup control', async () => {
    ...
    const { app, root } = mountPane(pinia, { mode: 'local' })
    await nextTick()
    ;(Array.from(root.querySelectorAll('button')).find((button) => button.textContent?.includes('启动服务')) as HTMLButtonElement).click()
    await flushPromises()
  ...
  it('keeps the local terminal toolbar dedicated to the service chain', async () => {
    ...
    expect(root.textContent).not.toContain('工作区日志(GBK)')
  ...
  it('closes the live project startup session from the startup control', async () => {
    ...
    const { app, root } = mountPane(pinia, { mode: 'local' })
    await nextTick()
    ;(Array.from(root.querySelectorAll('button')).find((button) => button.textContent?.includes('关闭服务')) as HTMLButtonElement).click()
    await flushPromises()
  ...
  it('starts a new project server when the previous startup session has exited', async () => {
    ...
    expect(root.textContent).toContain('启动服务')
    expect(root.textContent).not.toContain('关闭服务')
    ;(Array.from(root.querySelectorAll('button')).find((button) => button.textContent?.includes('启动服务')) as HTMLButtonElement).click()
    await flushPromises()
  ...
  it('does not apply a late project startup config after switching workspaces', async () => {
    ...
    expect(store.projectStartupConfig?.name).toBe('B 工作区')
  })
  ...
  it('does not apply an older startup config after switching from A to B and back to A', async () => {
    ...
    expect(store.projectStartupConfig?.name).toBe('新版 A 工作区')
  ...
  it('does not show a startup control when the project has no startup config', async () => {
    ...
    expect(Array.from(root.querySelectorAll('button')).some((button) => button.textContent?.trim() === '启动服务')).toBe(false)
  ...
  it('opens a CLI in external CMD on right click and keeps left click embedded', async () => {
    ...
    expect(mocks.openExternalCli).toHaveBeenCalledWith('codex', 'D:/ExampleProject')
  ...
  it('shows only the project start action in the local terminal toolbar', async () => {
    const store = useWorkspaceStore()
    store.projectTerminalActions = [
      { id: 'server-start', label: '启动服务', kind: 'start-project', startAfterCommand: false, restartDelayMs: 5000 },
      { id: 'server-restart', label: '重启服务', kind: 'restart-project', startAfterCommand: true, restartDelayMs: 5000 },
    ]
    store.projectServiceRun = {
      projectPath: 'D:/One',
      runId: 'run-1',
  ...
  it('opens a verified CLI terminal path in the normal code preview', async () => {
    const store = useWorkspaceStore()
    store.terminalSessions = [
      { id: 'session-main', title: 'Claude', providerKind: 'claude', cwd: 'D:/ExampleProject' },
    ]
    store.activeTerminalSessionId = 'session-main'
    store.activeCliSessionId = 'session-main'
    store.settings.activePreviewMode = 'local_terminal'
    const openFile = vi.spyOn(store, 'openFile').mockResolvedValue(true)
    const path = 'C:\\Users\\Example\\AppData\\Local\\Temp\\analysis.json'
  ...
  it.each(['jar', 'JAR', 'MP4', 'mp3'])('routes a verified .%s CLI link through the shared file opener', async (extension) => {
    ...
    const path = `D:/示例项目/示例项目目录工作区/plugins/反编译/workspaces/GermExtract/build/libs/GermExtract-0.7.0.${extension}`
    mocks.isFile.mockResolvedValue(true)
    mocks.resolveTerminalPath.mockResolvedValue({ path, isFile: true })
  ...
  it('keeps DSH frame output out of the Codex scrollback renderer', async () => {
    const store = useWorkspaceStore()
    store.terminalSessions = [
      { id: 'session-dsh', title: 'DeepSeek Harness', providerKind: 'dsh', cwd: 'D:/ExampleProject' },
    ]
    store.activeTerminalSessionId = 'session-dsh'
    store.activeCliSessionId = 'session-dsh'
    const { app, root } = mountPane(pinia, {
      mode: 'cli',
      sessionIds: ['session-dsh'],
      activeSessionId: 'session-dsh',
    })
    await nextTick()
    await flushPromises()
    await nextTick()

    const terminalHost = root.querySelector('.terminal-host')
    expect(terminalHost?.classList.contains('framed-terminal-host')).toBe(true)
    expect(terminalHost?.classList.contains('codex-terminal-host')).toBe(false)
    expect(mocks.terminalInstances[0]?.options).toMatchObject({
      scrollback: 10_000,
      scrollSensitivity: 1.25,
    })
    expect(mocks.codexRendererInstances).toHaveLength(0)
    ...
    outputListener?.({
      ...
      chunk: frame,
      startByte: 0,
      endByte: frame.length,
    })
    await new Promise((resolve) => window.setTimeout(resolve, 20))

    const writes = mocks.terminalInstances[0]?.write.mock.calls.map(([data]) => String(data)) ?? []
  ...
  it('enhances the CLI draft in its session cwd and can undo the replacement', async () => {
    ...
    expect(mocks.enhancePromptWithCodex).toHaveBeenCalledWith('D:/ExampleProject', '  修复终端按钮\n')
  ...
  it.skip('shows a preview popup and completes the newest unique user input on Tab', async () => {
    ...
    store.terminalSessions = [
      { id: 'session-main', title: 'Codex', providerKind: 'codex', cwd: 'D:/ExampleProject' },
      { id: 'session-claude', title: 'Claude', providerKind: 'claude', cwd: 'D:/ExampleProject' },
      { id: 'session-dsh', title: 'DSH', providerKind: 'dsh', cwd: 'D:/ExampleProject' },
    ]
    store.activeTerminalSessionId = 'session-main'
    store.activeCliSessionId = 'session-main'
  ...
  it('selects a project or global Skill with arrow keys and Enter, then sends a compatible instruction', async () => {
    ...
    mocks.isTauri.mockReturnValue(true)
    mocks.listSkills.mockResolvedValue([
      { name: 'agent-learning', description: '沉淀经验', scope: 'global', path: 'C:/skills/agent-learning/SKILL.md' },
      { name: 'agents-updater', description: '维护 Agent 资源', scope: 'project', path: 'D:/ExampleProject/.codex/skills/agents-updater/SKILL.md' },
    ])
    ...
    expect(mocks.listSkills).toHaveBeenCalledWith('D:/ExampleProject')
    const textarea = root.querySelector('.cli-message-input') as HTMLTextAreaElement
    textarea.value = '/'
    ...
    expect(items[1]?.textContent).toContain('/agent-learning')
  ...
  it('opens workspace prompts first, searches full text and pastes without sending', async () => {
    const store = useWorkspaceStore()
    store.terminalSessions = [{ id: 'session-main', title: 'Codex', providerKind: 'codex', cwd: 'D:/ExampleProject' }]
    store.activeTerminalSessionId = 'session-main'
    store.activeCliSessionId = 'session-main'
    const sendMessage = vi.spyOn(store, 'sendTerminalConversationMessage').mockResolvedValue(true)
    ...
    await flushPromises()
    await nextTick()
    expect(mocks.listWorkspacePromptHistory).toHaveBeenCalledWith('D:/ExampleProject')
  ...
  it('shows cached prompts immediately while checking for new terminal input', async () => {
    ...
    await promptHistoryCache.refresh('D:/ExampleProject')
  ...
  it.skip('completes the current word fragment when the draft start does not match', async () => {
    ...
    await nextTick()
    // 只替换词片段，保留「帮我用 」前缀
    expect(textarea.value).toBe('帮我用 typescript')
    expect(root.querySelector('.history-completion-popup')).toBeNull()
  ...
  it('does not paste DSH terminal clipboard text into the conversation draft on right click', async () => {
    const store = useWorkspaceStore()
    store.terminalSessions = [
      { id: 'session-dsh', title: 'DeepSeek Harness', providerKind: 'dsh', cwd: 'D:/ExampleProject' },
    ]
    store.activeTerminalSessionId = 'session-dsh'
    store.activeCliSessionId = 'session-dsh'
    const readText = vi.fn().mockResolvedValue('请检查当前 DSH 配置。')
    ...
    const contextMenu = new MouseEvent('contextmenu', { bubbles: true, cancelable: true })
    root.querySelector('.terminal-host')?.dispatchEvent(contextMenu)
    await flushPromises()
    await nextTick()

    expect(contextMenu.defaultPrevented).toBe(false)
    expect(readText).not.toHaveBeenCalled()
    expect((root.querySelector('.cli-message-input') as HTMLTextAreaElement).value).toBe('')
  ...
  it('recalls DSH user messages with input arrow keys and restores the draft', async () => {
    const store = useWorkspaceStore()
    store.terminalSessions = [
      { id: 'session-dsh', title: 'DeepSeek Harness', providerKind: 'dsh', cwd: 'D:/ExampleProject' },
    ]
    store.activeTerminalSessionId = 'session-dsh'
    store.activeCliSessionId = 'session-dsh'
    store.terminalDrafts = { 'session-dsh': '未发送草稿' }
    store.terminalConversation = {
      'session-dsh': [
        { id: 'old', sessionId: 'session-dsh', role: 'user', content: '第一条消息', timestamp: '2026-08-20T10:00:00.000Z' },
        { id: 'reply', sessionId: 'session-dsh', role: 'system', content: '回复', timestamp: '2026-08-20T10:01:00.000Z' },
        { id: 'new', sessionId: 'session-dsh', role: 'user', content: '第二条消息', timestamp: '2026-08-20T10:02:00.000Z' },
      ],
    }
    ...
    const textarea = root.querySelector('.cli-message-input') as HTMLTextAreaElement
    const up = () => textarea.dispatchEvent(new KeyboardEvent('keydown', {
      key: 'ArrowUp', bubbles: true, cancelable: true,
    }))
    const down = () => textarea.dispatchEvent(new KeyboardEvent('keydown', {
  ...
  it('pastes five clipboard images into CLI draft attachments and sends all paths', async () => {
    ...
    store.terminalSessions[0].providerKind = 'dsh'
    ...
    mocks.savePastedImage.mockImplementation(async (_projectPath, _dataUrl, preferredName) => (
      `D:/ExampleProject/.superhigh/pasted-images/${preferredName}`
    ))
    ...
    expect(store.sendTerminalConversationMessage).toHaveBeenCalledWith(
      'session-main',
      '[[superhigh-image path="D:/ExampleProject/.superhigh/pasted-images/image-1.png"]]\n'
        + '[[superhigh-image path="D:/ExampleProject/.superhigh/pasted-images/image-2.png"]]\n'
        + '[[superhigh-image path="D:/ExampleProject/.superhigh/pasted-images/image-3.png"]]\n'
        + '[[superhigh-image path="D:/ExampleProject/.superhigh/pasted-images/image-4.png"]]\n'
        + '[[superhigh-image path="D:/ExampleProject/.superhigh/pasted-images/image-5.png"]]',
    )
