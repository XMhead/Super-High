...
const mocks = vi.hoisted(() => ({
  ...
  normalizePromptSpacing: vi.fn(),
  promptSpacingStatus: vi.fn(),
  installPromptSpacing: vi.fn(),
...
vi.mock('@/lib/tauri', () => ({
  backend: {
    ...
    normalizePromptSpacing: mocks.normalizePromptSpacing,
    promptSpacingStatus: mocks.promptSpacingStatus,
    installPromptSpacing: mocks.installPromptSpacing,
...
describe('TerminalPane', () => {
  ...
  beforeEach(() => {
    ...
    mocks.normalizePromptSpacing.mockReset()
    mocks.promptSpacingStatus.mockReset().mockResolvedValue({ ready: true })
    mocks.installPromptSpacing.mockReset().mockResolvedValue(undefined)
  ...
  it('forwards Alt arrows with the original modifier to the active CLI session exactly once', async () => {
    const store = useWorkspaceStore()
    ...
    await nextTick()
    await flushPromises()
    const handler = mocks.terminalInstances[0]!.attachCustomKeyEventHandler.mock.calls[0]![0]
    mocks.writeTerminalInput.mockClear()
    for (const [key, suffix] of [['ArrowUp', 'A'], ['ArrowDown', 'B'], ['ArrowRight', 'C'], ['ArrowLeft', 'D']]) {
      const event = new KeyboardEvent('keydown', { key, altKey: true, cancelable: true })
      expect(handler(event)).toBe(false)
      expect(event.defaultPrevented).toBe(true)
      expect(mocks.writeTerminalInput).toHaveBeenLastCalledWith('session-main', `\x1b[1;3${suffix}`)
      expect(handler(new KeyboardEvent('keyup', { key, altKey: true }))).toBe(true)
    }
    expect(mocks.writeTerminalInput).toHaveBeenCalledTimes(4)
    for (const modifiers of [{}, { ctrlKey: true }, { altKey: true, shiftKey: true }, { altKey: true, ctrlKey: true }]) {
      expect(handler(new KeyboardEvent('keydown', { key: 'ArrowUp', ...modifiers }))).toBe(true)
    }
    expect(mocks.writeTerminalInput).toHaveBeenCalledTimes(4)
    app.unmount()
    root.remove()
  ...
  it('follows new output at bottom but lets the user scroll up during writes', async () => {
    const store = useWorkspaceStore()
    store.terminalSessions = [{ id: 'session-main', title: 'Codex', providerKind: 'codex', cwd: 'D:/Demo Files' }]
    store.activeCliSessionId = 'session-main'
    mocks.getTerminalBuffer.mockResolvedValue({ buffer: 'ready', startByte: 0, endByte: 5 })
    const { app, root } = mountPane(pinia, { mode: 'cli', sessionIds: ['session-main'], activeSessionId: 'session-main', compactTeam: true })
    await nextTick()
    await flushPromises()
    await new Promise((resolve) => window.setTimeout(resolve, 20))
    const terminal = mocks.terminalInstances[0]!
    const output = mocks.onTerminalOutput.mock.calls.at(-1)![0]
    terminal.scrollToBottom.mockClear()
    terminal.buffer.active.baseY = 100
    terminal.buffer.active.viewportY = 50
    output({ sessionId: 'session-main', chunk: 'a', startByte: 5, endByte: 6 })
    await flushPromises()
    await new Promise((resolve) => window.setTimeout(resolve, 20))
    expect(terminal.scrollToBottom).not.toHaveBeenCalled()
    terminal.buffer.active.viewportY = 100
    output({ sessionId: 'session-main', chunk: 'b', startByte: 6, endByte: 7 })
    await flushPromises()
    await new Promise((resolve) => window.setTimeout(resolve, 20))
    expect(terminal.scrollToBottom).toHaveBeenCalledOnce()
    terminal.scrollToBottom.mockClear()
    let finishWrite: (() => void) | undefined
    terminal.write.mockImplementation((_data, callback) => { finishWrite = callback })
    output({ sessionId: 'session-main', chunk: 'c', startByte: 7, endByte: 8 })
    await flushPromises()
    await new Promise((resolve) => window.setTimeout(resolve, 20))
    root.querySelector('.xterm')!.dispatchEvent(new WheelEvent('wheel', { deltaY: -100 }))
  ...
  it('normalizes spacing without sending and restores the exact original with undo', async () => {
    mocks.isTauri.mockReturnValue(true)
    mocks.normalizePromptSpacing.mockResolvedValue('我是奥特曼\n\n先保存 然后关闭')
    const store = useWorkspaceStore()
    store.terminalSessions = [{ id: 'spacing', title: 'Codex', providerKind: 'codex', cwd: 'D:/Demo' }]
    store.activeCliSessionId = 'spacing'
    const original = '我        是  奥特曼\n\n先保存       然后关闭'
    store.terminalDrafts = { spacing: original }
    const { app, root } = mountPane(pinia, { mode: 'cli', activeSessionId: 'spacing' })
    await flushPromises()
    ;(root.querySelector('[data-testid="terminal-normalize-spacing"]') as HTMLButtonElement).click()
    await flushPromises()
    await nextTick()
    expect(mocks.normalizePromptSpacing).toHaveBeenCalledWith(original)
    expect(store.terminalDrafts.spacing).toBe('我是奥特曼\n\n先保存 然后关闭')
    expect(mocks.writeTerminalInput).not.toHaveBeenCalled()
    ;(root.querySelector('[data-testid="terminal-undo-spacing"]') as HTMLButtonElement).click()
    await nextTick()
    expect(store.terminalDrafts.spacing).toBe(original)
    expect(root.querySelector('[data-testid="terminal-undo-spacing"]')).toBeNull()
    app.unmount()
    root.remove()
  })

  it('downloads spacing resources only after explicit confirmation, then normalizes', async () => {
    mocks.isTauri.mockReturnValue(true)
    mocks.promptSpacingStatus.mockResolvedValueOnce({ ready: false }).mockResolvedValue({ ready: true })
    mocks.normalizePromptSpacing.mockResolvedValue('我是')
    const store = useWorkspaceStore()
    store.terminalSessions = [{ id: 'spacing', title: 'Codex', providerKind: 'codex', cwd: 'D:/Demo' }]
    store.activeCliSessionId = 'spacing'
    store.terminalDrafts = { spacing: '我    是' }
    const { app, root } = mountPane(pinia, { mode: 'cli', activeSessionId: 'spacing' })
    await flushPromises()
    expect(mocks.promptSpacingStatus).not.toHaveBeenCalled()
    expect(mocks.installPromptSpacing).not.toHaveBeenCalled()
    ;(root.querySelector('[data-testid="terminal-normalize-spacing"]') as HTMLButtonElement).click()
    await flushPromises()
    await nextTick()
    expect(mocks.normalizePromptSpacing).not.toHaveBeenCalled()
    expect(mocks.installPromptSpacing).not.toHaveBeenCalled()
    expect(store.terminalDrafts.spacing).toBe('我    是')
    ;(root.querySelector('[data-testid="terminal-spacing-download"] button') as HTMLButtonElement).click()
    await flushPromises()
    await nextTick()
    expect(mocks.installPromptSpacing).toHaveBeenCalledOnce()
    expect(store.terminalDrafts.spacing).toBe('我是')
    app.unmount()
    root.remove()
  })

  it.each(['edited', 'timeout'])('preserves the draft when spacing processing is %s', async (outcome) => {
    mocks.isTauri.mockReturnValue(true)
    let finish!: (value: string) => void
    let fail!: (error: Error) => void
    mocks.normalizePromptSpacing.mockImplementation(() => new Promise<string>((resolve, reject) => { finish = resolve; fail = reject }))
    const store = useWorkspaceStore()
    store.terminalSessions = [{ id: 'spacing', title: 'Codex', providerKind: 'codex', cwd: 'D:/Demo' }]
    store.activeCliSessionId = 'spacing'
    store.terminalDrafts = { spacing: '我    是' }
    const { app, root } = mountPane(pinia, { mode: 'cli', activeSessionId: 'spacing' })
    await flushPromises()
    const button = root.querySelector('[data-testid="terminal-normalize-spacing"]') as HTMLButtonElement
    button.click()
    await nextTick()
    expect(button.disabled).toBe(true)
    if (outcome === 'edited') {
      store.setTerminalDraft('spacing', '刚刚新输入的内容')
      finish('我是')
    } else {
      fail(new Error('处理超过 5 秒'))
    }
    await flushPromises()
    await nextTick()
    expect(store.terminalDrafts.spacing).toBe(outcome === 'edited' ? '刚刚新输入的内容' : '我    是')
    expect(root.querySelector('[data-testid="terminal-undo-spacing"]')).toBeNull()
  ...
  it('selects a project or global Skill with arrow keys and Enter, then sends a compatible instruction', async () => {
    ...
    textarea.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: true, cancelable: true }))
    await nextTick()
    // Removing and reopening the slash menu must retain the selected skill.
    textarea.value = ''
    textarea.dispatchEvent(new Event('input', { bubbles: true }))
    await nextTick()
    ...
    textarea.dispatchEvent(new Event('input', { bubbles: true }))
    await nextTick()
    expect(root.querySelector('.skill-completion-item.selected')?.textContent).toContain('/agents-updater')
    ...
    expect(sendMessage).toHaveBeenCalledWith('session-main', '使用agents-updater\nSkill 文件：D:/Demo Files/.codex/skills/agents-updater/SKILL.md\nSkill 目录：D:/Demo Files/.codex/skills/agents-updater\n请先读取该 Skill 文件，并以 Skill 目录解析其中的相对路径。\n\n检查并更新配置')
  ...
  it('fills the selected Skill display tag with a mouse click', async () => {
    ...
    textarea.dispatchEvent(new Event('input', { bubbles: true }))
    await nextTick()
    // Filtering alone also establishes the remembered selection, without an arrow or hover.
    textarea.value = ''
    textarea.dispatchEvent(new Event('input', { bubbles: true }))
    await nextTick()
    textarea.value = '/'
    ...
    textarea.dispatchEvent(new Event('input', { bubbles: true }))
    await nextTick()
    expect(root.querySelector('.skill-completion-item.selected')?.textContent).toContain('/Agent Browser')
    textarea.value = '/Agent'
    textarea.setSelectionRange(textarea.value.length, textarea.value.length)
    textarea.dispatchEvent(new Event('input', { bubbles: true }))
    await nextTick()
    ...
    await nextTick()
    expect(root.querySelector('[data-testid="skill-invocation-chip"]')).toBeNull()
    expect(root.querySelector('.skill-completion-popup')).not.toBeNull()
    root.querySelector('.skill-completion-item')?.dispatchEvent(
      new MouseEvent('contextmenu', { button: 2, bubbles: true, cancelable: true }),
    )
    await flushPromises()
    expect(openFile).toHaveBeenCalledWith('C:/skills/agent-browser/SKILL.md', {
      createUnsupportedTabOnError: false, verifyExistingFile: true,
    })
    expect(store.settings.activePreviewMode).toBe('code')
  ...
  it('replays raw terminal colors when switching themes without sending input to the CLI', async () => {
    const store = useWorkspaceStore()
    store.settings.themeId = 'dark'
    store.terminalSessions = [{ id: 'session-main', title: 'Claude', providerKind: 'claude', cwd: 'D:/Demo Files' }]
    store.activeTerminalSessionId = 'session-main'
    store.activeCliSessionId = 'session-main'
    const output = '\x1b[48;2;41;41;41mPrompt\x1b[0m'
    mocks.getTerminalBuffer.mockResolvedValue({ buffer: output, startByte: 0, endByte: output.length })
    const { app, root } = mountPane(pinia, { mode: 'cli', sessionIds: ['session-main'], activeSessionId: 'session-main' })
    await flushPromises()
    const terminal = mocks.terminalInstances[0]!
    terminal.write.mockClear()
    mocks.writeTerminalInput.mockClear()
    store.settings.themeId = 'light'
    await nextTick()
    await flushPromises()
    const rgb = getTheme('light').colors.bg.secondary.match(/\w\w/g)!.map((part) => parseInt(part, 16)).join(';')
    expect(terminal.write.mock.calls.map(([data]) => data).join('')).toContain(`\x1b[48;2;${rgb}mPrompt`)
    terminal.write.mockClear()
    store.settings.themeId = 'dark'
    await nextTick()
    await flushPromises()
    expect(terminal.write.mock.calls.map(([data]) => data).join('')).toContain(output)
    expect(mocks.writeTerminalInput).not.toHaveBeenCalled()
    app.unmount()
    root.remove()
  })
