...
import { FolderOpen, Plus, Save, Trash2, X } from 'lucide-vue-next'
...
const canCreate = computed(() => !isCreating.value && !!sessionCwd.value.trim() && store.isCliProviderVisible(selectedProviderId.value) && visibleProviders.value.length > 0)
...
watch(visibleProviders, (providers) => {
  if (!providers.some((provider) => provider.id === selectedProviderId.value) && providers[0]) selectedProviderId.value = providers[0].id
})

watch(show, (open) => {
  if (!open) return
  const activeProvider = store.activeCliTerminalSession?.providerKind
  const preferredProvider = CLI_PROVIDER_OPTIONS.find((provider) => provider.id === activeProvider && store.isCliProviderVisible(provider.id))?.id
  sessionName.value = `会话 ${new Date().toLocaleTimeString('zh-CN', { hour12: false })}`
  sessionPrompt.value = ''
  phraseTitle.value = ''
  sessionCwd.value = store.workspace?.rootPath ?? ''
  selectedProviderId.value = preferredProvider ?? visibleProviders.value[0]?.id ?? 'claude'
  error.value = null
  void nextTick(() => {
        ...
        <div class="new-session-field">
          ...
          <div class="new-session-provider-list">
            <button
              ...
              :disabled="isCreating || !store.isCliProviderVisible(provider.id)"
              @click="selectedProviderId = provider.id"
            >
              <CliProviderIcon :provider="provider.id" :size="14" />
