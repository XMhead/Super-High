...
import { LoaderCircle, MessageSquarePlus, Mic, MicOff, Play, Plus, RemoveFormatting, RotateCcw, Save, Send, Sparkles, Square, TerminalSquare, Trash2, X } from 'lucide-vue-next'
import CliProviderIcon from './icons/CliProviderIcon.vue'
...
const startingLocalTerminal = ref(false)
...
const skillSelectionBySession = new Map<string, string>()
...
const spacingSessionId = ref<string | null>(null)
const spacingDownloadRequested = ref(false)
const isDownloadingSpacing = ref(false)
const spacingUndoBySession = ref<Record<string, { before: string; after: string } | undefined>>({})
const spacingStatusBySession = ref<Record<string, string | undefined>>({})
let spacingDisposed = false
const isNormalizingSpacing = computed(() => !!activeSession.value && spacingSessionId.value === activeSession.value.id)
const spacingStatus = computed(() => isNormalizingSpacing.value ? '正在整理空格…' : spacingStatusBySession.value[activeSession.value?.id ?? ''] ?? '')
const canNormalizeSpacing = computed(() => isTauri() && !!activeDraft.value.trim() && !isEnded.value && !isPastingImage.value && !isEnhancingPrompt.value && !spacingSessionId.value && !isDownloadingSpacing.value)
const canUndoSpacing = computed(() => {
  const undo = spacingUndoBySession.value[activeSession.value?.id ?? '']
  return !!undo && undo.after === activeDraft.value && !isNormalizingSpacing.value && !isEnhancingPrompt.value && !isEnded.value
})
...
// Codex 的整帧界面使用普通渲染器，避免 WebGL 在 DPI 变化后
// 画布与绘制比例不同步，造成文字缩小、偏移或重叠。
const isFrameRenderedTerminal = computed(() => (
  isCliMode.value
  && activeSession.value?.providerKind === 'codex'
))
...
const cliHistoryMessages = computed(() => mergeHistoryMessages(
  collectCliHistoryMessages(
    ...
    store.cliTerminalSessions.map((session) => session.id),
  ),
...
function refreshSkillPopup() {
  ...
  const rememberedPath = skillSelectionBySession.get(activeSession.value?.id ?? '')
  skillSelectedIndex.value = Math.max(0, candidates.findIndex((candidate) => candidate.path === rememberedPath))
  skillPopupOpen.value = candidates.length > 0
  if (skillPopupOpen.value) {
    skillSelectionBySession.set(activeSession.value?.id ?? '', candidates[skillSelectedIndex.value].path)
    closeHistoryPopup()
  }
}
...
function moveSkillPopup(delta: number) {
  ...
  moveToSkillPopup((skillSelectedIndex.value + delta + skillCandidates.value.length) % skillCandidates.value.length)
}
...
function moveToSkillPopup(index: number) {
  ...
  skillSelectionBySession.set(activeSession.value?.id ?? '', skillCandidates.value[index].path)
}

async function previewSkill(index: number) {
  const candidate = skillCandidates.value[index]
  if (!candidate || candidate.path === promptCommand.path) return
  moveToSkillPopup(index)
  closeSkillPopup()
  await store.openTerminalFileLink(candidate.path)
}
...
let terminalScrollGesture = 0
function noteTerminalScrollGesture() { terminalScrollGesture += 1 }
...
async function createLocalTerminal() {
  if (startingLocalTerminal.value) return
  startingLocalTerminal.value = true
  try {
    await store.createTerminalSession('local', { cwd: props.cwd })
    await nextTick()
    focusTerminal()
  } catch (error) {
    store.showErrorMessage(`启动 PowerShell 失败：${error instanceof Error ? error.message : String(error)}`)
  } finally {
    startingLocalTerminal.value = false
  }
}
...
async function openActiveConversationExternally() {
  ...
  if (!['claude', 'codex'].includes(session.providerKind)) {
...
function disposeTerminal() {
  ...
  terminal?.element?.removeEventListener('wheel', noteTerminalScrollGesture)
  terminal?.element?.removeEventListener('pointerdown', noteTerminalScrollGesture)
  terminal?.dispose()
  terminal = null
  fitAddon = null
...
/** 整帧 CLI 使用普通渲染器，其余终端保留 WebGL。 */
function syncTerminalRenderer() {
  if (!terminal) return
  const wantWebgl = !props.compactTeam && !isFrameRenderedTerminal.value
...
function ensureTerminal() {
  ...
  terminal = new Terminal({
    cursorBlink: true,
    // CLI output may set a dark cell background even under a light theme.
  ...
  terminal.attachCustomKeyEventHandler((event) => {
    // xterm 5.x rewrites plain Alt+arrows as Ctrl+arrows; preserve the modifier for TUIs.
    if (event.type === 'keydown' && event.altKey
      && !event.ctrlKey && !event.shiftKey && !event.metaKey && !event.isComposing) {
      const directions: Record<string, string> = { ArrowUp: 'A', ArrowDown: 'B', ArrowRight: 'C', ArrowLeft: 'D' }
      const direction = directions[event.key]
      if (direction) {
        event.preventDefault()
        if (!restoringTerminal.value && activeSession.value) {
          void store.writeTerminalInput(`\x1b[1;3${direction}`, activeSession.value.id)
        }
        return false
      }
    }
  ...
  terminal.element?.addEventListener('wheel', noteTerminalScrollGesture, { passive: true })
  terminal.element?.addEventListener('pointerdown', noteTerminalScrollGesture)
...
function applyBufferedOutput() {
  ...
  if (!data) return
  const renderedData = terminalColors.transform(codexTerminalRenderer?.render(data) ?? data, currentThemeId.value)
  if (!renderedData) {
  ...
  const followBottom = restoringTerminal.value || writeTerminal.buffer.active.viewportY >= writeTerminal.buffer.active.baseY
  const scrollGesture = terminalScrollGesture
  writeInFlight = true
  try {
    writeTerminal.write(renderedData, () => {
      if (outputGeneration !== terminalOutputGeneration || terminal !== writeTerminal) return
      writeInFlight = false
      if (followBottom && scrollGesture === terminalScrollGesture) writeTerminal.scrollToBottom()
      if (bufferedOutputChunks.length) renderBuffer()
      else restoringTerminal.value = false
    })
  } catch {
...
async function primeTerminalBuffer(sessionId: string, preserveOutputOnFailure = false) {
  if (bufferRefreshInFlightForSession === sessionId) return
  const activationToken = terminalActivationToken
...
async function sendDraft() {
  ...
  if (!activeSession.value || isEnded.value || isPastingImage.value || isEnhancingPrompt.value || isNormalizingSpacing.value) return
  const sessionId = activeSession.value.id
  ...
  delete spacingUndoBySession.value[sessionId]
  delete spacingStatusBySession.value[sessionId]
  skillInvocationBySession.value = {
    ...skillInvocationBySession.value,
    [sessionId]: null,
  }
...
async function normalizeDraftSpacing() {
  const sessionId = activeSession.value?.id
  if (!sessionId || !canNormalizeSpacing.value) return
  const original = activeDraft.value
  spacingSessionId.value = sessionId
  delete spacingStatusBySession.value[sessionId]
  closeHistoryPopup()
  closeSkillPopup()
  try {
    const status = await backend.promptSpacingStatus()
    if (spacingDisposed) return
    if (!status.ready) {
      spacingDownloadRequested.value = true
      return
    }
    const result = await backend.normalizePromptSpacing(original)
    if (spacingDisposed) return
    if (store.terminalDrafts[sessionId] !== original || !sessions.value.some(session => session.id === sessionId)) {
      spacingStatusBySession.value[sessionId] = '输入已变化，未替换当前内容。'
      return
    }
    if (!result.trim()) throw new Error('空格整理未返回有效内容，已保留原文。')
    if (result === original) {
      spacingStatusBySession.value[sessionId] = '无需整理。'
      return
    }
    spacingUndoBySession.value[sessionId] = { before: original, after: result }
    store.setTerminalDraft(sessionId, result)
    spacingStatusBySession.value[sessionId] = '已整理空格，可撤销。'
    ...
    if (!spacingDisposed) spacingStatusBySession.value[sessionId] = `整理失败，已保留原文：${error instanceof Error ? error.message : String(error)}`
  } finally {
    spacingSessionId.value = null
  }
}

async function downloadSpacingResources() {
  if (isDownloadingSpacing.value) return
  const sessionId = activeSession.value?.id
  isDownloadingSpacing.value = true
  spacingDownloadRequested.value = false
  try {
    await backend.installPromptSpacing()
    if (spacingDisposed) return
    isDownloadingSpacing.value = false
    if (activeSession.value?.id === sessionId) await normalizeDraftSpacing()
    else if (sessionId) spacingStatusBySession.value[sessionId] = '离线资源已就绪。'
  } catch (error) {
    if (!spacingDisposed) {
      spacingDownloadRequested.value = true
      if (sessionId) spacingStatusBySession.value[sessionId] = `下载失败，可重试：${error instanceof Error ? error.message : String(error)}`
    }
  } finally {
    isDownloadingSpacing.value = false
  }
...
function undoDraftSpacing() {
  const sessionId = activeSession.value?.id
  if (!sessionId || !canUndoSpacing.value) return
  store.setTerminalDraft(sessionId, spacingUndoBySession.value[sessionId]!.before)
  delete spacingUndoBySession.value[sessionId]
  spacingStatusBySession.value[sessionId] = '已撤销空格整理。'
  void nextTick(() => {
    syncDraftHeight(draftInput.value, true)
    draftInput.value?.focus()
  })
...
onBeforeUnmount(() => { spacingDisposed = true })

async function enhanceDraftPrompt() {
  const session = activeSession.value
  const original = activeDraft.value
  if (!session || !original.trim() || isEnded.value || isPastingImage.value || promptEnhancementSessionId.value || isNormalizingSpacing.value) return
...
async function openTerminalFileLink(path: string) {
  await store.openTerminalFileLink(path)
}
...
watch(currentThemeId, () => {
  ...
  if (activeSessionIdSnapshot) void primeTerminalBuffer(activeSessionIdSnapshot, true)
})
  ...
  >
    <div class="terminal-tabs-bar">
      ...
      <div v-if="!compactTeam" class="terminal-actions">
        <template v-if="isCliMode">
          ...
          >
            <CliProviderIcon :provider="provider.id" :size="13" />
        ...
        <template v-else>
          <button
            class="ghost-button"
            :disabled="startingLocalTerminal || !(props.cwd.trim() || store.workspace?.rootPath)"
            title="在当前项目目录新建 PowerShell"
            data-testid="local-terminal-create"
            @click="createLocalTerminal"
          >
            <Plus :size="13" />
            <span>{{ startingLocalTerminal ? '启动中' : '新建 PowerShell' }}</span>
          </button>
    ...
    <div v-if="activeSession" class="terminal-body" :class="{ 'with-cli-input': showCliConversation }">
      <div class="cli-render-region" @click="focusTerminal">
        ...
        />
        <button
          v-if="!compactTeam && isCliMode && activeSession && ['claude', 'codex'].includes(activeSession.providerKind)"
          type="button"
      ...
      >
        ...
        <div class="cli-input-shell">
          <div class="cli-input-stack">
            ...
            <div v-if="spacingStatus" class="cli-voice-status" role="status" data-testid="terminal-spacing-status">{{ spacingStatus }}</div>
            <div v-if="isDownloadingSpacing" class="cli-voice-status" role="status">正在下载离线整理资源，首次下载需要一些时间…</div>
            <div v-else-if="spacingDownloadRequested" class="cli-voice-status" data-testid="terminal-spacing-download">
              <span>首次使用需下载约 85 MB，解压后约 96 MB。以后离线使用，用完释放内存。</span>
              <button type="button" class="ghost-button small" @click="downloadSpacingResources">下载并整理</button>
              <button type="button" class="ghost-button small" @click="spacingDownloadRequested = false">暂不下载</button>
            </div>
          </div>
          <button
            type="button"
            class="conversation-voice-button prompt-enhance-button"
            :disabled="!canNormalizeSpacing"
            title="整理粘贴文本中的多余空格，保留段落和代码"
            aria-label="整理空格"
            data-testid="terminal-normalize-spacing"
            @click="normalizeDraftSpacing"
          >
            <LoaderCircle v-if="isNormalizingSpacing" :size="14" class="prompt-enhance-spinner" />
            <RemoveFormatting v-else :size="14" />
          </button>
          <button
            v-if="canUndoSpacing"
            type="button"
            ...
            title="撤销本次空格整理"
            aria-label="撤销本次空格整理"
            data-testid="terminal-undo-spacing"
            @click="undoDraftSpacing"
          >
            <RotateCcw :size="14" />
          </button>
          <button
    ...
    <div v-else class="terminal-empty-state">
      <div class="terminal-empty-card">
        ...
        <p>
          {{ isCliMode ? '创建 Claude Code 或 Codex 会话后，上方显示原始 CLI 渲染，下方保留对话输入区。' : '在当前项目目录打开 PowerShell，运行命令或脚本。' }}
        </p>
        <div class="terminal-empty-actions">
          ...
          <template v-else>
            <button
              class="ghost-button"
              :disabled="startingLocalTerminal || !(props.cwd.trim() || store.workspace?.rootPath)"
              data-testid="local-terminal-empty-create"
              @click="createLocalTerminal"
            >
              <Plus :size="13" />
              <span>{{ startingLocalTerminal ? '启动中' : '新建 PowerShell' }}</span>
