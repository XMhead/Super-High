...
import { cliProviderLaunchEnabled, CLI_PROVIDER_OPTIONS, cliProviderLabel } from '@/lib/cliProviders'
...
function createProviderSession(provider: TerminalProviderKind) {
  if (!cliProviderLaunchEnabled(provider)) return
  void (async () => {
...
async function toggleProjectStartup() {
  ...
  try {
    ...
    store.showErrorMessage(`工作区操作失败：${error instanceof Error ? error.message : String(error)}`)
  } finally {
  ...
  >
      ...
      <div v-if="!compactTeam" class="terminal-actions">
        <template v-if="isCliMode">
          <button
            ...
            :key="provider.id"
            class="ghost-button"
            :disabled="!cliProviderLaunchEnabled(provider.id)"
            :title="cliProviderLaunchEnabled(provider.id) ? `左键在 SuperHigh 内启动 ${provider.name}；右键在外部 CMD 启动` : 'dsh-tui 唤起暂未启用'"
        ...
        <template v-else>
          ...
          <button
            ...
            :title="projectStartupSession ? '关闭 SuperHigh 内托管的服务进程' : `在 SuperHigh 终端启动 ${projectStartupName}`"
            @click="toggleProjectStartup"
          >
            <Square v-if="projectStartupSession" :size="13" />
            <Play v-else :size="13" />
            <span>{{ projectStartupSession ? '关闭服务' : '启动服务' }}</span>
          </button>
        </template>
      </div>
    </div>
    ...
    <div v-if="activeSession" class="terminal-body" :class="{ 'with-cli-input': showCliConversation }">
      <div class="cli-render-region" @click="focusTerminal">
        ...
        />
        <button
          v-if="!compactTeam && isCliMode && activeSession && ['claude', 'codex', 'dsh'].includes(activeSession.providerKind)"
          type="button"
    ...
    <div v-else class="terminal-empty-state">
      <div class="terminal-empty-card">
        ...
        <p>
          {{ isCliMode ? '创建 Claude Code 或 Codex 会话后，上方显示原始 CLI 渲染，下方保留对话输入区。' : '启动服务链后，可在此查看并输入各服务控制台命令。' }}
        </p>
