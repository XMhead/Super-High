...
import { CalendarDays, Check, Copy, Eye, EyeOff, NotebookPen, Plus, Search, Trash2, X } from 'lucide-vue-next'
...
async function copyCurrentMemo() {
  ...
  if (!note || !note.content || copyStatus.value === 'copying') return
...
watch(() => [selectedId.value, selectedMemo.value?.content], () => { copyStatus.value = 'idle' })
...
const previewContent = computed(() => {
  ...
  previewLinks.value.forEach(({ match, target }, index) => {
    ...
    parts.push(escapeHtml(value.slice(offset, match.start)))
        ...
        <template v-if="selectedMemo">
          <div class="memo-editor-heading">
            ...
            <button
              ...
              :disabled="!selectedMemo.content || copyStatus === 'copying'"
          ...
          <textarea
            ...
            @blur="flushSave"
          />
          <div v-else class="memo-content-preview" @click="openPreviewLink" v-html="previewContent" />
        </template>
