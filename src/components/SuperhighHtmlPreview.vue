...
import { handleDragonCoreEditorOperation } from '@/lib/dragonCoreEditor'
...
import type { EditorTab, ItemLibrarySource } from '@/types'
...
type PreviewContext = { pageId: string | null; codePreviewRoot: string | null; mainSource: ItemLibrarySource }

const store = useWorkspaceStore()
const previewFrame = ref<HTMLIFrameElement | null>(null)
const context = ref<PreviewContext>({ pageId: null, codePreviewRoot: null, mainSource: 'ni' })
...
function postPreviewContext() {
  ...
  postToPreview({
    ...
    mainItemLibrarySource: context.value.mainSource,
  })
}
...
async function loadPreviewContext() {
  ...
  context.value = { pageId: null, codePreviewRoot: null, mainSource: 'ni' }
  if (!entry || !root) {
    previewContextLoaded.value = true
    return
  }

  const [mainSource, rawManifest] = await Promise.all([
    loadProjectItemLibraryMainSource(root),
    backend.readFile(`${editorDirectory()}/editor.json`).catch(() => ''),
  ])
...
async function handleItemLibraryRequest(data: Record<string, unknown>) {
  if (data.type !== 'superhigh-editor:item-library') return false
  if (typeof data.requestId !== 'string' || !workspaceRoot.value) return true
  try {
    const source = data.source === undefined ? context.value.mainSource : data.source
    if (source !== 'ni' && source !== 'mm') throw new Error('项目编辑器只支持 ni 或 mm 物品库。')
    if (typeof data.query !== 'string' || !data.query.trim()) throw new Error('物品库查询内容不能为空。')
    const page = Number.isInteger(data.page) && Number(data.page) > 0 ? Number(data.page) : undefined
    const pageSize = Number.isInteger(data.pageSize) && Number(data.pageSize) > 0 ? Number(data.pageSize) : undefined
    const result = await backend.searchItemLibrary(workspaceRoot.value, source, data.query, typeof data.expandedQuery === 'string' ? data.expandedQuery : undefined, store.settings.dragonCoreClientRootPath, page, pageSize)
    postToPreview({ type: 'superhigh-editor:item-library-result', requestId: data.requestId, result })
  } catch (error) {
    postToPreview({ type: 'superhigh-editor:item-library-error', requestId: data.requestId, error: formatError(error) })
  }
  return true
}

async function handleItemLibraryKeysRequest(data: Record<string, unknown>) {
  if (data.type !== 'superhigh-editor:item-library-keys') return false
  if (typeof data.requestId !== 'string' || !workspaceRoot.value) return true
  try {
    const source = data.source === undefined ? context.value.mainSource : data.source
    if (source !== 'ni' && source !== 'mm') throw new Error('项目编辑器只支持 ni 或 mm 物品库。')
    const result = (await backend.itemLibraryKeys(workspaceRoot.value, source)).itemKeys
    postToPreview({ type: 'superhigh-editor:item-library-keys-result', requestId: data.requestId, result })
  } catch (error) {
    postToPreview({ type: 'superhigh-editor:item-library-keys-error', requestId: data.requestId, error: formatError(error) })
  }
  return true
}

async function handleMonsterLibraryRequest(data: Record<string, unknown>) {
  if (data.type !== 'superhigh-editor:monster-library') return false
  if (typeof data.requestId !== 'string' || !workspaceRoot.value) return true
  try {
    const query = typeof data.query === 'string' ? data.query : ''
    const page = Number.isInteger(data.page) && Number(data.page) > 0 ? Number(data.page) : undefined
    const pageSize = Number.isInteger(data.pageSize) && Number(data.pageSize) > 0 ? Number(data.pageSize) : undefined
    const result = await backend.searchMonsterLibrary(workspaceRoot.value, query, page, pageSize)
    postToPreview({ type: 'superhigh-editor:monster-library-result', requestId: data.requestId, result })
  } catch (error) {
    postToPreview({ type: 'superhigh-editor:monster-library-error', requestId: data.requestId, error: formatError(error) })
  }
  return true
}

async function handleMinecraftRequest(data: Record<string, unknown>) {
  if (data.type !== 'superhigh-editor:minecraft') return false
  if (typeof data.requestId !== 'string' || typeof data.operation !== 'string' || !workspaceRoot.value) return true
  try {
    let result: unknown
    if (data.operation === 'players') result = await backend.onlinePlayers(workspaceRoot.value)
    else if (data.operation === 'give-item') {
      if (typeof data.itemKey !== 'string' || !data.itemKey.trim()) throw new Error('物品 key 不能为空。')
      const amount = Number.isInteger(data.amount) && Number(data.amount) > 0 ? Number(data.amount) : 1
      result = typeof data.playerName === 'string' && data.playerName.trim()
        ? await backend.sendItemToNamedPlayer(workspaceRoot.value, data.itemKey, data.playerName, amount)
        : await backend.sendItemToPlayer(workspaceRoot.value, data.itemKey, amount)
    } else if (data.operation === 'open-dragoncore-gui') result = await backend.openDragonCoreGui(workspaceRoot.value, workspaceFilePath(data.path))
    else throw new Error('不支持的项目编辑器 Minecraft 操作。')
    postToPreview({ type: 'superhigh-editor:minecraft-result', requestId: data.requestId, result })
  } catch (error) {
    postToPreview({ type: 'superhigh-editor:minecraft-error', requestId: data.requestId, error: formatError(error) })
  }
  return true
}

async function handleDragonCoreRequest(data: Record<string, unknown>) {
  if (data.type !== 'superhigh-editor:dragoncore') return false
  if (typeof data.requestId !== 'string' || typeof data.operation !== 'string' || !workspaceRoot.value) return true
  try {
    const payload = data.payload && typeof data.payload === 'object' && !Array.isArray(data.payload)
      ? data.payload as Record<string, unknown>
      : {}
    const result = await handleDragonCoreEditorOperation(data.operation, payload, {
      workspaceRoot: workspaceRoot.value,
      dragonCoreClientRootPath: store.settings.dragonCoreClientRootPath,
      readFile: backend.readFile,
      writeFile: backend.writeFile,
      listDirectory: backend.listDirectory,
      readMediaAsDataUrl: backend.readMediaAsDataUrl,
    })
    postToPreview({ type: 'superhigh-editor:dragoncore-result', requestId: data.requestId, result })
  } catch (error) {
    postToPreview({ type: 'superhigh-editor:dragoncore-error', requestId: data.requestId, error: formatError(error) })
  }
  return true
}
...
async function onMessage(event: MessageEvent) {
  ...
  if (await handleItemLibraryRequest(data)) return
  if (await handleItemLibraryKeysRequest(data)) return
  if (await handleMonsterLibraryRequest(data)) return
  if (await handleMinecraftRequest(data)) return
  if (await handleDragonCoreRequest(data)) return
  if (await handleScriptToolRequest(data)) return
  if (await handleCodePreviewRequest(data)) return
  if (data.type !== 'superhigh-editor:rcon' || typeof data.requestId !== 'string' || typeof data.command !== 'string' || !workspaceRoot.value) return
  try {
    const result = await backend.executeProjectRconCommand(workspaceRoot.value, data.command)
    postToPreview({ type: 'superhigh-editor:rcon-result', requestId: data.requestId, result })
  } catch (error) {
    postToPreview({ type: 'superhigh-editor:rcon-error', requestId: data.requestId, error: formatError(error) })
  }
}
