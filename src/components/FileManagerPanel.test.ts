...
vi.mock('./ExplorerPane.vue', () => ({
  default: {
    props: ['rootPath', 'title', 'emptyText', 'customFileOpen'],
    emits: ['file-open'],
    template: '<aside data-testid="explorer-stub">{{ title }}</aside>',
  },
}))
...
describe('FileManagerPanel', () => {
  ...
  it('opens an arbitrary directory as a separate tab', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')
    ...
    ;(root.querySelector('.file-manager-tab.add') as HTMLButtonElement).click()
    await flushPromises()
    await nextTick()
