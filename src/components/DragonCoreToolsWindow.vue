...
async function closeCliSessions() {
  await Promise.all(store.cliTerminalSessions.map((session) => store.closeTerminalSession(session.id)))
}
...
onMounted(() => {
  void (async () => {
    ...
    unlistenClose = await getCurrentWindow().onCloseRequested((event) => {
      ...
      void closeCliSessions().finally(() => emit('dragoncore-tools-close-request'))
    })
  })()
})

onBeforeUnmount(() => {
  void closeCliSessions()
  unlistenFile?.()
  unlistenClose?.()
})
</script>
...
<template>
  <main v-if="ready" class="dragoncore-tools-window" :class="{ 'is-cli-open': cliOpen }">
    ...
    <TerminalPane v-if="cliOpen" mode="cli" :cwd="store.workspace?.rootPath ?? ''" />
  </main>
</template>
