...
const steps = [
  ...
  },
  {
    ...
    actions: ['启动前确认工作目录和 CLI；工具不可用时，到「设置 → CLI 管理」检查环境。', '在左侧切换已有对话继续追问；重命名便于区分任务，删除对话会结束对应会话。'],
  ...
  },
  {
    ...
    description: '活动栏的「渠道检测」用于检查已配置供应商与模型的请求是否正常。先确认 CLI 环境，再定位模型连接问题。',
  ...
  },
  {
    ...
    actions: ['按手机端设置中显示的地址和连接信息操作；使用期间保持电脑和 Host 运行。', '以后想重新查看这份教程，打开「设置 → 新手教程」，点击「重放新手教程」。'],
...
onMounted(() => {
  ...
  if (!dismissed) void openTutorial()
})
onBeforeUnmount(() => {
