...
const PRIMARY_PROVIDERS = ['claude', 'codex', 'dsh'] as const
...
const providerPages = computed(() => [...PRIMARY_PROVIDERS, ...otherProviders.value])
...
function providerLabel(providerKind: string): string {
  if (providerKind === 'dsh') return 'DSH CLI'
      ...
      <div v-if="visibleConversations.length || currentProviders.length" class="conversation-provider-grid">
        ...
        >
          <header class="conversation-provider-header">
            ...
            <DshIcon v-else-if="providerKind === 'dsh'" :size="16" />
              ...
              <footer class="conversation-map-card-footer">
                ...
                <div class="conversation-map-card-actions">
                  ...
                  <button
                    ...
                    :disabled="!conversation.resumeSupported || !cliProviderLaunchEnabled(conversation.providerKind)"
      ...
      <template v-else>
        <div class="conversation-management-detail-actions">
          <button
            ...
            :disabled="resumeLoading || !selectedConversation.resumeSupported || !cliProviderLaunchEnabled(selectedConversation.providerKind)"
...
.conversation-provider-column.provider-dsh { border-top-color: var(--color-accent-green); }
...
.conversation-map-card.provider-dsh .conversation-map-card-title-line strong,
