...
describe('plugin runtime UI', () => {
  ...
  it('renders plugin commands in the activity bar and runs them', async () => {
    ...
    store.pluginCommands = [{ id: 'sample.hello', pluginId: 'sample-tools', title: '示例工具', tooltip: '运行示例工具' }]
    store.runPluginCommand = vi.fn()
    const { app, root } = mount(ActivityBar, pinia)

    const button = root.querySelector('[data-testid="plugin-command-sample.hello"]') as HTMLButtonElement
    expect(button?.title).toBe('运行示例工具')
    button.click()
    await nextTick()
    expect(store.runPluginCommand).toHaveBeenCalledWith('sample.hello')
    app.unmount()
    root.remove()
  })
  ...
  it('renders plugin manager status and disabled action', async () => {
    ...
    store.pluginStatuses = [{
      ...
      name: '示例工具',
      version: '0.1.0',
      rootPath: 'C:/Users/Admin/AppData/Roaming/SuperHigh/plugins/sample-tools',
      main: 'main.js',
      mainPath: 'C:/Users/Admin/AppData/Roaming/SuperHigh/plugins/sample-tools/main.js',
      style: 'style.css',
      stylePath: 'C:/Users/Admin/AppData/Roaming/SuperHigh/plugins/sample-tools/style.css',
      source: 'global',
      permissions: ['all'],
      unsafe: true,
      status: 'loaded',
    ...
    expect(pluginCard.textContent).toContain('示例工具')
    ...
    disable.click()
    expect(store.disablePlugin).toHaveBeenCalledWith('sample-tools')
    app.unmount()
    root.remove()
  })
})
