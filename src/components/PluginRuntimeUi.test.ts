...
describe('plugin runtime UI', () => {
  ...
  it('renders plugin manager status and disabled action', async () => {
    ...
    const pluginCard = root.querySelector('.settings-plugin-item') as HTMLElement
    ...
    const disable = pluginCard.querySelector('[role="switch"]') as HTMLButtonElement
    expect(disable.getAttribute('aria-checked')).toBe('true')
    disable.click()
    expect(store.disablePlugin).toHaveBeenCalledWith('horizon-tools')
    const search = root.querySelector('[aria-label="搜索插件"]') as HTMLInputElement
    search.value = '不存在的插件'
    search.dispatchEvent(new Event('input'))
    await nextTick()
    expect(root.querySelector('.settings-plugin-item')).toBeNull()
    expect(root.textContent).toContain('没有匹配的插件')
    app.unmount()
    root.remove()
  })
})
