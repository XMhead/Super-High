...
const mocks = vi.hoisted(() => ({
  manifest: {
    version: 1,
    pages: [{ id: 'dragoncore', title: 'DragonCore', entry: 'dragoncore.html', codePreviewRoot: 'DragonCore/Gui' }],
...
vi.mock('@/lib/itemLibraryConfig', () => ({
  loadProjectItemLibraryMainSource: vi.fn(async () => null),
}))
...
vi.mock('./ExplorerPane.vue', () => ({
  default: (_props: unknown, { emit }: { emit: (event: string, payload: unknown) => void }) => h('button', {
    ...
    onClick: () => emit('file-open', {
      path: 'D:/Project/DragonCore/Gui/b.yml',
      name: 'b.yml',
      type: 'file',
      extension: '.yml',
    }),
...
function tab(name: string): EditorTab {
  return {
    id: name,
    path: `D:/Project/DragonCore/Gui/${name}.yml`,
    name: `${name}.yml`,
    content: `${name}:\n  type: texture\n`,
    contentType: 'text',
    language: 'yaml',
...
describe('ProjectEditorPanel compact editor layout', () => {
  ...
  it('follows native resource-tree file switches in Monaco and the canvas', async () => {
    ...
    expect(frame.shadowRoot?.querySelector('#canvas-path')?.textContent).toBe('DragonCore/Gui/a.yml')
    mounted.root.querySelector<HTMLButtonElement>('.explorer-stub')!.click()
    await settle()

    expect(store.activeTabId).toBe('b')
    expect(frame.shadowRoot?.querySelector('#canvas-path')?.textContent).toBe('DragonCore/Gui/b.yml')
  })

  it('routes embedded page script-tool requests through the runner', async () => {
    mocks.pages.set('dragoncore.html', '<div id="runner-result"></div><script>window.superhighEditor.runScriptTool("bbmodel-ai", { action: "batch-generate" }).then((result) => { document.getElementById("runner-result").textContent = result.stdout })</script>')
    const mounted = mountPanel(pinia)
    apps.push(mounted.app)
    await settle()
  ...
  it('exposes browser timers to embedded pages for status polling', async () => {
    mocks.pages.set('dragoncore.html', '<div id="timer-result"></div><script>window.setTimeout(() => { document.getElementById("timer-result").textContent = "timer-ok" }, 0)</script>')
    const mounted = mountPanel(pinia)
    apps.push(mounted.app)
    await settle()
