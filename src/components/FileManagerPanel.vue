...
import { buildBreadcrumbs, extensionFromPath, fileNameFromPath, normalizePath } from '@/lib/path'
...
function isImageEntry(entry: FileEntry): boolean {
  return IMAGE_EXTENSIONS.has(extensionFor(entry))
}

function isVideoEntry(entry: FileEntry): boolean {
  return VIDEO_EXTENSIONS.has(extensionFor(entry))
}

function isMediaEntry(entry: FileEntry): boolean {
  return MEDIA_EXTENSIONS.has(extensionFor(entry))
}
...
function entryIcon(entry: FileEntry) {
  ...
  if (MEDIA_EXTENSIONS.has(extension)) return FileImage
  return File
}
