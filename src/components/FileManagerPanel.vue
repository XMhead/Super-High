...
const normalizedCurrentDirectory = computed(() => normalizeDirectoryPath(currentDirectory.value || explorerRoot.value || rootPath.value))
...
async function chooseDirectory() {
  ...
  if (!selected) return
  explorerRoot.value = normalizeDirectoryPath(selected)
  await navigateDirectory(explorerRoot.value)
}
...
async function openTreeFile(entry: FileEntry) {
  await navigateDirectory(parentDirectoryOf(entry.path))
  await openEntry(entry)
}

function startExplorerResize(event: PointerEvent) {
  event.preventDefault()
  resizeStart = { x: event.clientX, width: store.settings.panelLayout.explorerWidth }
  document.documentElement.classList.add('is-resizing-panels')
  window.addEventListener('pointermove', resizeExplorer)
...
function resizeExplorer(event: PointerEvent) {
  ...
  store.setPanelLayoutSize('explorerWidth', resizeStart.width + event.clientX - resizeStart.x)
}
...
function stopExplorerResize() {
  ...
  window.removeEventListener('pointermove', resizeExplorer)
...
<template>
  <section class="file-manager-panel" :style="{ gridTemplateColumns: `minmax(160px, ${store.settings.panelLayout.explorerWidth}px) 1px minmax(0, 1fr)` }" aria-label="文件管理器">
    <ExplorerPane
      :root-path="explorerRoot"
      custom-file-open
      @directory-open="navigateDirectory($event.path)"
      @file-open="openTreeFile"
    />
    <div
      class="workspace-resizer"
      role="separator"
      aria-label="调整文件资源管理器宽度"
    ...
    <main class="file-manager-browser-pane">
      ...
      <div class="file-manager-content">
        <section class="file-manager-grid-shell" @keydown="onGridKeydown">
    ...
    <div v-if="mediaPreview.open && currentMedia" class="file-manager-media-backdrop" @click.self="closeMediaPreview" @keydown="onGridKeydown">
