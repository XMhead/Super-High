...
function mountExtensionDocument() {
  ...
  const extensionWindow = {
    superhighScriptTool: {
      ...
      runStreaming: async (values: Record<string, unknown> = {}, onProgress: (event: { stream: 'stdout' | 'stderr'; chunk: string }) => void) => {
        const rootPath = workspaceRoot.value
        if (!rootPath || !values || Array.isArray(values) || typeof values !== 'object') throw new Error('脚本参数必须是键值对象。')
        if (typeof onProgress !== 'function') throw new Error('进度回调必须是函数。')
        const args = Object.fromEntries(Object.entries(values).map(([key, value]) => [key, String(value ?? '')]))
        return backend.runProjectScriptToolStreaming(rootPath, props.tool.id, args, onProgress)
      },
