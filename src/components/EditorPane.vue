...
import { buildWorkspaceBreadcrumbs, extensionFromPath, normalizePath } from '@/lib/path'
...
const yamlTopLevelPanelCollapsed = ref(true)
...
let yamlTopLevelPanelSuppressClick = false
...
const yamlTopLevelKeys = computed(() => {
  if (!activeTabIsText.value || activeTab.value?.language !== 'yaml') return []
...
const yamlTopLevelPanelHandleTitle = computed(() => (
  yamlTopLevelPanelCollapsed.value
    ? '拖动 key 面板；左键单击展开顶层 key 列表'
    : '拖动 key 面板；左键单击关闭顶层 key 列表'
))
...
function handleYamlTopLevelPanelHandleClick() {
  if (yamlTopLevelPanelSuppressClick) {
    yamlTopLevelPanelSuppressClick = false
    return
  }
  yamlTopLevelPanelCollapsed.value = !yamlTopLevelPanelCollapsed.value
}

function handleYamlTopLevelPanelClickCapture(event: MouseEvent) {
  if (!yamlTopLevelPanelSuppressClick) return
...
function stopYamlTopLevelPanelDrag() {
  const moved = yamlTopLevelPanelDrag?.moved ?? false
  yamlTopLevelPanelDrag = null
  if (moved) {
    yamlTopLevelPanelSuppressClick = true
    window.setTimeout(() => {
      yamlTopLevelPanelSuppressClick = false
    }, 0)
  }
    ...
    <div class="tabs-strip">
      ...
      <div ref="tabsActionMenuRoot" class="tabs-actions">
        ...
        <div v-if="tabsActionMenuOpen" class="menu-dropdown tabs-more-menu">
          ...
          <button class="menu-item" type="button" :disabled="!activeTab" @click="store.saveTabAs(activeTab!.id); closeTabsActionMenu()">
            <span><Save :size="13" /></span>
            <span>另存为...</span>
          </button>
    ...
    <div v-else class="editor-host-shell">
      ...
      <div class="editor-code-region">
        <div
          ...
          :class="{ collapsed: yamlTopLevelPanelCollapsed }"
          :style="yamlTopLevelPanelStyle"
          @pointerdown="startYamlTopLevelPanelDrag"
          @click.capture="handleYamlTopLevelPanelClickCapture"
          @wheel.stop
        >
          <button
            type="button"
            class="yaml-top-level-drag-handle"
            :title="yamlTopLevelPanelHandleTitle"
            :aria-label="yamlTopLevelPanelHandleTitle"
            :aria-expanded="!yamlTopLevelPanelCollapsed"
            @click="handleYamlTopLevelPanelHandleClick"
          >
            <GripHorizontal :size="15" />
          </button>
          <div v-if="!yamlTopLevelPanelCollapsed" class="yaml-top-level-list">
            <button
