...
describe('NewConversationDialog quick phrases', () => {
  ...
  it('hides disabled providers and selects a visible provider without removing live sessions', async () => {
    ...
    await store.setCliProviderVisible('kimi', false)
    await nextTick()
    expect(providerNames().join(' ')).not.toContain('Kimi')
    store.terminalSessions = [{ id: 'live-kimi', title: 'Kimi', providerKind: 'kimi', cwd: 'D:/Project' }]
    await store.setCliProviderVisible('kimi', true)
    await nextTick()
    expect(providerNames().join(' ')).toContain('Kimi')
    store.settings.hiddenCliProviderIds = ['claude', 'codex', 'grok', 'gemini', 'opencode']
    await nextTick()
    expect(root.querySelectorAll('.new-session-provider')).toHaveLength(1)
    expect(root.querySelector('.new-session-provider.active')?.textContent).toContain('Kimi')
    await store.setCliProviderVisible('kimi', false)
    await nextTick()
    expect(root.querySelectorAll('.new-session-provider')).toHaveLength(0)
    expect(store.terminalSessions.map((session) => session.id)).toEqual(['live-kimi'])
    app.unmount()
  })
