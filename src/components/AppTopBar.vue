...
const dialogueMapTitle = computed(() => (
  store.settings.dialogueMapMode ? '关闭对话管理' : '打开对话管理'
))
...
watch(selectedSearchIndex, async () => {
  await nextTick()
  searchRoot.value?.querySelector('.search-result-row.active')?.scrollIntoView({ block: 'nearest' })
})
...
function onSearchFocus() {
  ...
  if (store.searchQuery.trim() && !isSymbolMode.value && !store.searchResults && !store.searchLoading) {
    void store.searchProject(parsedSearchQuery.value.fileQuery, store.searchQuery, { fileNameOnly: true })
  }
}
...
function onSearchInput(event: Event) {
  ...
  store.searchOpen = true
  const backendQuery = isSymbolMode.value ? '' : parsedSearchQuery.value.fileQuery
...
function toggleProjectEditor() {
  store.setWorkspaceSurface(store.activeWorkspaceSurface === 'editor' ? 'project' : 'editor')
}
...
function focusSearch(value?: string, selectText = true) {
  ...
  if (typeof value === 'string') {
    void store.searchProject(value.trim().startsWith('@') ? '' : parseNavigationQuery(value).fileQuery, value, { fileNameOnly: true })
  }
  store.searchOpen = true
  void nextTick(() => {
...
function onSearchKeydown(event: KeyboardEvent) {
  ...
  const count = store.searchLoading ? 0 : searchItems.value.length
  ...
  if (event.key === 'Enter') {
    event.preventDefault()
    if (count) activateSearchItem(searchItems.value[selectedSearchIndex.value] ?? searchItems.value[0])
    return
  }
  if (event.key === 'Escape') {
    event.preventDefault()
    ...
    <div class="app-menu-row" data-tauri-drag-region>
      ...
      <div class="window-controls">
        ...
        <button
          ...
          type="button"
          :title="dialogueMapTitle"
          :aria-label="dialogueMapTitle"
    ...
    <div class="app-command-row">
      ...
      <div ref="searchRoot" class="search-slot command-search">
        ...
        <input
          ...
          placeholder="搜索文件名或路径（:行号，@符号）"
          @input="onSearchInput"
          @compositionend="onSearchInput"
          @compositionstart="store.cancelSearch()"
          @focus="onSearchFocus"
      ...
      <div class="preview-mode-strip">
        <button
          ...
          type="button"
          :title="store.activeWorkspaceSurface === 'editor' ? '返回工作区' : '项目编辑器预览'"
          :aria-pressed="store.activeWorkspaceSurface === 'editor'"
        ...
        <button
          class="mode-button"
          v-if="store.itemLibraryAvailable"
          :class="{ active: store.itemSearchDialogOpen }"
          type="button"
          title="物品搜索"
          @click="store.setItemSearchDialogOpen(!store.itemSearchDialogOpen)"
        >
          <FileSearch class="mode-button-icon" :size="13" />
          <span>物品搜索</span>
        </button>
      </div>
    </div>
