...
function isExited(session: TerminalSession): boolean {
  return !!session.restoredFromDisk || store.terminalExitCodes[session.id] != null
}
...
async function resumeConversation(sessionId: string) {
  ...
  busyId.value = sessionId
  try {
    await store.resumeExitedCliSession(sessionId)
  } finally {
    busyId.value = null
  }
}
...
function onWindowKeydown(event: KeyboardEvent) {
  ...
  const currentIndex = sessions.value.findIndex((session) => session.id === store.activeCliSessionId)
  if (!sessions.value.length) return

  const nextIndex = currentIndex < 0
    ? event.key === 'ArrowUp' ? sessions.value.length - 1 : 0
    : Math.max(0, Math.min(
  ...
  event.preventDefault()
  store.setActiveTerminalSession(sessions.value[nextIndex].id)
}
    ...
    <div class="conversation-list">
      ...
      >
        ...
        <div class="conversation-card-actions">
          ...
          <template v-else>
            <button
              v-if="isExited(session)"
              type="button"
              class="conversation-icon-button resume"
              title="恢复对话"
              :disabled="busyId === session.id"
              @click.stop="resumeConversation(session.id)"
            >
              <LoaderCircle v-if="busyId === session.id" class="conversation-resume-spinner" :size="15" />
...
.conversation-icon-button.resume {
  color: var(--color-accent-blue);
}

.conversation-resume-spinner {
  animation: conversation-resume-spin 0.8s linear infinite;
}

@keyframes conversation-resume-spin {
