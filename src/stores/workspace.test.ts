...
import type { CliNativeConversationSummary, DirectoryListing, EditorTab, MinecraftClientConfig, PlanProposal, ProjectMinecraftCapabilities, Workspace } from '@/types'
...
const mocks = vi.hoisted(() => ({
  ...
  getProjectMinecraftCapabilities: vi.fn(async (): Promise<ProjectMinecraftCapabilities> => ({ itemSources: [], monsterLibrary: false, dragonCore: false })),
  getMinecraftClientConfig: vi.fn(),
  openDragonCoreGui: vi.fn(),
}))
...
vi.mock('@/lib/tauri', () => ({
  backend: {
    ...
    getProjectMinecraftCapabilities: mocks.getProjectMinecraftCapabilities,
    getMinecraftClientConfig: mocks.getMinecraftClientConfig,
    openDragonCoreGui: mocks.openDragonCoreGui,
  },
...
describe('mobile-created terminal selection', () => {
  it('selects the new CLI session on desktop without duplicating replayed events', () => {
    setActivePinia(createPinia())
    mocks.isTauri.mockReturnValue(true)
    mocks.listen.mockClear()
    const store = useWorkspaceStore()
    vi.spyOn(store, 'schedulePersistTerminalState').mockImplementation(() => {})
    const fourth = { id: 'fourth', title: 'Codex', cwd: 'D:/project', providerKind: 'codex' }
    const fifth = { ...fourth, id: 'fifth' }
    store.terminalSessions = [fourth]
    store.setActiveTerminalSession(fourth.id)
    store.bindBackendEvents()
    const handler = mocks.listen.mock.calls.find(([name]) => name === 'mobile-terminal-created')![1]
    handler({ payload: fifth })
    expect(store.activeCliSessionId).toBe('fifth')
    expect(store.activeTerminalSessionId).toBe('fifth')
    expect(store.terminalDrafts.fifth).toBe('')
    store.setActiveTerminalSession(fourth.id)
    handler({ payload: fifth })
    expect(store.terminalSessions).toHaveLength(2)
    expect(store.activeCliSessionId).toBe('fourth')
...
function minecraftConfig(projectPath = 'D:/One'): MinecraftClientConfig {
  return {
    projectPath,
    configPath: `${projectPath}/.superhigh/minecraft-client.json`,
    displayName: 'Test Minecraft',
    clientRoot: `${projectPath}/client`,
    launcherPath: `${projectPath}/client/launcher.exe`,
...
describe('Minecraft client workspace state', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    mocks.getMinecraftClientConfig.mockReset()
    mocks.openDragonCoreGui.mockReset()
  })

  it('hides the Minecraft feature when the workspace has no config', async () => {
    mocks.getMinecraftClientConfig.mockResolvedValue(null)
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')

    await store.loadMinecraftClientConfig()

    expect(store.minecraftClientAvailable).toBe(false)
    expect(store.minecraftClientConfigError).toBe('')
  })

  it('enables the Minecraft feature when the workspace has a valid config', async () => {
    mocks.getMinecraftClientConfig.mockResolvedValue(minecraftConfig())
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')

    await store.loadMinecraftClientConfig()

    expect(store.minecraftClientAvailable).toBe(true)
    expect(store.minecraftClientConfig).toEqual(minecraftConfig())
  })
})
...
describe('workspace dirty tab handling', () => {
  beforeEach(() => {
    ...
    mocks.openDragonCoreGui.mockResolvedValue({
      playerName: 'Agent_1',
      guiName: 'menu',
      filePath: 'D:/One/DragonCore/Gui/menu.yml',
      command: 'dc open menu',
      output: 'ok',
    })
  ...
  it('defaults item search phrases when loading older settings', async () => {
    const store = useWorkspaceStore()
    mocks.getSettings.mockResolvedValue({
      themeId: 'dark',
    ...
    expect(store.settings.itemSearchPhrases).toEqual([])
  })

  it('defaults the DragonCore client source path when loading older settings', async () => {
    const store = useWorkspaceStore()
    mocks.getSettings.mockResolvedValue({
      themeId: 'dark',
      panelLayout: { previewWidth: 520 },
    ...
    expect((store.settings as any).dragonCoreClientRootPath).toBe('')
  })
  ...
  it('resumes an exited CLI card through the latest matching native conversation and replaces it', async () => {
    mocks.isTauri.mockReturnValue(true)
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')
    ...
    store.activeCliSessionId = 'session-old'
    store.terminalExitCodes['session-old'] = 0
    store.terminalConversation['session-old'] = [{
      id: 'message-1',
    ...
    const resumed = await store.resumeExitedCliSession('session-old')

    expect(mocks.resumeCliConversation).toHaveBeenCalledWith('D:/One', 'codex', 'D:/One', 'native-session-1')
    expect(resumed?.id).toBe('resumed-session-1')
    expect(resumed?.name).toBe('修复对话地图')
    expect(store.terminalSessions.map((session) => session.id)).toEqual(['resumed-session-1'])
    expect(store.terminalConversation['resumed-session-1'][0]).toEqual(expect.objectContaining({
      sessionId: 'resumed-session-1',
      content: '修复对话地图',
    }))
  })

  it('saves the DragonCore client source path for later app starts', async () => {
    const store = useWorkspaceStore()

    expect(typeof (store as any).setDragonCoreClientRootPath).toBe('function')
    await (store as any).setDragonCoreClientRootPath('  D:/Client/resourcepacks/DragonCore  ')

    expect((store.settings as any).dragonCoreClientRootPath).toBe('D:/Client/resourcepacks/DragonCore')
    expect(mocks.saveSettings).toHaveBeenLastCalledWith(expect.objectContaining({
      dragonCoreClientRootPath: 'D:/Client/resourcepacks/DragonCore',
    }))
  })

  it('saves item search phrases when adding updating and deleting rules', async () => {
    const store = useWorkspaceStore()

    await store.addItemSearchPhrase('1c', '哥布林')
    const phrase = store.settings.itemSearchPhrases[0]

    expect(phrase).toMatchObject({ phrase: '1c', value: '哥布林' })
    expect(phrase.history).toEqual([expect.objectContaining({
      action: 'created',
      phrase: '1c',
      value: '哥布林',
    })])
    expect(mocks.saveSettings).toHaveBeenLastCalledWith(expect.objectContaining({
      itemSearchPhrases: [expect.objectContaining({ phrase: '1c', value: '哥布林' })],
    }))

    await store.updateItemSearchPhrase(phrase.id, { phrase: '2c', value: '樱骸' })

    expect(store.settings.itemSearchPhrases[0]).toMatchObject({ id: phrase.id, phrase: '2c', value: '樱骸' })
    expect(store.settings.itemSearchPhrases[0].createdAt).toBe(phrase.createdAt)
    expect(store.settings.itemSearchPhrases[0].history).toEqual([
      expect.objectContaining({ action: 'created', phrase: '1c', value: '哥布林' }),
    ...
    await store.deleteItemSearchPhrase(phrase.id)
...
describe('DragonCore tools workspace state', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    mocks.openProject.mockReset()
    mocks.readFile.mockReset()
  ...
  it('opens only the requested GUI file without scanning the workspace', async () => {
    mocks.openProject.mockResolvedValue(workspace('D:/One'))
    mocks.readFile.mockResolvedValue('Title: menu')
    const store = useWorkspaceStore()

    const opened = await store.openDragonCoreToolsFile('D:/One', 'D:/One/DragonCore/Gui/menu.yml')

    expect(opened).toBe(true)
    expect(mocks.openProject).toHaveBeenCalledWith('D:/One')
    expect(mocks.readFile).toHaveBeenCalledWith('D:/One/DragonCore/Gui/menu.yml')
...
describe('workspace file refresh and scoped state performance', () => {
  ...
  it('refreshes the requested explorer root and its expanded children', async () => {
    ...
    store.expandedPaths = [
      ...
      'D:/One/src',
      'D:/One/DragonCore',
      'D:/One/DragonCore/cards',
    ]

    await store.refreshExplorer('D:/One/DragonCore')

    expect(mocks.listDirectory).toHaveBeenCalledTimes(2)
    expect(mocks.listDirectory).toHaveBeenNthCalledWith(1, 'D:/One/DragonCore')
    expect(mocks.listDirectory).toHaveBeenNthCalledWith(2, 'D:/One/DragonCore/cards')
  })
  ...
  it('ignores a delayed watcher event for the last confirmed DragonCore write', async () => {
    const store = useWorkspaceStore()
    const tab = textTab('dragoncore', 'D:/One/DragonCore/Gui/menu.yml', true, 'next draft')
    tab.lastKnownDiskContent = 'saved by DragonCore'
    store.tabs = [tab]
    store.fileConflictPaths['d:/one/dragoncore/gui/menu.yml'] = true
    mocks.readFile.mockResolvedValueOnce('saved by DragonCore')

    await store.reloadOpenTabsForChangedPaths(
      store.tabs,
      new Set(['d:/one/dragoncore/gui/menu.yml']),
    )

    expect(tab.content).toBe('next draft')
    expect(tab.isDirty).toBe(true)
    expect(store.fileConflictPaths['d:/one/dragoncore/gui/menu.yml']).toBeUndefined()
  })

  it('keeps a dirty draft and reports a real external file change', async () => {
    const store = useWorkspaceStore()
    const tab = textTab('dragoncore', 'D:/One/DragonCore/Gui/menu.yml', true, 'local draft')
    ...
    expect(tab.isDirty).toBe(true)
    expect(store.fileConflictPaths['d:/one/dragoncore/gui/menu.yml']).toBe(true)
  })

  it('records clean DragonCore preview content and clears mixed-case conflicts', () => {
    const store = useWorkspaceStore()
    const tab = textTab('dragoncore', 'D:/One/DragonCore/Gui/Menu.yml', true, 'old')
    store.tabs = [tab]
    store.fileConflictPaths['d:/one/dragoncore/gui/menu.yml'] = true

    const updated = store.setScopedCodePreviewContent(
      'd:/one/dragoncore/gui/menu.yml',
      'saved by DragonCore',
      { isDirty: false },
    )

    expect(updated).toBe(true)
    expect(tab.content).toBe('saved by DragonCore')
    expect(tab.isDirty).toBe(false)
    expect(tab.lastKnownDiskContent).toBe('saved by DragonCore')
    expect(store.fileConflictPaths['d:/one/dragoncore/gui/menu.yml']).toBeUndefined()
  })
  ...
  it('opens DragonCore mode immediately and sends RCON after the reload delay', async () => {
    mocks.isTauri.mockReturnValue(true)
    const store = useWorkspaceStore()
    store.minecraftCapabilities = { itemSources: ['ni', 'mm'], monsterLibrary: true, dragonCore: true }
    mocks.getProjectMinecraftCapabilities.mockResolvedValueOnce(store.minecraftCapabilities)
    store.workspace = workspace('D:/One')
    store.openWorkspacePaths = ['D:/One']
    store.settings.activePreviewMode = 'local_terminal'
    store.directoryCache = {
      'D:/One/DragonCore/Gui': { path: 'D:/One/DragonCore/Gui', entries: [] },
    }
    ...
    store.scheduleWorkspaceFilesChangedRefresh({
      ...
      changedPaths: [
        'D:\\One\\DragonCore\\Gui\\menu.yml',
        'D:/One/DragonCore/Gui/menu.yml',
        'D:/One/DragonCore/Gui/sub/panel.yaml',
      ],
      affectedDirectories: ['D:/One/DragonCore/Gui'],
      kind: 'modify',
      sequence: 8,
    })

    expect(store.dragonCoreModeOpen).toBe(true)
    expect(store.settings.activePreviewMode).toBe('code')
    expect(mocks.readFile).toHaveBeenCalledWith('D:/One/DragonCore/Gui/menu.yml')
    expect(mocks.readFile).toHaveBeenCalledWith('D:/One/DragonCore/Gui/sub/panel.yaml')
    expect(mocks.openDragonCoreGui).not.toHaveBeenCalled()
    await flushPromises()
    ...
    expect(mocks.openDragonCoreGui).toHaveBeenCalledTimes(2)
    expect(mocks.openDragonCoreGui).toHaveBeenCalledWith('D:/One', 'D:/One/DragonCore/Gui/menu.yml')
    expect(mocks.openDragonCoreGui).toHaveBeenCalledWith('D:/One', 'D:/One/DragonCore/Gui/sub/panel.yaml')
    expect(mocks.listDirectory).toHaveBeenCalled()
  })

  it('does not send DragonCore RCON when a Gui yaml file is removed', async () => {
    mocks.isTauri.mockReturnValue(true)
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')
    store.openWorkspacePaths = ['D:/One']
    store.settings.activePreviewMode = 'local_terminal'
    mocks.openDragonCoreGui.mockClear()
    mocks.readFile.mockClear()

    store.scheduleWorkspaceFilesChangedRefresh({
      rootPath: 'D:/One',
      changedPaths: ['D:/One/DragonCore/Gui/menu.yml'],
      affectedDirectories: ['D:/One/DragonCore/Gui'],
      kind: 'remove',
      sequence: 10,
    })
    ...
    expect(store.dragonCoreModeOpen).toBe(false)
    expect(store.settings.activePreviewMode).toBe('local_terminal')
    expect(mocks.readFile).not.toHaveBeenCalled()
    expect(mocks.openDragonCoreGui).not.toHaveBeenCalled()
  })

  it('ignores changed files outside DragonCore Gui yaml files for DragonCore mode', async () => {
    mocks.isTauri.mockReturnValue(true)
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')
    store.openWorkspacePaths = ['D:/One']
    ...
    store.scheduleWorkspaceFilesChangedRefresh({
      rootPath: 'D:/One',
      changedPaths: [
        'D:/One/DragonCore/ItemIcon.yml',
        'D:/One/DragonCore/Gui/menu.png',
        'D:/One/src/menu.yml',
      ],
      affectedDirectories: ['D:/One/DragonCore/Gui'],
      kind: 'modify',
  ...
  it('includes DragonCore Gui in workspace watch targets', () => {
    const store = useWorkspaceStore()

    const target = store.workspaceWatchTargetFor('D:/One', ['D:/One/src'], [])

    expect(target?.directories).toContain('D:/One/DragonCore/Gui')
  })
  ...
  it('drops stale project-managed terminal sessions instead of restoring dead service tabs', async () => {
    ...
    mocks.readFile.mockResolvedValue(JSON.stringify({
      version: 1,
      savedAt: '2026-05-09T00:00:00Z',
      terminalSessions: [{ id: 'server-old', title: '示例项目目录名服务端', providerKind: 'project-service', cwd: 'D:/Server' }],
  ...
  it('reattaches live project service sessions and rebuilds the service run state', async () => {
    ...
    mocks.readFile.mockResolvedValue(JSON.stringify({
      version: 1,
      savedAt: '2026-05-09T00:00:00Z',
      terminalSessions: [{ id: 'server-live', title: '示例项目目录名服务端', providerKind: 'project-service', cwd: 'D:/Server' }],
    ...
    store.projectStartupConfig = {
      ...
      mode: 'services',
      services: [
        { id: 'server', name: '示例项目目录名服务端', scriptPath: 'D:/Server/start.bat', args: [], workingDirectory: 'D:/Server', startAfter: [] },
      ],
    }
  ...
  it('opens SuperHigh editor HTML as a regular code-preview tab', async () => {
    const store = useWorkspaceStore()
    const path = 'D:/One/.superhigh/editor/items.html'
    store.workspace = workspace('D:/One')
    store.settings.activePreviewMode = 'code'
    mocks.readFile.mockResolvedValue('<main>Items</main>')

    await expect(store.openFile(path, { revealInExplorer: false })).resolves.toBe(true)

    expect(store.activeTab).toMatchObject({ path, content: '<main>Items</main>', contentType: 'text', language: 'html' })
    expect(store.activeWorkspaceSurface).toBe('project')
    expect(store.tabs).toHaveLength(1)
    await store.openFile(path, { revealInExplorer: false })
    expect(store.tabs).toHaveLength(1)
    expect(store.activeTab?.path).toBe(path)
  })

  it('refreshes an open SuperHigh editor HTML tab after an external save', async () => {
    const store = useWorkspaceStore()
    const path = 'D:/One/.superhigh/editor/items.html'
    store.workspace = workspace('D:/One')
    store.openWorkspacePaths = ['D:/One']
    mocks.readFile.mockResolvedValueOnce('<main>Before</main>').mockResolvedValueOnce('<main>After</main>')
    await store.openFile(path, { revealInExplorer: false })

    store.scheduleWorkspaceFilesChangedRefresh({
      rootPath: 'D:/One',
      changedPaths: [path],
      affectedDirectories: ['D:/One/.superhigh/editor'],
      kind: 'modify',
      sequence: 1,
    })
    await vi.advanceTimersByTimeAsync(130)

    expect(store.activeTab?.content).toBe('<main>After</main>')
    expect(store.activeTab?.lastKnownDiskContent).toBe('<main>After</main>')
  })
...
describe('managed terminal replacement cleanup', () => {
  ...
  it.each(['project-startup', 'project-service', 'minecraft-client'])(
...
describe('Minecraft capability visibility', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    mocks.getProjectMinecraftCapabilities.mockReset()
  })

  it('keeps item search hidden without matching sources and closes unavailable tools', async () => {
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')
    store.setItemSearchDialogOpen(true)
    expect(store.itemSearchDialogOpen).toBe(false)
    store.minecraftCapabilities = { itemSources: ['ni'], monsterLibrary: false, dragonCore: true }
    store.setItemSearchDialogOpen(true)
    store.openDragonCoreMode()
    mocks.getProjectMinecraftCapabilities.mockResolvedValue({ itemSources: [], monsterLibrary: false, dragonCore: false })
    await store.loadMinecraftCapabilities()
    expect(store.itemLibraryAvailable).toBe(false)
    expect(store.itemSearchDialogOpen).toBe(false)
    expect(store.dragonCoreModeOpen).toBe(false)
  })

  it('clears visibility when the last workspace is closed', async () => {
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')
    store.openWorkspacePaths = ['D:/One']
    store.minecraftCapabilities = { itemSources: ['ni'], monsterLibrary: false, dragonCore: true }
    store.setItemSearchDialogOpen(true)
    store.openDragonCoreMode()
    await store.closeWorkspaceTab('D:/One')
    expect(store.workspace).toBeNull()
    expect(store.minecraftCapabilities).toBeNull()
    expect(store.itemLibraryAvailable).toBe(false)
    expect(store.dragonCoreAvailable).toBe(false)
    expect(store.itemSearchDialogOpen).toBe(false)
    expect(store.dragonCoreModeOpen).toBe(false)
  })

  it.each(['ni', 'mm'] as const)('shows item search for %s independently', async (source) => {
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')
    mocks.getProjectMinecraftCapabilities.mockResolvedValue({ itemSources: [source], monsterLibrary: false, dragonCore: false })
    await store.loadMinecraftCapabilities()
    store.setItemSearchDialogOpen(true)
    expect(store.itemLibraryAvailable).toBe(true)
    expect(store.itemSearchDialogOpen).toBe(true)
  })

  it('ignores a capability response from the previous workspace', async () => {
    const store = useWorkspaceStore()
    store.workspace = workspace('D:/One')
    let resolve!: (value: { itemSources: ['ni']; monsterLibrary: boolean; dragonCore: boolean }) => void
    mocks.getProjectMinecraftCapabilities.mockReturnValue(new Promise((done) => { resolve = done }))
    const pending = store.loadMinecraftCapabilities()
    store.restoreWorkspaceScopedState(workspace('D:/Two'))
    resolve({ itemSources: ['ni'], monsterLibrary: true, dragonCore: true })
    await pending
    expect(store.minecraftCapabilities).toBeNull()
    expect(store.itemLibraryAvailable).toBe(false)
  })
