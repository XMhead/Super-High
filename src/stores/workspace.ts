...
import { CLI_PROVIDER_OPTIONS, cliProviderVisible } from '@/lib/cliProviders'
...
interface WorkspaceScopedState {
  ...
  minecraftClientConfig?: MinecraftClientConfig | null
  minecraftClientConfigLoaded?: boolean
  minecraftClientConfigError?: string
  minecraftClientStatus?: MinecraftClientStatus | null
  dragonCliSessionIds: string[]
  activeDragonCliSessionId: string | null
}
...
function normalizeItemSearchPhrases(rules: ItemSearchPhraseRule[] = []): ItemSearchPhraseRule[] {
...
// 终端动作等待服务端退出或客户端窗口释放时使用，保持调用链可读。
...
function isDragonCoreGuiYamlPath(path: string, rootPath: string): boolean {
  const normalizedPath = normalizeFilePath(path)
  const normalizedRoot = normalizeFilePath(rootPath).replace(/\/+$/, '')
  const guiRoot = `${normalizedRoot}/DragonCore/Gui`
  const lowerPath = normalizedPath.toLowerCase()
  return (
    lowerPath.startsWith(`${guiRoot.toLowerCase()}/`) &&
...
const DRAGONCORE_GUI_OPEN_DELAY_MS = 3000
...
function isProjectManagedTerminalSessionKind(kind: string): boolean {
  return kind === 'project-startup' || kind === 'project-service' || kind === 'minecraft-client'
}
...
function buildPersistedTerminalStateFromSource(source: TerminalStateSource): PersistedTerminalState {
  ...
  const activeLocalTerminalSessionId = source.activeLocalTerminalSessionId && sessionIds.has(source.activeLocalTerminalSessionId)
    ...
    : null
  const activeDragonCliSessionId = source.activeDragonCliSessionId && sessionIds.has(source.activeDragonCliSessionId)
    ? source.activeDragonCliSessionId
    : null
  ...
  return {
    ...
    activeDragonCliSessionId,
    dragonCliSessionIds: source.dragonCliSessionIds.filter((id) => sessionIds.has(id)),
    terminalDrafts,
    terminalConversation,
    terminalExitCodes,
  }
...
export const useWorkspaceStore = defineStore('workspace', {
  state: () => ({
    ...
    minecraftClientConfig: null as MinecraftClientConfig | null,
    minecraftClientConfigLoaded: false,
    minecraftClientConfigError: '',
    minecraftClientStatus: null as MinecraftClientStatus | null,
    minecraftClientBusy: false,
    minecraftCapabilities: null as ProjectMinecraftCapabilities | null,
    minecraftCapabilitiesRequest: 0,
    dragonCoreModeOpen: false,
    ...
    activeDragonCliSessionId: null as string | null,
    dragonCliSessionIds: [] as string[],
  ...
  getters: {
    ...
    cliTerminalSessions(state) {
      const dragonIds = new Set(state.dragonCliSessionIds)
      return state.terminalSessions.filter((session) => !isLocalTerminalSessionKind(session.providerKind) && !dragonIds.has(session.id))
    },
    dragonCliTerminalSessions(state) {
      const dragonIds = new Set(state.dragonCliSessionIds)
      return state.terminalSessions.filter((session) => !isLocalTerminalSessionKind(session.providerKind) && dragonIds.has(session.id))
    },
    ...
    activeDragonCliTerminalSession(state) {
      return state.terminalSessions.find((session) => session.id === state.activeDragonCliSessionId && !isLocalTerminalSessionKind(session.providerKind)) ?? null
    },
    ...
    itemLibraryAvailable(state): boolean {
      return !!state.workspace && !!state.minecraftCapabilities?.itemSources.length
    },
    dragonCoreAvailable(state): boolean {
      return !!state.workspace && state.minecraftCapabilities?.dragonCore === true
    },
    minecraftClientAvailable(state): boolean {
      return !!state.minecraftClientConfig || !!state.minecraftClientConfigError
    },
  ...
  actions: {
    ...
    async loadMinecraftCapabilities(rootPath?: string) {
      const targetRoot = rootPath ?? this.workspace?.rootPath
      if (!targetRoot || !this.workspace || workspaceKey(targetRoot) !== workspaceKey(this.workspace.rootPath)) return
      const request = ++this.minecraftCapabilitiesRequest
      try {
        const capabilities = await backend.getProjectMinecraftCapabilities(targetRoot)
        if (request !== this.minecraftCapabilitiesRequest || !this.workspace || workspaceKey(targetRoot) !== workspaceKey(this.workspace.rootPath)) return
        this.minecraftCapabilities = capabilities
      } catch {
        if (request !== this.minecraftCapabilitiesRequest) return
        this.minecraftCapabilities = null
      }
      if (!this.itemLibraryAvailable) this.itemSearchDialogOpen = false
      if (!this.dragonCoreAvailable) this.dragonCoreModeOpen = false
    },

    async loadMinecraftClientConfig(rootPath?: string) {
      const targetRoot = rootPath ?? this.workspace?.rootPath
      if (!targetRoot) return
      this.minecraftClientConfigLoaded = false
      this.minecraftClientConfig = null
      this.minecraftClientConfigError = ''
      this.minecraftClientStatus = null
      try {
        const config = await backend.getMinecraftClientConfig(targetRoot)
        if (!this.workspace || workspaceKey(this.workspace.rootPath) !== workspaceKey(targetRoot)) return
        this.minecraftClientConfig = config
      } catch (error) {
        if (!this.workspace || workspaceKey(this.workspace.rootPath) !== workspaceKey(targetRoot)) return
        this.minecraftClientConfigError = formatError(error)
      } finally {
        if (this.workspace && workspaceKey(this.workspace.rootPath) === workspaceKey(targetRoot)) {
          this.minecraftClientConfigLoaded = true
        }
      }
    },

    async startMinecraftClient() {
      if (!this.workspace) throw new Error('请先打开项目。')
      const rootPath = this.workspace.rootPath
      const workspaceId = this.workspace.id
      const existing = this.findMinecraftClientTerminalSession()
      if (existing) {
        this.setActiveTerminalSession(existing.id)
        this.minecraftClientStatus = await backend.minecraftClientStatus(rootPath)
        return this.minecraftClientStatus
      }
      this.minecraftClientBusy = true
      try {
        const result = await backend.startMinecraftClient(workspaceId, rootPath)
        if (this.workspace?.id !== workspaceId || workspaceKey(this.workspace?.rootPath ?? '') !== workspaceKey(rootPath)) {
          if (result.session) void backend.closeTerminal(result.session.id)
          return result.status
        }
        this.minecraftClientStatus = result.status
        if (result.session) this.registerMinecraftClientTerminalSession(result.session)
        return this.minecraftClientStatus
      } finally {
        this.minecraftClientBusy = false
      }
    },

    findMinecraftClientTerminalSession(): TerminalSession | null {
      const clientRoot = this.minecraftClientConfig?.clientRoot
      return this.terminalSessions.find((session) => (
        session.providerKind === 'minecraft-client'
        && terminalSessionIsLive(session)
        && this.terminalExitCodes[session.id] == null
        && (clientRoot ? workspaceKey(session.cwd) === workspaceKey(clientRoot) : true)
      )) ?? null
    },
    ...
    registerMinecraftClientTerminalSession(session: TerminalSession) {
      this.closeExitedManagedTerminalSessions(session)
      if (!this.terminalSessions.some((item) => item.id === session.id)) {
    ...
    async stopMinecraftClient() {
      if (!this.workspace) throw new Error('请先打开项目。')
      this.minecraftClientBusy = true
      try {
        const session = this.findMinecraftClientTerminalSession()
        this.minecraftClientStatus = await backend.stopMinecraftClient(this.workspace.rootPath)
        if (session) await this.closeTerminalSession(session.id)
        return this.minecraftClientStatus
      } finally {
        this.minecraftClientBusy = false
      }
    ...
    async restartMinecraftClient() {
      await this.stopMinecraftClient()
      await delay(800)
      return this.startMinecraftClient()
    },

    async refreshMinecraftClientStatus() {
      if (!this.workspace) throw new Error('请先打开项目。')
      this.minecraftClientStatus = await backend.minecraftClientStatus(this.workspace.rootPath)
      return this.minecraftClientStatus
    },

    async embedMinecraftClient(rect: MinecraftEmbedRect) {
      if (!this.workspace) throw new Error('请先打开项目。')
      this.minecraftClientBusy = true
      try {
        this.minecraftClientStatus = await backend.embedMinecraftClient(this.workspace.rootPath, rect)
      } finally {
        this.minecraftClientBusy = false
      }
    },

    async resizeMinecraftClient(rect: MinecraftEmbedRect) {
      if (!this.workspace || !this.minecraftClientStatus?.embedded) return
      this.minecraftClientStatus = await backend.resizeMinecraftClient(this.workspace.rootPath, rect)
    },

    async hideMinecraftClient() {
      const status = await backend.hideMinecraftClient()
      if (this.minecraftClientStatus?.embedded) this.minecraftClientStatus = status
    },

    async showMinecraftClient() {
      if (!this.workspace) throw new Error('请先打开项目。')
      this.minecraftClientBusy = true
      try {
        this.minecraftClientStatus = await backend.showMinecraftClient(this.workspace.rootPath)
      } finally {
        this.minecraftClientBusy = false
      }
    },

    async releaseMinecraftClient() {
      if (!this.workspace) throw new Error('请先打开项目。')
      this.minecraftClientBusy = true
      try {
        this.minecraftClientStatus = await backend.releaseMinecraftClient(this.workspace.rootPath)
      } finally {
        this.minecraftClientBusy = false
      }
    },

    async releaseAllMinecraftClients() {
      await backend.releaseAllMinecraftClients()
      if (this.minecraftClientStatus?.embedded) {
        this.minecraftClientStatus = {
          ...this.minecraftClientStatus,
          state: 'released',
          message: 'Minecraft game window released',
          embedded: false,
        }
      }
    },
    ...
    stashActiveWorkspaceState() {
      ...
      this.workspaceSnapshots[workspaceKey(this.workspace.rootPath)] = {
        ...
        dragonCliSessionIds: this.dragonCliSessionIds,
        activeDragonCliSessionId: this.activeDragonCliSessionId,
        activeWorkspaceSurface: this.activeWorkspaceSurface,
        projectEditorEntryPath: this.projectEditorEntryPath,
        minecraftClientConfig: this.minecraftClientConfig,
        minecraftClientConfigLoaded: this.minecraftClientConfigLoaded,
        minecraftClientConfigError: this.minecraftClientConfigError,
        minecraftClientStatus: this.minecraftClientStatus,
      }
    },

    resetWorkspaceScopedState(rootPath: string) {
      this.resetMinecraftCapabilities()
      this.$patch({
        directoryCache: {},
        ...
        minecraftClientConfig: null,
        minecraftClientConfigLoaded: false,
        minecraftClientConfigError: '',
        minecraftClientStatus: null,
        minecraftClientBusy: false,
      })
    },

    resetMinecraftCapabilities() {
      this.minecraftCapabilitiesRequest++
      this.minecraftCapabilities = null
      this.itemSearchDialogOpen = false
      this.dragonCoreModeOpen = false
    },
    ...
    restoreWorkspaceScopedState(workspace: Workspace) {
      ...
      let activeDragonCliSessionId = snapshot.activeDragonCliSessionId ?? null
      const terminalSessions = snapshot.terminalSessions
      const liveTerminalSessions = terminalSessions.filter(terminalSessionIsLive)
      const dragonCliSessionIds = snapshot.dragonCliSessionIds ?? []
      const dragonIdSet = new Set(dragonCliSessionIds)
      const cliTerminalSessions = terminalSessions.filter((s) => !isLocalTerminalSessionKind(s.providerKind) && !dragonIdSet.has(s.id))
      const dragonCliTerminalSessions = terminalSessions.filter((s) => !isLocalTerminalSessionKind(s.providerKind) && dragonIdSet.has(s.id))
      ...
      if (activeDragonCliSessionId && !dragonCliTerminalSessions.some((s) => s.id === activeDragonCliSessionId)) {
        activeDragonCliSessionId = dragonCliTerminalSessions[0]?.id ?? null
      }
      ...
      this.$patch({
        ...
        codePreviewProjectEditorOpen: false,
        minecraftClientConfig: snapshot.minecraftClientConfig ?? null,
        minecraftClientConfigLoaded: snapshot.minecraftClientConfigLoaded ?? false,
        minecraftClientConfigError: snapshot.minecraftClientConfigError ?? '',
        minecraftClientStatus: snapshot.minecraftClientStatus ?? null,
        minecraftClientBusy: false,
      })
    },
    ...
    async switchWorkspace(rootPath: string) {
      ...
      void this.syncWorkspaceWatchTargets()
      void this.loadMinecraftCapabilities(rootPath)
      void this.loadMinecraftClientConfig(rootPath)
    ...
    async closeWorkspaceTab(rootPath: string, choice: 'save' | 'discard' | null = null) {
      ...
      if (!fallbackPath) {
        ...
        this.dragonCliSessionIds = []
        this.activeDragonCliSessionId = null
        this.activeWorkspaceSurface = 'project'
        this.settingsOpen = false
    ...
    snapshotTerminalStateSource(snapshot: WorkspaceScopedState): TerminalStateSource {
      return {
        ...
        activeDragonCliSessionId: snapshot.activeDragonCliSessionId ?? null,
        dragonCliSessionIds: snapshot.dragonCliSessionIds ?? [],
        terminalDrafts: snapshot.terminalDrafts ?? {},
        terminalConversation: snapshot.terminalConversation ?? {},
        terminalConversationReplyPending: snapshot.terminalConversationReplyPending ?? {},
        terminalExitCodes: snapshot.terminalExitCodes ?? {},
    ...
    ensureSnapshotTerminalStateSource(snapshot: WorkspaceScopedState): TerminalStateSource {
      ...
      snapshot.dragonCliSessionIds ??= []
      snapshot.activeDragonCliSessionId ??= null
    ...
    async loadPersistedTerminalState(rootPath: string) {
      ...
      this.dragonCliSessionIds = Array.from(new Set([
        ...this.dragonCliSessionIds,
        ...((parsed.dragonCliSessionIds ?? []).filter((id) => restoredIds.has(id))),
      ]))
      const dragonIdSet = new Set(this.dragonCliSessionIds)
      const restoredCliSessions = restoredSessions.filter((session) => !isLocalTerminalSessionKind(session.providerKind) && !dragonIdSet.has(session.id))
      const restoredDragonSessions = restoredSessions.filter((session) => !isLocalTerminalSessionKind(session.providerKind) && dragonIdSet.has(session.id))
      ...
      if (!this.activeDragonCliSessionId) {
        const persistedId = parsed.activeDragonCliSessionId
        this.activeDragonCliSessionId = restoredDragonSessions.some((session) => session.id === persistedId)
          ? persistedId ?? null
          : restoredDragonSessions[0]?.id ?? null
      }
    ...
    bindBackendEvents() {
      ...
      void listen('mobile-preferences-updated', () => {
        void this.loadSettings().catch((error) => this.showErrorMessage(`设置同步失败：${formatError(error)}`))
      }).then((unlisten) => {
        if (backendEventListenersDisposed) unlisten()
        else backendEventUnlisteners.push(unlisten)
      }).catch((error) => this.showErrorMessage(`设置监听失败：${formatError(error)}`))
      void listen('mobile-terminal-created', (event) => {
        const session = (event.payload ?? event) as TerminalSession
        if (session?.id && !this.terminalSessions.some((item) => item.id === session.id)) {
          this.terminalSessions.push(session)
          this.terminalDrafts[session.id] = this.terminalDrafts[session.id] ?? ''
          this.terminalConversation[session.id] = this.terminalConversation[session.id] ?? []
          this.terminalConversationReplyPending[session.id] = this.terminalConversationReplyPending[session.id] ?? false
      ...
      }).catch((error) => this.showErrorMessage(`手机会话监听失败：${formatError(error)}`))
    ...
    workspaceWatchTargetFor(rootPath: string, expandedPaths: string[], tabs: EditorTab[]): WorkspaceWatchTarget | null {
      ...
      directories.add(`${normalizedRootPath}/DragonCore/Gui`)
      directories.add(`${normalizedRootPath}/plugins/NeigeItems/Items`)
      directories.add(`${normalizedRootPath}/plugins/MythicMobs/Items`)
    ...
    openDragonCoreMode() {
      this.dragonCoreModeOpen = this.dragonCoreAvailable
    },
    ...
    toggleDragonCoreMode() {
      this.dragonCoreModeOpen = this.dragonCoreAvailable && !this.dragonCoreModeOpen
    },
    ...
    refreshWorkspaceMetadataInBackground(rootPath: string) {
      ...
      this.metadataRefreshTimer = window.setTimeout(() => {
        ...
        void this.loadMinecraftCapabilities(rootPath)
        this.runBackgroundTask('Minecraft 客户端配置刷新', () => this.loadMinecraftClientConfig(rootPath).then(() => undefined))
        this.runBackgroundTask('文档检测', () => this.detectVitePressDocs(rootPath).then(() => undefined))
      }, 1000)
    },
    ...
    scheduleWorkspaceFilesChangedRefresh(event: WorkspaceFilesChangedEvent) {
      ...
      if (this.workspace && workspaceKey(this.workspace.rootPath) === key) void this.loadMinecraftCapabilities(rootPath)
      this.handleDragonCoreGuiChangedFiles(normalizedEvent)
    ...
    handleDragonCoreGuiChangedFiles(event: WorkspaceFilesChangedEvent) {
      const currentRootPath = this.workspace?.rootPath
      if (!currentRootPath || workspaceKey(currentRootPath) !== workspaceKey(event.rootPath)) return
      if (event.kind === 'remove') return
      const seen = new Set<string>()
      for (const path of event.changedPaths) {
        const normalizedPath = normalizeFilePath(path)
        const key = normalizedPath.toLowerCase()
        if (seen.has(key) || !isDragonCoreGuiYamlPath(normalizedPath, currentRootPath)) continue
        seen.add(key)
        this.openDragonCoreMode()
        if (this.settings.activePreviewMode !== 'code') this.setPreviewMode('code')
        void this.openChangedDragonCoreGuiFile(currentRootPath, normalizedPath)
      }
    },

    async initializeDragonCoreTools() {
      if (this.initialized) return
      applyTheme(DEFAULT_THEME_ID)
      if (!isTauri()) {
        this.initialized = true
      ...
      try {
        ...
        this.showErrorMessage(`DragonCore 工具初始化失败：${formatError(error)}`)
      } finally {
        this.initialized = true
      }
    },

    async openChangedDragonCoreGuiFile(rootPath: string, filePath: string) {
      ...
      if (!opened) return
      if (!this.workspace || workspaceKey(this.workspace.rootPath) !== workspaceKey(rootPath)) return
      await new Promise<void>((resolve) => window.setTimeout(resolve, DRAGONCORE_GUI_OPEN_DELAY_MS))
      if (!this.workspace || workspaceKey(this.workspace.rootPath) !== workspaceKey(rootPath)) return
      try {
        await backend.openDragonCoreGui(rootPath, filePath)
      } catch (error) {
        this.showErrorMessage(`DragonCore GUI 打开失败：${formatError(error)}`)
      }
    },
    ...
    async applyWorkspaceFilesChangedEvents(key: string, events: WorkspaceFilesChangedEvent[]) {
      ...
      if (currentKey === key && this.workspace) {
        ...
        const dragonCorePath = `${normalizeFilePath(rootPath)}/DragonCore`
        if ([...allChangedPaths].some((p) => p.startsWith(dragonCorePath.toLowerCase()))) {
          window.dispatchEvent(new CustomEvent('superhigh:dragon-files-changed'))
        }
        return
      }
    ...
    async loadSettings() {
      ...
      this.settings.hiddenCliProviderIds = Array.isArray(this.settings.hiddenCliProviderIds)
        ? [...new Set(this.settings.hiddenCliProviderIds.filter((id) => CLI_PROVIDER_OPTIONS.some((provider) => provider.id === id)))]
        : ['dsh']
      ...
      this.settings.dragonCoreClientRootPath = normalizeDirectorySettingPath(this.settings.dragonCoreClientRootPath ?? '')
      this.settings.itemSearchPhrases = normalizeItemSearchPhrases(this.settings.itemSearchPhrases)
    ...
    async addItemSearchPhrase(phraseText: string, valueText: string): Promise<ItemSearchPhraseRule | null> {
      const phrase = phraseText.trim()
      const value = valueText.trim()
      if (!phrase || !value) return null
      const now = new Date().toISOString()
      const rule: ItemSearchPhraseRule = {
        id: randomId('item-search-phrase'),
        phrase,
        value,
        createdAt: now,
        updatedAt: now,
        history: [{
          id: randomId('item-search-phrase-history'),
          action: 'created',
          phrase,
          value,
          createdAt: now,
      ...
      this.settings.itemSearchPhrases = [rule, ...(this.settings.itemSearchPhrases ?? [])]
      await this.saveSettings()
      return rule
    },

    async updateItemSearchPhrase(
      id: string,
      patch: Partial<Pick<ItemSearchPhraseRule, 'phrase' | 'value'>>,
    ) {
      const phrase = patch.phrase?.trim()
      const value = patch.value?.trim()
      if ((patch.phrase !== undefined && !phrase) || (patch.value !== undefined && !value)) return
      let changed = false
      this.settings.itemSearchPhrases = (this.settings.itemSearchPhrases ?? []).map((rule) => {
        if (rule.id !== id) return rule
        const nextPhrase = phrase !== undefined ? phrase : rule.phrase
        const nextValue = value !== undefined ? value : rule.value
        if (nextPhrase === rule.phrase && nextValue === rule.value) return rule
        changed = true
        const now = new Date().toISOString()
        return {
          ...rule,
    ...
    async deleteItemSearchPhrase(id: string) {
      this.settings.itemSearchPhrases = (this.settings.itemSearchPhrases ?? []).filter((phrase) => phrase.id !== id)
      await this.saveSettings()
    },

    async setDragonCoreClientRootPath(path: string) {
      const next = normalizeDirectorySettingPath(path)
      if (next === this.settings.dragonCoreClientRootPath) return
      this.settings.dragonCoreClientRootPath = next
      await this.saveSettings()
    },

    isCliProviderVisible(providerKind: string) {
      return cliProviderVisible(providerKind, this.settings.hiddenCliProviderIds)
    },

    async setCliProviderVisible(providerKind: string, visible: boolean) {
      if (!CLI_PROVIDER_OPTIONS.some((provider) => provider.id === providerKind)) return
      this.settings.hiddenCliProviderIds = visible
        ? this.settings.hiddenCliProviderIds.filter((id) => id !== providerKind)
        : [...new Set([...this.settings.hiddenCliProviderIds, providerKind])]
      await this.saveSettings()
    },
    ...
    async refreshExplorer(rootPath?: string) {
      ...
      await this.loadMinecraftCapabilities()
    },
    ...
    async openDragonCoreToolsFile(projectPath: string, filePath: string): Promise<boolean> {
      const normalizedProjectPath = normalizeFilePath(projectPath)
      if (!normalizedProjectPath) return false
      if (!this.workspace || workspaceKey(this.workspace.rootPath) !== workspaceKey(normalizedProjectPath)) {
        this.workspace = await backend.openProject(normalizedProjectPath)
        this.resetMinecraftCapabilities()
        await this.loadMinecraftCapabilities(normalizedProjectPath)
      }
      const normalizedFilePath = normalizeFilePath(filePath)
      if (!normalizedFilePath) return true
      return this.openFile(normalizedFilePath, {
        createUnsupportedTabOnError: false,
        revealInExplorer: false,
        skipWorkspaceWatchSync: true,
      })
    },
    ...
    async createTerminalSession(
      ...
      options: { name?: string; cwd?: string; initialPrompt?: string; scope?: 'project' | 'dragon'; launchPrompt?: boolean } = {},
    ) {
      ...
      this.terminalSessions.push(session)
      if (options.scope === 'dragon' && !isLocalTerminalSessionKind(session.providerKind)) {
        this.dragonCliSessionIds = Array.from(new Set([...this.dragonCliSessionIds, session.id]))
        this.activeDragonCliSessionId = session.id
      }
      this.setActiveTerminalSession(session.id)
      this.terminalDrafts[session.id] = this.terminalDrafts[session.id] ?? ''
      this.terminalConversation[session.id] = this.terminalConversation[session.id] ?? []
    ...
    // 执行客户端动作；启动器与 Java 日志始终托管在 Super High 本地终端。
    async runMinecraftClientTerminalAction(action: ProjectTerminalAction) {
      if (!this.workspace) throw new Error('请先打开项目。')
      const operation = action.operation ?? 'start'
      if (operation === 'start') {
        await this.startMinecraftClient()
      } else if (operation === 'stop') {
        await this.stopMinecraftClient()
      } else if (operation === 'restart') {
        await this.restartMinecraftClient()
      } else if (operation === 'release') {
        this.minecraftClientStatus = await backend.releaseMinecraftClient(this.workspace.rootPath)
      } else {
        this.minecraftClientStatus = await backend.minecraftClientStatus(this.workspace.rootPath)
      }
      this.showActivityMessage(`${action.label}：${this.minecraftClientStatus?.message ?? '已执行'}`)
    },
    ...
    async runProjectTerminalAction(action: ProjectTerminalAction) {
      ...
      try {
        ...
        } else if (action.kind === 'minecraft-client') {
          await this.runMinecraftClientTerminalAction(action)
        }
      } catch (error) {
    ...
    setActiveTerminalSession(sessionId: string | null) {
      ...
      } else if (this.dragonCliSessionIds.includes(session.id)) {
        this.activeDragonCliSessionId = session.id
      } else {
    ...
    async closeTerminalSession(sessionId: string) {
      ...
      this.dragonCliSessionIds = this.dragonCliSessionIds.filter((id) => id !== sessionId)
      ...
      if (this.activeDragonCliSessionId === sessionId) {
        this.activeDragonCliSessionId = this.dragonCliTerminalSessions[0]?.id ?? null
      }
    ...
    setWorkspaceSurface(surface: WorkspaceSurfaceKind) {
      ...
      if ((surface === 'editor' || surface === 'docs' || surface === 'dragon' || surface === 'files') && !this.workspace) return
      if (surface === 'minecraft' && (!this.workspace || (!this.minecraftClientConfig && !this.minecraftClientConfigError))) return
    ...
    setItemSearchDialogOpen(open: boolean) {
      this.itemSearchDialogOpen = open && this.itemLibraryAvailable
    },
