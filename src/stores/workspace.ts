...
export const useWorkspaceStore = defineStore('workspace', {
  ...
  actions: {
    ...
    async resumeExitedCliSession(sessionId: string): Promise<TerminalSession | null> {
      const endedSession = this.terminalSessions.find((session) => session.id === sessionId)
      if (!endedSession || (!endedSession.restoredFromDisk && this.terminalExitCodes[sessionId] == null)) return null

      const conversations = await this.refreshNativeCliConversations()
      const candidates = conversations.filter((conversation) => (
        conversation.resumeSupported
        && !!conversation.nativeSessionId?.trim()
        && conversation.providerKind === endedSession.providerKind
        && workspaceKey(conversation.cwd) === workspaceKey(endedSession.cwd)
      ))
      const exact = endedSession.nativeConversationId
        ? candidates.find((conversation) => conversation.id === endedSession.nativeConversationId)
        : null
      const firstPrompt = (this.terminalConversation[sessionId] ?? [])
        .find((message) => message.role === 'user' && !!message.content.trim())?.content ?? ''
      const compact = (value: string) => value.replace(/\s+/g, ' ').trim().toLocaleLowerCase()
      const promptText = compact(firstPrompt)
      const sessionName = compact(endedSession.name ?? '')
      const ranked = candidates.map((conversation) => {
        const title = compact(conversation.title)
        const summary = compact(conversation.summary)
        let score = 0
        if (promptText && title === promptText) score += 8
        else if (promptText && title && (promptText.startsWith(title) || title.startsWith(promptText))) score += 5
        if (promptText && summary && (summary.includes(promptText) || promptText.includes(summary))) score += 2
        if (sessionName && title === sessionName) score += 3
        return { conversation, score, updatedAt: Date.parse(conversation.updatedAt) || 0 }
      }).sort((left, right) => right.score - left.score || right.updatedAt - left.updatedAt)
      const conversation = exact ?? ranked[0]?.conversation
      if (!conversation) {
        this.showErrorMessage('没有找到这个已退出对话对应的可恢复记录。')
        return null
      }

      const previousIndex = this.terminalSessions.findIndex((session) => session.id === sessionId)
      const previousDraft = this.terminalDrafts[sessionId] ?? ''
      const previousMessages = this.terminalConversation[sessionId] ?? []
      const replacement = await this.resumeNativeCliConversation(conversation)
      if (!replacement || replacement.id === sessionId) return replacement

      if (endedSession.name?.trim()) replacement.name = endedSession.name.trim()
      if (previousDraft) this.terminalDrafts[replacement.id] = previousDraft
      if (previousMessages.length) {
        this.terminalConversation[replacement.id] = previousMessages.map((message) => ({
          ...message,
          sessionId: replacement.id,
        }))
      }
      await this.closeTerminalSession(sessionId)
      const replacementIndex = this.terminalSessions.findIndex((session) => session.id === replacement.id)
      if (replacementIndex >= 0 && previousIndex >= 0 && replacementIndex !== previousIndex) {
        this.terminalSessions.splice(replacementIndex, 1)
        this.terminalSessions.splice(Math.min(previousIndex, this.terminalSessions.length), 0, replacement)
      }
      this.setActiveTerminalSession(replacement.id)
      this.schedulePersistTerminalState()
      return replacement
    },
    ...
    setWorkspaceSurface(surface: WorkspaceSurfaceKind) {
      ...
      if ((surface === 'docs' || surface === 'files') && !this.workspace) return
    ...
    async openTerminalFileLink(path: string) {
      this.setWorkspaceSurface('project')
      if (this.settings.activePreviewMode !== 'code') this.setPreviewMode('code')
