...
const WORKSPACE_FILE_BURST_DELAY_MS = 350
const WORKSPACE_FILE_BURST_THRESHOLD = 50
...
function isLocalTerminalSessionKind(kind: string): boolean {
  return kind === 'local' || kind === 'ssh' || isProjectManagedTerminalSessionKind(kind)
}
...
export const useWorkspaceStore = defineStore('workspace', {
  state: () => ({
    ...
    remoteConfig: null as RemoteConnectionConfig | null,
    remoteDriveStatus: null as RemoteDriveStatus | null,
    remoteLoading: false,
    mobileHostConfig: null as MobileHostConfig | null,
    mobileHostStatus: null as MobileHostStatus | null,
    mobileHostLoading: false,
    sshConnections: [] as SshConnectionConfig[],
    sshConnectionsLoading: false,
    tailscaleStatus: null as TailscaleStatus | null,
  ...
  actions: {
    ...
    async initialize() {
      ...
      try {
        ...
        if (startupTarget) {
          ...
          if (opened && startupTarget.filePath) await this.openFile(startupTarget.filePath)
    ...
    bindBackendEvents() {
      ...
      void onTerminalOutput((event) => {
        this.appendTerminalConversationReply(event.sessionId, event.chunk)
        this.schedulePlanDetection(event.sessionId, event.chunk)
      }).then((unlisten) => {
        if (backendEventListenersDisposed) unlisten()
        else backendEventUnlisteners.push(unlisten)
      }).catch((error) => {
    ...
    scheduleWorkspaceFilesChangedRefresh(event: WorkspaceFilesChangedEvent) {
      ...
      const existingTimer = this.workspaceFileRefreshTimers[key]
      if (existingTimer) window.clearTimeout(existingTimer)
      const pendingCount = (this.pendingWorkspaceFileChanges[key] ?? []).length
      const delay = pendingCount > WORKSPACE_FILE_BURST_THRESHOLD
        ? WORKSPACE_FILE_BURST_DELAY_MS
      ...
      for (const tab of tabs) {
        ...
        try {
          ...
          const content = isImage
            ? await backend.readImageAsDataUrl(tab.path)
            : await backend.readFile(tab.path)
          tab.content = content
          tab.contentType = isImage ? 'image' : 'text'
          tab.language = isImage ? 'image' : inferLanguage(tab.path)
          if (!isImage) tab.lastKnownDiskContent = content
          delete this.fileConflictPaths[tabPathKey]
        } catch {
          if (tab.contentType === 'text') {
            this.fileConflictPaths[tabPathKey] = true
          } else if (tab.contentType === 'unsupported') {
            tab.content = '此文件已被删除或无法读取。'
          }
        }
      }
    },
    ...
    async saveChannel(channel: ChannelConfig) {
      const channels = this.settings.channels ?? []
      const existing = channels.findIndex((item) => item.id === channel.id)
      if (existing >= 0) {
    ...
    async deleteChannel(id: string) {
      this.settings.channels = (this.settings.channels ?? []).filter((channel) => channel.id !== id)
      await this.saveSettings()
    },
    ...
    async openFile(path: string, options: OpenFileOptions = {}): Promise<boolean> {
      ...
      if (existing) {
        if (options.verifyExistingFile) {
          try {
            this.showErrorMessage('')
            await backend.readFile(normalizedPath)
          } catch (error) {
      ...
      const isImage = isPreviewableImage(normalizedPath)
      let content = ''
      try {
        this.showErrorMessage('')
        content = isImage ? await backend.readImageAsDataUrl(normalizedPath) : await backend.readFile(normalizedPath)
      } catch (error) {
        if (options.createUnsupportedTabOnError === false) {
          this.showErrorMessage(formatError(error))
          return false
        }
        const tab: EditorTab = {
      ...
      const tab: EditorTab = {
        ...
        isDirty: false,
        ...(!isImage ? { lastKnownDiskContent: content } : {}),
      }
      this.tabs.push(tab)
      this.activeTabId = tab.id
      if (revealInExplorer) await this.revealPathInExplorer(normalizedPath)
    ...
    async searchProject(query: string, displayQuery = query, options: ProjectSearchOptions = {}) {
      ...
      if (!backendQuery) {
        ...
        if (searchDebounceTimer) { window.clearTimeout(searchDebounceTimer); searchDebounceTimer = null }
    ...
    async ensureRemoteConfig() {
      if (!isTauri()) return null
      if (this.remoteConfig) return this.remoteConfig
      this.remoteConfig = await backend.defaultRemoteConfig()
    ...
    async checkTailscaleStatus() {
      if (!isTauri()) return null
      try {
        this.tailscaleStatus = await backend.remoteTailscaleStatus()
        return this.tailscaleStatus
      } catch {
        this.tailscaleStatus = { available: false, ip: null }
        return this.tailscaleStatus
      }
    },

    async refreshRemoteDriveStatus() {
      if (!isTauri()) return null
      this.remoteLoading = true
      try {
        const config = await this.ensureRemoteConfig()
        if (!config) return null
        this.remoteDriveStatus = await backend.remoteDriveStatus(config)
        return this.remoteDriveStatus
      } catch (error) {
        this.showErrorMessage(`远程端状态读取失败：${formatError(error)}`)
        return null
      } finally {
    ...
    async mapRemoteDrive() {
      if (!isTauri()) return null
      this.remoteLoading = true
      try {
        const config = await this.ensureRemoteConfig()
        if (!config) return null
        const result = await backend.mapRemoteDrive(config)
        if (result.success) {
          this.showActivityMessage(`已映射 ${result.driveLetter} → ${result.uncPath}`)
        } else {
          this.showErrorMessage(`映射失败：${result.error ?? '未知错误'}`)
        }
        await this.refreshRemoteDriveStatus()
        return result
      } catch (error) {
        this.showErrorMessage(`映射失败：${formatError(error)}`)
        return null
      } finally {
        this.remoteLoading = false
      }
    ...
    async unmapRemoteDrive() {
      if (!isTauri()) return null
      const letter = this.remoteConfig?.localDriveLetter ?? 'S'
      this.remoteLoading = true
      try {
        const result = await backend.unmapRemoteDrive(letter)
        if (result.success) {
          this.showActivityMessage(`已断开 ${result.driveLetter}`)
        }
        await this.refreshRemoteDriveStatus()
        return result
      } catch (error) {
        this.showErrorMessage(`断开失败：${formatError(error)}`)
        return null
      } finally {
        this.remoteLoading = false
      }
    ...
    async openRemoteWorkspace() {
      const status = this.remoteDriveStatus ?? await this.refreshRemoteDriveStatus()
      const root = status?.workspaceRoot
      if (!root) return
      await this.openProject(root)
    },

    async updateRemoteConfig(partial: Partial<RemoteConnectionConfig>) {
      const config = await this.ensureRemoteConfig()
      if (!config) return
      Object.assign(config, partial)
      this.remoteConfig = { ...config }
      // Persist to app settings alongside other settings
      // (remoteConfig is stored separately for now, refreshed from defaults each time)
    },
    ...
    async createTerminalSession(
      ...
      if (initialPrompt && !options.launchPrompt) {
        ...
        this.appendTerminalConversationMessage({
          id: crypto.randomUUID(),
    ...
    async runProjectTerminalAction(action: ProjectTerminalAction) {
      ...
      try {
        if (action.kind === 'start-project') {
          await this.startProjectFromTerminalAction()
          this.showActivityMessage(`${action.label}：已执行`)
    ...
    async refreshSshConnections() {
      if (!isTauri()) return
      this.sshConnectionsLoading = true
      try {
        this.sshConnections = await backend.listSshConnections()
      } catch (error) {
        this.showErrorMessage(`SSH 连接列表加载失败：${formatError(error)}`)
      } finally {
        this.sshConnectionsLoading = false
      }
    },

    async saveSshConnection(config: SshConnectionConfig) {
      if (!isTauri()) return
      try {
        await backend.saveSshConnection(config)
        await this.refreshSshConnections()
        this.showActivityMessage(`已保存 SSH 连接：${config.name}`)
        this.settingsOpen = false
      } catch (error) {
        this.showErrorMessage(`保存 SSH 连接失败：${formatError(error)}`)
      }
    },

    async deleteSshConnection(id: string) {
      if (!isTauri()) return
      const config = this.sshConnections.find((c) => c.id === id)
      try {
        await backend.deleteSshConnection(id)
        this.sshConnections = this.sshConnections.filter((c) => c.id !== id)
        if (config) this.showActivityMessage(`已删除 SSH 连接：${config.name}`)
      } catch (error) {
        this.showErrorMessage(`删除 SSH 连接失败：${formatError(error)}`)
      }
    },

    async connectSsh(connectionId: string) {
      if (!isTauri() || !this.workspace) return null
      const config = this.sshConnections.find((c) => c.id === connectionId)
      if (!config) {
        this.showErrorMessage('SSH 连接配置未找到')
        return null
      }
      try {
        const session = await backend.createSshTerminalSession(
          this.workspace.id,
          connectionId,
        ...
        this.activeLocalTerminalSessionId = session.id
        this.setWorkspaceSurface('project')
        this.setPreviewMode('local_terminal')
        this.schedulePersistTerminalState()
        this.showActivityMessage(`已连接 SSH：${config.name}`)
        return session
      } catch (error) {
        this.showErrorMessage(`SSH 连接失败：${formatError(error)}`)
        return null
      }
    },
    ...
    setSettingsOpen(open: boolean) {
      ...
      if (open) {
        void this.refreshCliEnvironments()
        void this.refreshAppStorageInfo()
        void this.refreshRemoteDriveStatus()
      }
    },
