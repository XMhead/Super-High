...
function scheduleFit() {
  ...
  resizeTimer = setTimeout(() => {
    ...
    if (fitted && fitted.cols < sourceCols && terminal.options.fontSize > 6) {
      terminal.options.fontSize = fittedTerminalFontSize(terminal.options.fontSize, fitted.cols, sourceCols)
    }
    // A viewer changes only its font; ANSI cursor coordinates belong to the shared PTY.
    terminal.resize(sourceCols, sourceRows)
    const screen = terminal.element?.querySelector<HTMLElement>('.xterm-screen')
    if (terminal.element && screen?.offsetWidth) {
      const width = screen.offsetWidth + 15
      terminal.element.style.width = `${width}px`
      terminal.element.style.zoom = String(Math.min(
        1, terminalHost.value.clientWidth / width,
        terminalHost.value.clientHeight / Math.max(1, screen.offsetHeight),
      ))
    }
  }, 150)
}
