...
import { MobileTerminalRenderer, writeMobileTerminalProjection, MOBILE_TERMINAL_FRAME_TIMEOUT_MS } from './mobileTerminalRenderer'
...
function scheduleFit() {
  ...
  resizeTimer = setTimeout(() => {
    ...
    if (!available) return
    terminal.options.fontSize = fittedTerminalFontSize(props.fontSize, available.cols, sourceCols)
    // Account for the renderer rounding fractional cell widths to physical pixels.
    const fitted = fit.proposeDimensions()
    if (fitted && fitted.cols < sourceCols && terminal.options.fontSize > 3) {
      terminal.options.fontSize = fittedTerminalFontSize(terminal.options.fontSize, fitted.cols, sourceCols)
    }
    const viewport = fit.proposeDimensions()
    if (!viewport) return
    terminal.resize(sourceCols, viewport.rows)
    renderer?.resizeViewport(viewport.rows)
    if (renderer) void writeMobileTerminalProjection(terminal, renderer.render(''))
  }, 150)
}
...
function scheduleFrameFlush() {
  ...
  frameTimer = setTimeout(() => {
    ...
    void writeMobileTerminalProjection(terminal, renderer.flushPending())
  ...
  try {
    ...
    if (snapshot.cols && snapshot.rows) {
      if (sourceCols !== snapshot.cols || sourceRows !== snapshot.rows) {
        ...
        if (terminal) terminal.resize(sourceCols, terminal.rows)
        scheduleFit()
      }
    } else {
    ...
    if (terminal && renderer) {
      ...
      await writeMobileTerminalProjection(terminal, output)
      scheduleFrameFlush()
    }
    if (disposed || epoch !== generation) return
...
watch([selectedId, () => props.theme], async () => {
  ...
  terminal?.reset()
  renderer?.dispose()
  renderer = new MobileTerminalRenderer(sourceCols, sourceRows, terminal?.rows ?? sourceRows, activeSession.value?.providerKind === 'codex')
...
onMounted(async () => {
  terminal = new Terminal({
    cols: sourceCols, rows: sourceRows,
