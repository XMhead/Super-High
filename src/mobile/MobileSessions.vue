...
function scheduleFit() {
  ...
  resizeTimer = setTimeout(() => {
    ...
    fit?.fit()
    terminal.options.fontSize = fittedTerminalFontSize(props.fontSize, terminal.cols)
    fit.fit()
    // Account for the renderer rounding fractional cell widths to physical pixels.
    if (terminal.cols < 80 && terminal.options.fontSize > 6) {
      terminal.options.fontSize = fittedTerminalFontSize(terminal.options.fontSize, terminal.cols)
      fit.fit()
    }
