...
const props = withDefaults(defineProps<{ api: MobileHostApi; root: string; mode: 'terminal' | 'ai'; connected?: boolean; fontSize?: number; theme?: string; hiddenCliProviderIds?: string[]; initialDraft?: string }>(), { connected: true, fontSize: 14, theme: 'dark', hiddenCliProviderIds: () => ['dsh'], initialDraft: '' })
...
async function sendDraft() {
  if ((!draft.value.trim() && !attachments.value.length) || !attachmentsReady.value) return
  ...
  <section class="mobile-sessions">
    ...
    <div v-if="!showHistory && connected && root && visibleSessions.length" class="mobile-session-tabs" role="tablist" aria-label="会话列表">
      <button
        ...
        :aria-label="`${cliProviderLabel(session.providerKind)} ${session.title}`"
      ...
      >
        ...
        <span class="mobile-session-tab-label">
          <strong>{{ cliProviderLabel(session.providerKind) }}</strong>
    ...
    <form v-if="!showHistory" class="mobile-session-input" :class="{ 'is-focused': inputFocused }" @submit.prevent="sendDraft">
      <div class="mobile-compose-row">
        ...
        <div class="mobile-compose-actions">
          ...
          <button class="mobile-icon-button" type="button" aria-label="唤起指令" title="唤起自定义指令" :disabled="!running || sending"><Slash :size="19" /></button>
