...
<template>
  <main class="mobile-app">
    ...
    <MobileSessions v-show="!viewer" :key="activeProject?.path ?? 'disconnected'" :api="api" :root="activeProject?.path ?? ''" :connected="connected" :inert="settingsOpen || drawerOpen" mode="ai" :theme="appearance.theme" :font-size="appearance.fontSize" @connect="connected ? drawerOpen = true : settingsOpen = true" />
    <template v-if="drawerOpen">
      ...
      <div class="mobile-file-list">
        ...
        <footer class="mobile-account-row"><Server :size="20" /><span>{{ connected ? baseUrl : '未连接电脑' }}</span><button class="mobile-icon-button" type="button" aria-label="账号与设置" @click="settingsOpen = true"><MoreHorizontal :size="21" /></button></footer>
      </aside>
    </template>
