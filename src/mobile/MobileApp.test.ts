...
const terminalRefresh = vi.hoisted(() => vi.fn())
vi.mock('@capacitor/core', () => ({ Capacitor: { isNativePlatform: () => native.enabled } }))
vi.mock('./MobileSessions.vue', () => ({ default: { props: ['root', 'connected', 'api'], methods: { refresh: terminalRefresh }, template: '<div class="test-terminal" :data-root="root" :data-connected="connected"><button class="probe-active-host" @click="api.status()">Probe</button></div>' } }))
...
describe('Host-delivered mobile workspace', () => {
  it('expands folders in place, keeps siblings, and collapses nested files without changing the root', async () => {
    vi.mocked(MobileHostApi.prototype.status).mockResolvedValue({ activeProjectPath: 'D:/tree' } as never)
    vi.spyOn(MobileHostApi.prototype, 'listDirectory').mockImplementation(async path => ({ path, entries: path === 'D:/tree' ? [
      { path: 'D:/tree/src', name: 'src', type: 'directory' }, { path: 'D:/tree/README.md', name: 'README.md', type: 'file' },
    ] : [{ path: 'D:/tree/src/main.ts', name: 'main.ts', type: 'file' }] }) as never)
    vi.spyOn(MobileHostApi.prototype, 'readTextFile').mockResolvedValue('export {}')
    ...
    const folder = root.querySelector<HTMLButtonElement>('[title="D:/tree/src"]')!
    folder.click(); await flush()
    expect(folder.getAttribute('aria-expanded')).toBe('true')
    expect(root.querySelector('.mobile-path')?.textContent).toBe('D:/tree')
    expect([...root.querySelectorAll('.mobile-file-row strong')].map(e => e.textContent)).toEqual(['src', 'main.ts', 'README.md'])
    root.querySelector<HTMLButtonElement>('[title="D:/tree/src/main.ts"]')!.click(); await flush()
    expect(root.querySelector('.mobile-viewer pre')?.textContent).toBe('export {}')
    root.querySelector<HTMLButtonElement>('[aria-label="打开工作区资源管理器"]')!.click(); await flush()
    root.querySelector<HTMLButtonElement>('[title="D:/tree/src"]')!.click(); await flush()
    expect(root.querySelector('[title="D:/tree/src/main.ts"]')).toBeNull()
    expect(root.querySelector('[title="D:/tree/README.md"]')).not.toBeNull()
    expect(root.querySelector('.mobile-path')?.textContent).toBe('D:/tree')
  })
  ...
  it('ignores a directory refresh that finishes after disconnect', async () => {
    ...
    expect(root.querySelector('.test-terminal')?.getAttribute('data-connected')).toBe('false')
  ...
  it('retries saved connections until the restarted desktop becomes available', async () => {
    ...
    expect(MobileHostApi.prototype.status).toHaveBeenCalled()
    await vi.advanceTimersByTimeAsync(3000); await flush()
  ...
  it('recovers after a temporary outage without replacing the current terminal view', async () => {
    ...
    expect(MobileHostApi.prototype.status).toHaveBeenCalled()
    await vi.advanceTimersByTimeAsync(3000); await flush()
    expect(root.querySelector('.test-terminal')?.getAttribute('data-connected')).toBe('true')
  ...
  it('routes terminal sessions to the desktop instance that owns a selected historical workspace', async () => {
    vi.mocked(MobileHostApi.prototype.status).mockResolvedValue({
      activeProjectPath: 'D:/工作区甲',
      workspaceHosts: [
        { path: 'D:/工作区甲', port: 10320 },
        { path: 'D:/工作区乙', port: 43125 },
      ],
    } as never)
    vi.mocked(MobileHostApi.prototype.listRecentProjects).mockResolvedValue([
      { path: 'D:/工作区甲', name: '工作区甲', lastOpenedAt: '' },
      { path: 'D:/工作区乙', name: '工作区乙', lastOpenedAt: '' },
    ])
    vi.spyOn(MobileHostApi.prototype, 'listDirectory').mockImplementation(async path => ({ path, entries: [] }) as never)
    const { root } = mount('/mobile/', '#token=handoff-token')
    await flush()
    root.querySelector<HTMLButtonElement>('[aria-label="打开工作区资源管理器"]')!.click(); await flush()
    root.querySelector<HTMLButtonElement>('[aria-label="选择历史工作区"]')!.click(); await flush()
    clickText(root, '工作区乙D:/工作区乙'); await flush()
    expect(root.querySelector('.test-terminal')?.getAttribute('data-root')).toBe('D:/工作区乙')
    root.querySelector<HTMLButtonElement>('.probe-active-host')!.click(); await flush()
    const context = vi.mocked(MobileHostApi.prototype.status).mock.contexts.at(-1) as unknown as { baseUrl: string }
    expect(context.baseUrl).toBe('http://host.test:43125')
  })
