...
describe('Host-delivered mobile workspace', () => {
  it('ignores older preference polls and late responses after disconnect', async () => {
    vi.mocked(MobileHostApi.prototype.status).mockResolvedValue({ themeId: 'dark', hiddenCliProviderIds: ['dsh'] } as never)
  ...
  it('uses the desktop theme and persists mobile theme and CLI visibility changes', async () => {
    vi.mocked(MobileHostApi.prototype.status).mockResolvedValue({ themeId: 'shen-dian', hiddenCliProviderIds: ['dsh'] } as never)
    const save = vi.spyOn(MobileHostApi.prototype, 'updatePreferences').mockImplementation(async value => {
      const result = { themeId: value.themeId ?? 'light', hiddenCliProviderIds: value.hiddenCliProviderIds ?? ['dsh'] }
    ...
    const dsh = root.querySelector<HTMLInputElement>('input[role="switch"][aria-label="dsh-tui"]')!
    expect(dsh.checked).toBe(false)
    dsh.checked = true; dsh.dispatchEvent(new Event('change')); await flush()
