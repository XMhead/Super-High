...
describe('mobile CLI sessions', () => {
  it('uses the shared PTY grid before parsing output and never resizes it for the phone', async () => {
    ...
    expect(output.resize.mock.invocationCallOrder.at(-1)).toBeLessThan(output.write.mock.invocationCallOrder[0])
    ...
    expect(output.resize).toHaveBeenLastCalledWith(132, 20)
