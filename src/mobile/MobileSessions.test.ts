...
const output = vi.hoisted(() => ({ write: vi.fn(), reset: vi.fn(), resize: vi.fn(), sourceResize: vi.fn(), input: (_input: string) => {}, focus: vi.fn(), line: '' }))
vi.mock('./mobileTerminalRenderer', () => ({
  MOBILE_TERMINAL_CLEAR: '',
  MOBILE_TERMINAL_FRAME_TIMEOUT_MS: 250,
  MobileTerminalRenderer: class {
    hasPendingOutput = false
    resizeSource(cols: number, rows: number) { output.sourceResize(cols, rows) }
    resizeViewport() {}
    reset() {}
    dispose() {}
    render(data: string) { return data }
    flushPending() { return '' }
    takeHistoryCorrections() { return [] }
  },
  writeMobileTerminalProjection: async (terminal: { write: (text: string, done: () => void) => void }, text: string) => {
...
vi.mock('@xterm/xterm', () => ({ Terminal: class {
  ...
  resize(cols: number, rows: number) { this.cols = cols; this.rows = rows; output.resize(cols, rows) }
  loadAddon() {}
  open() {}
...
vi.mock('@xterm/addon-fit', () => ({ FitAddon: class { proposeDimensions() { return { cols: 60, rows: 20 } } } }))
...
describe('mobile CLI sessions', () => {
  it('uses the shared PTY grid before parsing output and never resizes it for the phone', async () => {
    const api = mockApi()
    api.terminalBuffer.mockResolvedValueOnce({ buffer: 'wide frame', startByte: 0, endByte: 10, running: true, reset: false, cols: 132, rows: 40 })
      .mockImplementation(() => new Promise(() => {}))
    mount(api); await flush()
    expect(output.sourceResize).toHaveBeenCalledWith(132, 40)
    expect(output.sourceResize.mock.invocationCallOrder.at(-1)).toBeLessThan(output.write.mock.invocationCallOrder[0])
    ...
    root.querySelector('textarea')!.dispatchEvent(new FocusEvent('focus'))
    await vi.advanceTimersByTimeAsync(150)
    expect(output.resize).toHaveBeenLastCalledWith(60, 20)
    expect(api.resizeTerminal).not.toHaveBeenCalled()
    dispose?.(); dispose = undefined
    expect(api.resizeTerminal).not.toHaveBeenCalled()
  })

  it('waits for output even at cursor zero and cancels the pending read when leaving', async () => {
    const api = mockApi()
    api.terminalBuffer.mockResolvedValueOnce({ buffer: '', startByte: 0, endByte: 0, running: true, reset: false, cols: 120, rows: 32 })
      .mockImplementation(() => new Promise(() => {}))
    mount(api); await flush()
    await vi.advanceTimersByTimeAsync(1)
    expect(api.terminalBuffer).toHaveBeenCalledTimes(2)
    const options = api.terminalBuffer.mock.calls[1][2] as { waitMs: number; signal: AbortSignal }
  ...
  it('starts the next incremental read after drawing without a 350 ms pause', async () => {
    const api = mockApi()
    api.terminalBuffer.mockResolvedValueOnce({ buffer: 'first', startByte: 0, endByte: 5, running: true, reset: false, cols: 120, rows: 32 })
      .mockResolvedValueOnce({ buffer: 'second', startByte: 0, endByte: 11, running: true, reset: false, cols: 120, rows: 32 })
      .mockImplementation(() => new Promise(() => {}))
    mount(api); await flush()
    await vi.advanceTimersByTimeAsync(2)
    expect(output.write.mock.calls.map(call => call[0])).toEqual(['first', 'second'])
    expect(api.terminalBuffer.mock.calls.map(call => call[1])).toEqual([0, 5, 11])
  })

  it('closes the clicked session immediately and shows launchers when no sessions remain', async () => {
    const api = mockApi()
    mount(api); await flush()
    root.querySelector<HTMLButtonElement>('[aria-label="关闭 terminal-1"]')!.click()
    await flush()
    expect(api.closeTerminal.mock.calls).toEqual([['terminal-1']])
    expect(root.querySelector('[role="tab"]')).toBeNull()
    expect(root.querySelector('.terminal-empty-card')?.textContent).toContain('暂无 CLI 会话')
    expect(root.querySelector('.mobile-empty-launchers button[title="Codex CLI"]')).not.toBeNull()
  })
  it('keeps the session available when closing fails', async () => {
    const api = mockApi()
    api.closeTerminal.mockRejectedValue(new Error('close failed'))
    mount(api); await flush()
    root.querySelector<HTMLButtonElement>('[aria-label="关闭 terminal-1"]')!.click()
    await flush()
    expect(root.querySelector('[role="tab"]')).not.toBeNull()
    expect(root.textContent).toContain('close failed')
  })
  it('starts a CLI from the empty-state card', async () => {
    const api = { ...mockApi(), createTerminal: vi.fn().mockResolvedValue(session) }
    api.listTerminals.mockResolvedValue([])
    mount(api); await flush()
    root.querySelector<HTMLButtonElement>('.mobile-empty-launchers button[title="Codex CLI"]')!.click()
    await flush()
    expect(api.createTerminal.mock.calls).toEqual([['C:/work', 'codex']])
    expect(root.querySelector('.terminal-empty-card')).toBeNull()
    expect(root.querySelector('[role="tab"]')).not.toBeNull()
  })
  it('mounts product launchers into a header created in the same render', async () => {
    const api = mockApi()
    root = document.createElement('div')
    document.body.append(root)
    const app = createApp({ render: () => h('main', [
      h('header', { id: 'mobile-cli-launchers' }),
      h(MobileSessions, { api: api as unknown as MobileHostApi, root: 'C:/work', mode: 'ai', launcherTarget: true }),
    ]) })
    app.mount(root)
    dispose = () => { app.unmount(); root.remove() }
    await flush()
    const launcher = root.querySelector<HTMLButtonElement>('#mobile-cli-launchers button[title="Codex CLI"]')!
    expect(launcher).not.toBeNull()
    expect(launcher.disabled).toBe(false)
    expect(launcher.querySelector('svg')?.getAttribute('aria-label')).toBe('Codex')
    expect(root.querySelector('.mobile-sessions .mobile-cli-launchers')).toBeNull()
  })
  ...
  it('shows one session identifier and opens and navigates CLI commands without submitting a message', async () => {
    ...
    expect(output.focus).toHaveBeenCalledTimes(2)
  ...
  it('removes a remotely closed session without an ended-session warning', async () => {
    const api = mockApi()
    ...
    expect(api.terminalBuffer).toHaveBeenLastCalledWith('terminal-1', 5, expect.objectContaining({ waitMs: 1000 }))
    expect(root.textContent).not.toContain('会话已结束')
    expect(root.querySelector('[role="tab"]')).toBeNull()
  ...
  it('synchronizes inactive tab closures and selects a remaining session when the active one closes', async () => {
    const api = mockApi()
    const second = { ...session, id: 'terminal-2' }
    const third = { ...session, id: 'terminal-3' }
    api.listTerminals.mockResolvedValue([session, second, third])
    api.terminalBuffer.mockImplementation(() => new Promise(() => {}))
    mount(api); await flush()
    api.listTerminals.mockResolvedValue([session, second])
    await vi.advanceTimersByTimeAsync(1000)
    expect(root.querySelector('[aria-label="关闭 terminal-3"]')).toBeNull()
    expect(root.querySelector('[role="tab"][aria-selected="true"]')?.getAttribute('aria-label')).toBe('terminal-1')
    api.listTerminals.mockResolvedValue([second])
    await vi.advanceTimersByTimeAsync(1000)
    expect(root.querySelector('[aria-label="关闭 terminal-1"]')).toBeNull()
    expect(api.terminalBuffer).toHaveBeenLastCalledWith('terminal-2', 0, expect.any(Object))
    expect(root.querySelector('.mobile-error')).toBeNull()
  })
