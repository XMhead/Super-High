...
const output = vi.hoisted(() => ({ write: vi.fn(), reset: vi.fn(), resize: vi.fn(), input: (_input: string) => {}, focus: vi.fn(), line: '' }))
...
describe('mobile CLI sessions', () => {
  it('uses the shared PTY grid before parsing output and never resizes it for the phone', async () => {
    ...
    expect(output.resize).toHaveBeenCalledWith(132, 40)
    ...
    expect(output.resize).toHaveBeenLastCalledWith(132, 40)
