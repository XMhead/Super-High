...
import type { MobileHostApi } from '@/lib/hostApi'
...
async function updateAction(action: 'check' | 'download' | 'install') {
  ...
  try {
    ...
    if (action === 'install' && update.value.phase === 'installing') waitForUpdatedHost(Date.now(), update.value.version)
...
function waitForUpdatedHost(started: number, expectedVersion: string | null) {
  message.value = t('正在等待电脑重启，恢复连接后自动加载新版界面。', 'Waiting for the desktop to restart. The updated interface will load automatically.')
  reconnectTimer = setTimeout(async () => {
    ...
    else message.value = t('电脑连接尚未恢复，请检查电脑端安装状态后重新连接。', 'The computer is not available yet. Check installation on the desktop and reconnect.')
  }, 3000)
}
  ...
  <section class="mobile-settings" aria-label="设置">
    ...
    <div class="mobile-settings-content">
      ...
      <div v-else-if="section === 'update'" class="mobile-form-grid">
        ...
        <button class="mobile-primary-button" type="button" :disabled="busy || !connected" @click="updateAction('check')">{{ busy ? t('处理中', 'Working') : t('检查版本更新（含界面）', 'Check app and interface updates') }}</button>
        ...
        <button v-if="update?.phase === 'available'" class="mobile-text-button" type="button" :disabled="busy" @click="updateAction('download')">{{ t('下载更新', 'Download update') }}</button>
        <template v-if="update?.phase === 'ready'"><p>{{ t('安装会重启电脑端；仍有内部终端会话时不会安装。', 'Installation restarts the desktop app and is blocked while internal terminal sessions are active.') }}</p><button class="mobile-text-button" type="button" :disabled="busy" @click="updateAction('install')">{{ t('安装并重启电脑端', 'Install and restart desktop') }}</button></template>
