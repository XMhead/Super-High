...
function toggleCli(id: string, event: Event) {
  const hidden = new Set(props.hiddenCliProviderIds ?? [])
...
<template>
  <section class="mobile-settings" aria-label="设置">
    ...
    <div class="mobile-settings-content">
      ...
      <div v-else-if="section === 'cli'" class="mobile-settings-card"><label v-for="entry in CLI_PROVIDER_OPTIONS" :key="entry.id" class="mobile-settings-row"><span>{{ entry.name }}</span><input type="checkbox" role="switch" :aria-label="entry.name" :checked="!(hiddenCliProviderIds ?? []).includes(entry.id)" :disabled="!connected || preferencesBusy" @change="toggleCli(entry.id, $event)" /></label></div>
