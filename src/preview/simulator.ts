...
import { backend, type AppUpdateStatus } from '@/lib/tauri'
...
export function installSettingsSimulator(pinia: Pinia, options: SettingsSimulatorOptions): {
  ...
  const update = useAppUpdateStore(pinia)
  ...
  function persist() {
    ...
    try { sessionStorage.setItem(SETTINGS_SIMULATOR_KEY, JSON.stringify({ settings: workspace.settings, automatic: update.automatic, imageViewerEnabled })) }
  ...
  replace(backend, 'listDshProviders', async () => channelRead('读取 DSH 渠道', options.scenario === 'empty' ? [] : [{
    id: 'simulated-dsh', name: '模拟 DSH 渠道', baseUrl: 'https://dsh.preview.invalid/v1',
    api: 'openai-completions', hasApiKey: true, models: [{ id: 'simulated-model', name: '模拟模型' }],
  }]))
  replace(backend, 'listDshProfiles', async () => channelRead('读取 DSH 配置', options.scenario === 'empty' ? [] : [{
    id: 'simulated-profile', name: '模拟配置', provider: 'simulated-dsh', model: 'simulated-model', modelSource: 'config',
  }]))
  replace(backend, 'getDshDefaultModel', async () => channelRead('读取默认模型', options.scenario === 'empty' ? null : { provider: 'simulated-dsh', model: 'simulated-model' }))
  replace(backend, 'getDshProviderApiKey', async (id) => channelRead('读取模拟凭据', options.scenario === 'empty' || id !== 'simulated-dsh' ? null : 'SIMULATED-KEY'))
  ...
  function status(phase: AppUpdateStatus['phase']): AppUpdateStatus {
    return { phase, currentVersion: '0.0.0-simulated', version: phase === 'idle' || phase === 'upToDate' ? null : '0.0.1-simulated', notes: '模拟更新说明：仅展示检查、下载和安装回执，不连接网络、不下载或安装程序。', downloadedBytes: phase === 'ready' ? 1048576 : 0, totalBytes: 1048576, error: phase === 'error' ? '模拟更新失败：预设错误场景。' : null, blockingSessions: [] }
  }
  replace(update, 'refresh', async () => { receipt('模拟更新状态已刷新。') })
  replace(update, 'run', async () => { receipt('模拟模式不执行外部更新操作。') })
  replace(update, 'check', async (downloadAutomatically = false) => {
    if (update.busy || disposed) return
    update.busy = true; update.message = ''; update.status = status('checking')
    if (!await pause()) return
    update.status = status(options.scenario === 'error' ? 'error' : options.scenario === 'empty' ? 'upToDate' : 'available')
    update.busy = false
    if (downloadAutomatically && update.automatic && update.status.phase === 'available') await update.download()
  })
  replace(update, 'download', async () => {
    if (update.busy || disposed) return
    if (options.scenario === 'error') { update.status = status('error'); return }
    if (update.status?.phase !== 'available') return
    update.busy = true; update.status = status('downloading')
    if (!await pause()) return
    update.status.downloadedBytes = 524288
    if (!await pause()) return
    update.status = status('ready'); update.busy = false
  })
  replace(update, 'install', async () => {
    if (update.status?.phase !== 'ready' || update.busy) return
    update.message = '模拟安装回执：流程已完成演示，未启动安装程序，也未重启应用。'
    receipt(update.message)
  })
  replace(update, 'setAutomatic', (value) => { update.automatic = value; persist(); receipt(`自动更新已${value ? '启用' : '关闭'}（模拟）。`) })
  ...
  function seed() {
    ...
    update.automatic = true; update.busy = false; update.installing = false; update.message = ''
    update.status = status(options.scenario === 'error' ? 'error' : 'idle')
  ...
  if (options.persist) {
    try {
      ...
      if (saved && typeof saved === 'object' && 'settings' in saved && 'automatic' in saved && typeof saved.automatic === 'boolean') {
        workspace.settings = restoreSettings(saved.settings, workspace.settings)
        workspace.mobileHostConfig = { ...workspace.settings.mobileHost }
        update.automatic = saved.automatic
  ...
  return {
    ...
    dispose() {
      if (disposed) return
      disposed = true; cancelPending(); update.busy = false
