...
<template>
  <div class="html-preview">
    <header class="html-preview-bar">
      ...
      <span>{{ options.mode === 'simulate' ? '交互模拟 · 使用模拟数据，不操作本机' : '只读预览' }}</span>
      <select v-if="simulator" class="settings-input" aria-label="模拟场景" :value="options.scenario" @change="changeScenario"><option value="ready">可用</option><option value="empty">空状态</option><option value="error">失败</option></select>
      <button v-if="simulator" class="ghost-button small" @click="resetSimulation">重置模拟</button>
