...
Release 阶段遵循根 [AGENTS.md](../../../../AGENTS.md)：Alpha 标题为 `Super High v<版本> Alpha`，Beta 标题为 `Super High v<版本> Beta`（用户指定序号时追加），正式版标题为 `Super High v<版本>`。阶段标记用于发布展示，数字版本、标签和更新清单仍按现有规则递增。CHANGELOG 的版本标题：`## [<版本>] - YYYY-MM-DD`。
