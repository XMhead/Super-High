...
- 功能任务里，用户说“更新源码”“同步源码”或“源码即可”时，先按会话保护构建并覆盖桌面快捷方式实际指向的本机 EXE、从该入口验收，再按 [公开树检查规则](../superhigh-public-publish/SKILL.md#源码脱敏同步) 过滤、扫描和同步；该范围不生成 Release、标签、安装包或 APK。说明、项目技能或发布配置调整只核对内容及相关脚本并同步源码，不重建本机程序。
- 安装包发布独立负责完整构建、签名和分发；用户明确要求发布时才进入对应阶段。
- 收齐本次修改后按影响范围验收；纯样式和按钮删除只检查目标交互及相关布局，启动、跨页面、失败和退出路径仅在对应逻辑改变时检查。需要记录的更新先统一待发布版本与日志；确认所有实现者已停止修改待构建源码，再启动本机正式构建，由其覆盖前端检查，只有暂时无法正式构建或需要定位失败时单独运行前端构建。
- 先完成本轮受影响的完整用户操作链路，再冻结源码交接；浏览器预览按 [superhigh-html 验收](../superhigh-html/SKILL.md#验收与排错)，入口响应之后的编辑、提交或返回也属于对应功能的完成条件。交接记录源码提交和已有验证，发布执行者从该快照构建；发现遗漏先修复并补受影响验证，再重新交接，新源码不能沿用旧快照产物。
...
- 根 `CHANGELOG.md` 是当前与历史日志的唯一内容来源，经 `src/lib/releaseNotes.ts` 加载；设置中的 `AppUpdatePanel.vue` 复用 Markdown 渲染并保留历史版本入口。
- 更新后首次进入工作区，由 `appUpdate.ts` 将本版日志写入应用数据目录的 Markdown 文件，通过现有 `workspace.openFile` 在代码预览打开标签。沿用标签关闭行为，同版不重复自动打开；不另设日志浮层或确认按钮。
- 此链路改动验收实际桌面入口的自动开标签、Markdown 渲染、关闭与重启去重；浏览器日志预览不替代桌面启动验收。
...
公开工作树必须干净且检出本次 `origin/main`；入口校验远端来源与版本标签，使用本机原有签名、独立公开构建缓存，生成安装包、`.sig`、`latest.json` 和 APK，上传草稿并校验后公开。Release 阶段遵循根 [AGENTS.md](../../../AGENTS.md)，标题按 [Release 格式](references/release-notes.md)；用户指定阶段时通过 `--title` 传入完整标题。正文和更新说明取 `CHANGELOG.md` 的同版完整双语内容。
...
- 主 Agent 完成本机验收后，一次交接源码快照与已有验证，发布执行者负责同步、发布及收尾。提前预检仅回传阶段进展并等待交接，不结束任务再反复唤醒；主 Agent 直接使用其证据，不另设状态查询或复核任务。
- 发布执行者进入构建、上传、回下载，以及失败或完成时，主动向主 Agent 通知实际阶段、结果和未退出命令的运行句柄。等待以这些通知为准，单次工具等待保持在当前沟通时限内；没有状态变化时不连续短轮询，也不重复读取日志。
