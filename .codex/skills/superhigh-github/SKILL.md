...
- 收齐本次修改后按影响范围验收；纯样式和按钮删除只检查目标交互及相关布局，启动、跨页面、失败和退出路径仅在对应逻辑改变时检查。发布 Release 时先统一版本与日志；本机正式构建覆盖前端检查，只有暂时无法正式构建或需要定位失败时单独运行前端构建。
- 验收通过后冻结源码，按授权范围同步或发布。发布入口已完成的构建、扫描和下载校验直接复用；新增改动或失败只补受影响步骤。

## 同步范围与缺失更新排查

- 交付范围以根 [AGENTS.md](../../../AGENTS.md) 为准。仅本机验收与源码同步时保留版本号，不生成版本日志、标签、Windows 分发安装包或 APK；本机实际入口更新仍按根指令验收。下述版本与安装包流程仅在发布 Release 时执行。
...
1. 读取 `src-tauri/tauri.conf.json` 的版本，并查询仓库最新 Release 和目标标签。用户指定版本优先；未指定时，当前版本高于线上且标签未占用就沿用，否则常规小更新递增补丁号。
...
3. 按 [Release 格式](references/release-notes.md) 在根目录 `CHANGELOG.md` 顶部增加版本和日期。Agent 根据实际变化选择固定分类，先写完整中文，再写对应英文；省略空分类。日志会公开，不能带私有项目资料；Release 正文与应用更新说明使用该版本内容。
4. 运行 `node scripts/check-update-release.mjs v<版本>`。标签为 `v<版本>`，只指向脱敏后 `origin/main` 的当前提交。已公开的版本和标签不覆盖、不移动；需要修复已发布产物时发新版本。
