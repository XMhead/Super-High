...
需要同步当前桌面工作区、查看真实资源或操作手机界面时，执行 `npm run superhigh-html -- --page mobile --no-open`，打开 `http://127.0.0.1:1421/`（手机尺寸容器）或 `/mobile/`（真实 MobileApp）。首次在手机设置的账号管理中填写 Host 令牌，地址使用预览服务同源地址；后续自动恢复连接。代理默认访问本机 `10320`，其他 Host 在启动前设置 `SUPERHIGH_MOBILE_HOST`。沿用现有 Host 认证，不将令牌写入源码、查询参数或日志。
