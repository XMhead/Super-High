...
设置预览入口为 `http://127.0.0.1:1422/superhigh-html.html`；更新日志用 `npm run superhigh-html -- --section updates --no-open`，直接加载真实日志。需要模拟设置交互时才选择对应场景，例如 `--section mobile --scenario ready`。完整参数与有效值以 `node scripts/superhigh-html.mjs --help` 和 [options.ts](../../../src/preview/options.ts) 为准。
