...
5. 发布与产物验收完成后，使用 `agent-cleaner` 回收本次临时工作树：核对绝对路径、类型和链接指向，只摘除 `node_modules` junction，保留根依赖；将工作树移入所在磁盘的回收站并核对恢复记录，清理该工作树的 Git 登记。回收被系统拒绝时保留目录与登记，说明限制并交付已验收结果。日志和下载的验收产物保留。
...
- `-KeepWorktree`：默认开启；成功后保留工作树供验收和回收站收尾。常规发布不使用 `-KeepWorktree:$false` 的永久删除分支。
...
- 工作树清理失败时，先核对工作树绝对路径和 `node_modules` junction 指向，再只摘除 junction，按根 `AGENTS.md` 的 Windows 删除规则处理工作树；不得递归删除链接指向的在用依赖。
