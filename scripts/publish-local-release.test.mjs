...
test('builds both packages in persistent public caches and verifies drafts before publishing', async t => {
  ...
  assert.equal(f.releases.get(tag).name, `Super High ${tag} Alpha`)
...
test('a Windows Beta title survives upload resume without changing tags or Android titles', async t => {
  ...
  assert.equal(f.builds.length, 2)
  f.title = undefined
  const beforeDefaultResume = f.calls.length
  await f.publish(true)
  assert.equal(f.releases.get(tag).name, `Super High ${tag} Beta 1`)
  assert.ok(f.calls.slice(beforeDefaultResume).every(call => !['create', 'upload', 'edit'].includes(call.args[1])))
