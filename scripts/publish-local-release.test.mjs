...
function fixture(t) {
  ...
  put(join(source, 'package.json'), JSON.stringify({ scripts: { 'tauri:build': 'tauri build' } }))
  ...
  const run = (command, args, options = {}) => {
    ...
    if (command === 'gh') {
      ...
      if (args[1] === 'create') {
        ...
        f.releases.set(releaseTag, { id: f.releases.size + 1, tag_name: releaseTag, target_commitish: commit, draft: true, prerelease: args.includes('--prerelease'), assets: [] })
        return ''
      }
      if (args[1] === 'upload') {
  ...
  f.publish = resume => publishLocalRelease({ source, tag, resume }, { root, env, run, download, log: message => f.logs.push(message) })
  return f
}
