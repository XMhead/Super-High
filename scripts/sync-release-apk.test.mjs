...
function fixture(t, { assets = [], draft = true, corrupt = false, missingAndroid = false, draftTag404 = true, listDenied = false, concurrentDigest } = {}) {
  mkdirSync('.agent/tmp', { recursive: true })
  const directory = mkdtempSync(join('.agent/tmp', 'release-apk-test-'))
  t.after(() => rmSync(directory, { recursive: true }))
  const destination = { id: 123, tag_name: 'v0.1.5', draft, prerelease: false, assets }
  const calls = []
  const gh = args => {
    calls.push(args)
    if (args[0] === 'api') {
      if (args[1].includes('?per_page=100')) {
        if (listDenied) throw Object.assign(new Error('release list denied'), { stderr: 'HTTP 404' })
        return JSON.stringify([[destination]])
      }
      if (args[1].endsWith('/tags/v0.1.5') && draft && draftTag404) throw Object.assign(new Error('draft hidden'), { stderr: 'HTTP 404' })
...
test('draft tag 404 resolves through authenticated release listing and ID', t => {
  const f = fixture(t)
  assert.match(syncReleaseApks('v0.1.5', f.directory, { gh: f.gh, allowDraft: true, requireApk: true }), /verified/)
  assert.ok(f.calls.some(call => call.includes('--paginate') && call.includes('--slurp')))
  assert.ok(f.calls.some(call => call[1].endsWith('/releases/123')))
  assert.equal(f.calls.filter(call => call[1] === 'upload').length, 1)
})

test('permission failure during draft lookup is not mistaken for a missing optional APK', t => {
  const f = fixture(t, { missingAndroid: true, listDenied: true })
  assert.throws(() => syncReleaseApks('v0.1.5', f.directory, { gh: f.gh }), /release list denied/)
  assert.ok(f.calls.every(call => call[1] !== 'upload'))
})
