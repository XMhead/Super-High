...
export function getReleaseByTag(releaseTag, gh = runGh) {
  try {
    return JSON.parse(gh(['api', `repos/${repository}/releases/tags/${releaseTag}`]))
  } catch (error) {
    if (!/HTTP 404/.test(String(error.stderr ?? ''))) throw error
    // GitHub's tag endpoint does not expose drafts; resolve the authenticated list by ID.
    const pages = JSON.parse(gh(['api', `repos/${repository}/releases?per_page=100`, '--paginate', '--slurp']))
    const matching = pages.flat().find(item => item.tag_name === releaseTag)
    if (!matching) return null
    if (!Number.isSafeInteger(matching.id) || matching.id <= 0) throw new Error('Invalid release ID.')
    return JSON.parse(gh(['api', `repos/${repository}/releases/${matching.id}`]))
  }
}
...
export function syncReleaseApks(tag, artifactDirectory, { allowDraft = false, requireApk = false, sourceOnly = false, gh = runGh } = {}) {
  ...
  const release = (releaseTag) => getReleaseByTag(releaseTag, gh)
  const digest = (asset) => {
