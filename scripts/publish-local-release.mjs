...
export async function publishLocalRelease({ source, tag, resume = false }, {
  ...
  assert(source && /^v\d+\.\d+\.\d+$/.test(tag ?? ''), 'Usage: node scripts/publish-local-release.mjs --source <clean-public-worktree> --tag v<version> [--resume]')
  ...
  assert(config.version === version && json(join(source, 'package.json')).scripts['tauri:build'] === 'tauri build', 'The source must be the generated public package with the requested version.')
  ...
  const ensureRelease = (releaseTag, prerelease) => {
    ...
    if (!current) {
      gh(['release', 'create', releaseTag, '--repo', repository, '--verify-tag', '--target', commit, '--title', `Super High${prerelease ? ' Android' : ''} ${releaseTag.replace(/^android-/, '')}`, '--notes-file', notesPath, '--draft', '--latest=false', ...(prerelease ? ['--prerelease'] : [])])
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  while (args.length) {
    ...
    else if (['--source', '--tag'].includes(name) && args[0] && !args[0].startsWith('--')) options[name.slice(2)] = args.shift()
